...
if (typeof document !== "undefined") document.title = "DocWorks v" + DOCWORKS_VERSION;
  ...
  try {
    ...
    // Chrome necesita un recurso de icono real. Como LOGO_B64 apunta a /logo.png,
    // generamos el favicon desde esa misma imagen cuando carga la app.
    ...
    iconImg.onload = function(){
      try {
        ...
        var scale = Math.min(64/iconImg.width,64/iconImg.height);
        var w = iconImg.width*scale, h = iconImg.height*scale;
        c.drawImage(iconImg,(64-w)/2,(64-h)/2,w,h);
        fav.href = canvas.toDataURL("image/png");
      } catch(e2) { fav.href = LOGO_B64 + "?v=" + DOCWORKS_VERSION; }
    };
    iconImg.onerror = function(){ fav.href = LOGO_B64 + "?v=" + DOCWORKS_VERSION; };
    iconImg.src = LOGO_B64 + "?v=" + DOCWORKS_VERSION;
  } catch(e) {}
        ...
        return(
          <div>
            <div className="card" style={{padding:"12px 14px",marginBottom:14}}>
              <div style={{fontSize:13,fontWeight:600,color:"var(--text)",marginBottom:3}}>Plantilla del documento</div>
              <div style={{fontSize:11.5,color:"var(--dim)",lineHeight:1.5}}>Elegí el tipo de documento y ordená sus cláusulas. Todo lo demás queda automático.</div>
            </div>
            ...
            {puedeEditarPlantillas && lista.length>0 && (
              <div style={{fontSize:11,color:"var(--dim)",marginBottom:10,display:"flex",alignItems:"flex-start",gap:6}}>
                <span>☑</span>
                <span>Ordená las cláusulas con <b>arrastrar y soltar</b> o con ↑ ↓. El orden define cómo salen en el documento. Usá «Incluir por defecto» para que aparezcan marcadas en operaciones nuevas y 🔒 «Obligatoria» para impedir que el broker las quite.</span>
              </div>
            )}
                ...
                return(
                  <div key={c.id} className="op-card" draggable={puedeEditarPlantillas}
                    ...
                    onDrop={function(e){
                      ...
                      moveClausulaTo(fromId,c.id,listaIds);
