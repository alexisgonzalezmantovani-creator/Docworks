...
async function generarPDF(doc, logoDataUrl, perfil) {
  try {
    ...
    // Firmas principales: centradas. Espacio moderado para no empujar
    // PRESTA CONFORMIDAD / PROPIETARIO a una página casi vacía.
    ensureSpace(96);
    hline(y-10); y-=32;
    ...
    // Presta Conformidad (reserva): aceptar + firma del propietario en el
    // mismo bloque para evitar una última página casi vacía.
    if(doc.conformidad&&doc.conformidad.items&&doc.conformidad.items.length){
      ...
      confItems.forEach(function(txt,i){
        // Antes del último ítem, reservar espacio para firma del propietario.
        if(i===confItems.length-1) ensureSpace(92);
        drawParagraph([{txt:(i+1)+". ",bold:true}], txt, {spaceAfter:8});
      });
      y-=6;
      var cxConformidad=ML+(W-ML-MR)/2;
      var sigWConformidad=Math.min(180,Math.max(120,W-ML-MR-24));
      lines.push("0.5 G 0.5 w "+(cxConformidad-sigWConformidad/2)+" "+(y+8)+" m "+(cxConformidad+sigWConformidad/2)+" "+(y+8)+" l S 0 G");
      textCentered(cxConformidad,y,"PROPIETARIO",8,false);
      textCentered(cxConformidad,y-12,doc.conformidad.firmante||"",8,true);
      if(doc.conformidad.dni) textCentered(cxConformidad,y-23,tipoIdentificacion(doc.conformidad.dni)+": "+doc.conformidad.dni,8,false);
    }
