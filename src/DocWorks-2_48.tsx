// DocWorks 2.48 — ajustes de guardado, encabezados y identificación DNI/CUIT
  ...
  try {
    ...
    if(perfil&&perfil.nombre){
      ...
      if(encNombre||encMatricula||encWeb){
        var nombreEnc=perfil.nombre.toUpperCase();
        var matEnc=perfil.matricula ? "Mat. "+perfil.matricula : "";
        var webEnc=perfil.web||"";
        if(estilo==="corporativo"){
          // Franja compacta: inmobiliaria y matrícula en la misma línea; web debajo.
          rect(ML,y-10,W-ML-MR,34,rgbPdfStr(colorPrim));
          var blanco={r:1,g:1,b:1}, blancoTenue=tintRgb(colorPrim,0.55);
          if(encNombre) text(ML+8,y+6,nombreEnc,12,true,0,blanco);
          if(encMatricula&&matEnc) text(W-MR-8-mw(matEnc,8,false),y+6,matEnc,8,false,0,blancoTenue);
          if(encWeb&&webEnc) text(ML+8,y-7,webEnc,7,false,0,blancoTenue);
        } else if(esClasico){
          // Caja con doble filete, sin color — estilo escribanía tradicional.
          hline(y+8,0.3,null,0.75);
          if(encNombre) text(centeredX(nombreEnc,12,true),y+6,nombreEnc,12,true);
          if(encMatricula&&matEnc) text(W-MR-8-mw(matEnc,8,false),y+6,matEnc,8,false);
          if(encWeb&&webEnc) text(centeredX(webEnc,7,false),y-7,webEnc,7,false);
          hline(y-14,0.3,null,0.75);
        } else {
          // Minimalista: tinte muy suave del color de marca, sin bloque duro.
          rect(ML,y-10,W-ML-MR,34,rgbPdfStr(tintRgb(colorPrim,0.94)));
          if(encNombre) text(ML+8,y+6,nombreEnc,12,true,0,colorPrim);
          if(encMatricula&&matEnc) text(W-MR-8-mw(matEnc,8,false),y+6,matEnc,8,false);
          if(encWeb&&webEnc) text(ML+8,y-7,webEnc,7,false);
        }
        y-=38;
      }
    }
...
function buildDocSectionsPorBloques(op, bloques, clausulasLib) {
  ...
  var doc = {
    ...
    encabezado: "Recibimos de "+(op.comprador_nombre||"___________")+", "+etiquetaIdentificacion(op.comprador_dni)+" "+(op.comprador_dni||"___________")+", con domicilio a estos efectos en "+(op.comprador_domicilio||"___________")+", en adelante el Oferente, la suma de "+monedaTxt+" "+fmt$L(op.anticipo,op.moneda)+" en concepto de \"Reserva Ad Referéndum\" de la aceptación del propietario "+(op.vendedor_nombre||"___________")+", "+etiquetaIdentificacion(op.vendedor_dni)+" "+(op.vendedor_dni||"___________")+", en adelante el Propietario, para la compra del inmueble ubicado en "+(op.inmueble_direccion||"___________")+", "+(op.inmueble_partido||"___________")+", Provincia de "+(op.inmueble_provincia||"___________")+(op.nomenclatura_catastral?", Nomenclatura Catastral "+op.nomenclatura_catastral:"")+", en adelante el Inmueble.\n\nEn adelante Oferente y Propietario podrán ser denominados conjuntamente como Las Partes.\n\nEl presente se sujetará en un todo a los siguientes términos y condiciones:",
...
function buildDocSections(op, clausulasLib, tipo) {
  ...
  var garantiaDetalle = (op.alquiler_garantia_texto_personalizado && op.alquiler_garantia_texto_personalizado.trim())
    ...
    ? "Se constituye garantía propietaria a cargo de "+(op.alquiler_garantia_titular||"___")+", "+etiquetaIdentificacion(op.alquiler_garantia_dni)+" "+(op.alquiler_garantia_dni||"___")+", titular del inmueble sito en "+(op.alquiler_garantia_inmueble||"___")+".-"
...
async function generarDOCX(doc, perfil) {
  try {
    ...
    if (perfil && perfil.nombre) {
      ...
      var headerRunsD=[];
      if(encNombreD) headerRunsD.push({text:perfil.nombre.toUpperCase(),bold:true,size:14,color:DOCX_COLOR_PRIM});
      if(encMatriculaD && perfil.matricula) headerRunsD.push({text:"    Mat. "+perfil.matricula,size:9,color:"666666"});
      if(headerRunsD.length) body += pxRuns(headerRunsD,{center:true,spacing:{before:60,after:20}});
      if(encWebD && perfil.web) body += px(perfil.web,{size:9,center:true,color:"666666",spacing:{before:0,after:25}});
  ...
  return(
    <div>
      ...
      <div className="grid2">
        ...
        <Inp label={tipoIdentificacion(f("dni"))} value={f("dni")} onChange={s("dni")} placeholder="20-12345678-9"/>
          ...
          return(
            <div key={i} style={{padding:"10px 12px",borderRadius:10,background:"rgba(255,255,255,0.02)",border:"1px solid var(--border)",marginBottom:8}}>
              ...
              <div className="grid2">
                ...
                <Inp label={tipoIdentificacion(p.dni||"")} value={p.dni||""} onChange={function(e){actualizarCotitular(i,"dni",e.target.value);}} placeholder="27-98765432-1"/>
    ...
    <div>
      ...
      {data.alquiler_garantia_tipo==="propietario"&&(
        <div className="grid2" style={{marginTop:12,paddingTop:12,borderTop:"1px solid var(--border)"}}>
          ...
          <Inp label={tipoIdentificacion(data.alquiler_garantia_dni||"")+" del garante"} value={data.alquiler_garantia_dni||""} onChange={function(e){onChange("alquiler_garantia_dni",e.target.value);}} placeholder="20-98765432-1"/>
...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  function savePerfil(){
    ...
    try {
      onChange(localPerfil);
      window.setTimeout(function(){
        try {
          var persisted=lsGet("perfil", null);
          var ok=persisted && JSON.stringify(persisted)===JSON.stringify(localPerfil);
          if(ok){
            setPerfilGuardado(true);
            setPerfilGuardando(false);
            window.setTimeout(function(){setPerfilGuardado(false);},2800);
          } else {
            setPerfilError(true);
            setPerfilGuardando(false);
          }
        ...
        <div style={{maxWidth:480}}>
          ...
          <div className="card" style={{padding:20,marginTop:14}}>
            ...
            {(function(){
              ...
              return (
                <div style={{border:"1px solid var(--border2)",borderRadius:10,overflow:"hidden",background:"#ffffff"}}>
                  <div style={{padding:"11px 16px 9px",background:headerBg,borderBottom:est==="clasico"?"2px solid #1a1a1a":"none",fontFamily:fam}}>
                    <div style={{display:"flex",alignItems:"baseline",justifyContent:est==="clasico"?"center":"space-between",gap:12}}>
                      <div style={{fontSize:13,fontWeight:700,color:headerColor,letterSpacing:"0.02em",textAlign:est==="clasico"?"center":"left",minWidth:0}}>{(localPerfil.nombre||"TU INMOBILIARIA").toUpperCase()}</div>
                      <div style={{fontSize:10,color:est==="corporativo"?"#ffffffaa":"#64748b",whiteSpace:"nowrap"}}>Mat. {localPerfil.matricula||"1234"}</div>
                    </div>
                    {localPerfil.web&&<div style={{fontSize:9,color:est==="corporativo"?"#ffffffaa":"#64748b",marginTop:2,textAlign:est==="clasico"?"center":"left"}}>{localPerfil.web}</div>}
                  </div>
            ...
            <div style={{paddingTop:16,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              <Btn v="success" onClick={savePerfil} disabled={perfilGuardando}>{perfilGuardando?"Guardando…":"Guardar cambios ✓"}</Btn>
              {perfilGuardando&&<span style={{fontSize:12,color:"var(--muted)"}}>Guardando…</span>}
              {perfilGuardado&&<span style={{fontSize:12.5,color:"var(--green)",fontWeight:700}}>✓ Cambios guardados</span>}
              {perfilError&&<span style={{fontSize:12.5,color:"var(--red)",fontWeight:700}}>No se pudieron guardar los cambios.</span>}
            </div>
          </div>
        </div>
      )}
          ...
          <div className="card" style={{padding:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              <div style={{paddingTop:8,display:"flex",flexDirection:"column",gap:8}}>
                <Btn v="success" onClick={savePerfil} disabled={perfilGuardando} style={{width:"100%"}}>{perfilGuardando?"Guardando…":"Guardar cambios ✓"}</Btn>
                {perfilGuardado&&<div style={{fontSize:12.5,color:"var(--green)",fontWeight:700,textAlign:"center"}}>✓ Cambios guardados</div>}
                {perfilError&&<div style={{fontSize:12.5,color:"var(--red)",fontWeight:700,textAlign:"center"}}>No se pudieron guardar los cambios.</div>}
              </div>
            </div>
          </div>
        </div>
