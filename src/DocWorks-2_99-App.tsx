...
// DocWorks 2.99 — personalización amigable del texto de PRECIO + activación de
...
function EstadisticasView({operaciones,equipo,perfil}){
  ...
  var usd=operacionesRaiz.filter(function(o){return o.moneda==="USD"&&o.precio;}).reduce(function(s,o){return s+parseFloat(o.precio||0);},0);
  var ars=operacionesRaiz.filter(function(o){return o.moneda==="ARS"&&o.precio;}).reduce(function(s,o){return s+parseFloat(o.precio||0);},0);
  var canon=operacionesRaiz.filter(function(o){return o.tipo==="alquiler"&&o.alquiler_monto_inicial;}).reduce(function(s,o){return s+parseFloat(o.alquiler_monto_inicial||0);},0);
  ...
  var comisiones=operacionesRaiz.map(function(o){var c=comisionOperacion(o);var brokerShare=o.broker_id?c.monto*splitBroker/100:0;return Object.assign({op:o,brokerShare:brokerShare,inmobiliariaShare:c.monto-brokerShare},c);});
  ...
  var porTipo=[
    {name:"Compraventas",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).some(function(x){return x.tipo==="reserva"||x.tipo==="boleto";});}).length,color:"#60a5fa"},
    {name:"Alquileres",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).some(function(x){return x.tipo==="reserva_alquiler"||x.tipo==="alquiler";});}).length,color:"#2dd4bf"},
    {name:"Otros",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).every(function(x){return !["reserva","boleto","reserva_alquiler","alquiler"].includes(x.tipo);});}).length,color:"#a78bfa"},
  ...
  return(
    <div>
      <div className="section-header">
        <div><div className="section-title">Estadísticas</div><div className="section-sub">Una lectura clara de tus operaciones, sin contar documentos vinculados dos veces.</div></div>
      </div>

      {/* Resumen global */}
      <div className="stats-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
        {[{l:"Total operaciones",v:total,i:"📁"},{l:"Activas",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).some(function(x){return x.estado==="activo";});}).length,i:"🟢"},{l:"Alquileres",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).some(function(x){return x.tipo==="alquiler"||x.tipo==="reserva_alquiler";});}).length,i:"🏠"},{l:"Compraventas",v:operacionesRaiz.filter(function(o){return grupoRaiz(o).some(function(x){return x.tipo==="boleto"||x.tipo==="reserva";});}).length,i:"📄"}].map(function(s){
      ...
      <div className="card" style={{padding:18,marginBottom:20,borderRadius:16,borderColor:"rgba(212,168,83,.20)",background:"linear-gradient(135deg,rgba(212,168,83,.06),transparent)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12,flexWrap:"wrap",marginBottom:14}}>
          <div><div style={{fontSize:14,fontWeight:800,color:"var(--text)"}}>💰 Comisiones y split</div><div style={{fontSize:10.5,color:"var(--dim)",marginTop:3}}>Este mes · el split configurado para el broker es {splitBroker}%</div></div>
          <div style={{fontSize:10.5,color:"var(--gold)",fontWeight:700,padding:"6px 9px",borderRadius:20,background:"rgba(212,168,83,.10)"}}>Broker {splitBroker}% · Inmobiliaria {100-splitBroker}%</div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:8}}>
          {[{l:"Honorarios · USD",v:"U$D "+(totalBrokerUsd+totalInmoUsd).toLocaleString("es-AR")},{l:"Broker · USD",v:"U$D "+totalBrokerUsd.toLocaleString("es-AR")},{l:"Inmobiliaria · USD",v:"U$D "+totalInmoUsd.toLocaleString("es-AR")},{l:"Honorarios · ARS",v:"$ "+(totalBrokerArs+totalInmoArs).toLocaleString("es-AR")},{l:"Broker · ARS",v:"$ "+totalBrokerArs.toLocaleString("es-AR")},{l:"Inmobiliaria · ARS",v:"$ "+totalInmoArs.toLocaleString("es-AR")}].map(function(k){return <div key={k.l} style={{padding:"11px 12px",borderRadius:11,background:"rgba(255,255,255,.025)",border:"1px solid var(--border)"}}><div style={{fontSize:9.5,color:"var(--dim)",marginBottom:4}}>{k.l}</div><div style={{fontSize:16,fontWeight:800,color:"var(--text)"}}>{k.v}</div></div>})}
        </div>
      </div>
...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  return (
    <div>
      ...
      {tab==="cuenta"&&(
        <div style={{maxWidth:980,margin:"0 auto"}}>
          <div style={{
            ...
            <div style={{fontSize:12,color:"var(--gold)",fontWeight:600}}>{localPerfil.matricula?"Mat. "+localPerfil.matricula:(localPerfil.nombre_usuario||"Usuario")}</div>
      ...
      {tab==="cuenta"&&<div className="card" style={{padding:"18px 20px",marginTop:14,borderRadius:14,borderColor:"rgba(212,168,83,.18)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap",marginBottom:12}}><div><div style={{fontSize:14.5,fontFamily:"DM Serif Display,serif",color:"var(--text)"}}>Comisión del broker</div><div style={{fontSize:10.8,color:"var(--dim)",marginTop:3}}>Definí una sola vez qué porcentaje de los honorarios recibe el broker. DocWorks lo aplica automáticamente en las estadísticas.</div></div><div style={{fontSize:20,fontWeight:800,color:"var(--gold)"}}>{localPerfil.broker_split_pct==null?50:localPerfil.broker_split_pct}%</div></div>
        <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) 180px",gap:14,alignItems:"end"}}><div><input type="range" min="0" max="100" step="5" value={localPerfil.broker_split_pct==null?50:localPerfil.broker_split_pct} onChange={function(e){setLocalPerfil(function(p){return Object.assign({},p,{broker_split_pct:parseInt(e.target.value,10)});});}} style={{width:"100%",accentColor:"var(--gold)"}}/><div style={{display:"flex",justifyContent:"space-between",fontSize:9.5,color:"var(--dim)",marginTop:3}}><span>0% · Todo inmobiliaria</span><span>50% · Mitad y mitad</span><span>100% · Todo broker</span></div></div><Inp label="Split del broker (%)" type="number" min="0" max="100" value={localPerfil.broker_split_pct==null?50:localPerfil.broker_split_pct} onChange={fP("broker_split_pct")} hint="Ej.: 35, 40 o 50"/></div>
      </div>}
...
const DEFAULT_PERFIL={nombre:"",nombre_usuario:"",matricula:"",telefono:"",email:"",logoDataUrl:"",logoPosicion:"derecha",logoScale:100,cuit:"",direccion:"",web:"",rol:"dueno",pie_pagina_texto:"",pie_pagina_leyenda_activa:true,pie_pagina_logo_debajo:false,pie_pagina_logo_posicion:"centro",pie_pagina_logo_scale:100,pie_pagina_numero_activo:true,pie_pagina_numero_posicion:"derecha",broker_split_pct:50,clausulas_default_ids:[],plantillas:{},colorPrimario:"#142a4d",colorSecundario:"#c9a227",plantillaEstilo:"corporativo",encabezados_custom:{}};
