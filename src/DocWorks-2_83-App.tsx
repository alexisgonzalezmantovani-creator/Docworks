      ...
      {tab==="auditoria"&&(function(){
        ...
        return <div>
          ...
          <div style={{display:"flex",gap:7,marginBottom:10,flexWrap:"wrap"}}>
            <Inp value={auditQuery} onChange={function(e){setAuditQuery(e.target.value);}} placeholder="Buscar…" style={{flex:"1 1 180px",minWidth:160}}/>
              ...
              return <div key={en.id} className="card" style={{padding:"10px 12px",borderColor:abierta?"rgba(212,168,83,.30)":"var(--border)"}}>
                <button type="button" onClick={function(){setAuditExpandida(abierta?null:en.id);}} style={{width:"100%",border:0,background:"transparent",padding:0,color:"inherit",cursor:cambios.length?"pointer":"default",textAlign:"left"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6,marginTop:5,flexWrap:"wrap"}}>
                    ...
                    {cambios.length>0&&<span style={{fontSize:10.5,color:"var(--gold)"}}>· Modificó: {resumen}</span>}
                    {cambios.length>0&&<span style={{fontSize:10,color:"var(--dim)"}}>{abierta?"▲":"▼"}</span>}
                  </div>
                </button>
                {abierta&&cambios.length>0&&<div style={{marginTop:9,paddingTop:9,borderTop:"1px solid var(--border)",display:"flex",flexDirection:"column",gap:7}}>
                  {cambios.map(function(c,i){return <div key={i} style={{padding:"8px 9px",borderRadius:8,background:"rgba(255,255,255,.018)",border:"1px solid var(--border2)"}}>
                    <div style={{fontSize:10.8,fontWeight:700,color:"var(--text)",marginBottom:5}}>Modificó: {c.label}</div>
                    <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:7}}>
                      <div><div style={{fontSize:8.8,textTransform:"uppercase",letterSpacing:".05em",color:"var(--red)",marginBottom:3}}>Antes</div>{cajaValor(c.antes,!c.antes)}</div>
                      <div><div style={{fontSize:8.8,textTransform:"uppercase",letterSpacing:".05em",color:"var(--green)",marginBottom:3}}>Después</div>{cajaValor(c.despues,!c.despues)}</div>
                    </div>
                  </div>})}
                </div>}
              </div>;
