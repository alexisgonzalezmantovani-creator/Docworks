...
// DocWorks 2.62 — personalización amigable del texto de PRECIO + activación de
...
const PLANTILLA_TIPOS = [
  "reserva", "boleto", "reserva_alquiler", "alquiler", "comodato", "exclusividad"
];
...
function normalizarNarrativaPrecio(texto){
  ...
  if(!t) return "El Oferente ofrece la suma de {{moneda_txt}} {{precio_letras}} para la compra del Inmueble, en adelante el Precio.-";
  // La inmobiliaria redacta libremente; estos dos datos siempre siguen viniendo
  // del formulario de Operaciones. Si los omite, los agregamos al final sin
  // mostrar tokens técnicos en la interfaz.
  var tieneMoneda=t.indexOf("{{moneda_txt}}")!==-1;
  var tienePrecio=t.indexOf("{{precio_letras}}")!==-1;
  if(!tieneMoneda || !tienePrecio){
    var faltantes=[];
    if(!tieneMoneda) faltantes.push("{{moneda_txt}}");
    if(!tienePrecio) faltantes.push("{{precio_letras}}");
    t += (/[\s.,:;-]$/.test(t)?" ":" — ") + faltantes.join(" ");
  }
  return t;
}

var PRECIO_NARRATIVAS_PRESET = [
  {label:"Estándar", texto:"El Oferente ofrece la suma de {{moneda_txt}} {{precio_letras}} para la compra del Inmueble, en adelante el Precio.-"},
  {label:"Precio total", texto:"El precio total de la operación se fija en {{moneda_txt}} {{precio_letras}}, que las partes reconocen como justo y conveniente.-"},
  {label:"Oferta", texto:"El Oferente formula oferta por {{moneda_txt}} {{precio_letras}} para la adquisición del Inmueble.-"},
  {label:"Más breve", texto:"Precio ofrecido: {{moneda_txt}} {{precio_letras}}.-"}
];
...
function PrecioNarrativaEditor({bloque, onSave, onCancel}){
  ...
  return (
    <div style={{marginTop:8,padding:12,borderRadius:10,background:"rgba(212,168,83,0.055)",border:"1px solid rgba(212,168,83,0.22)"}}>
      ...
      <div style={{fontSize:10.8,color:"var(--dim)",lineHeight:1.45,marginBottom:9}}>Elegí una redacción o escribí la propia. DocWorks mantiene automáticamente vinculados la moneda y el precio cargados en la operación.</div>
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:9}}>
        {PRECIO_NARRATIVAS_PRESET.map(function(preset){
          return <button key={preset.label} type="button" onClick={function(){setTexto(preset.texto);}}
            style={{padding:"6px 9px",borderRadius:8,border:"1px solid var(--border2)",background:"var(--card)",color:"var(--muted)",cursor:"pointer",fontSize:10.5}}>{preset.label}</button>;
        })}
      </div>
      <label className="field-label" style={{display:"block",marginBottom:5}}>Redacción del precio</label>
      <textarea className="inp" rows={4} value={texto} onChange={function(e){setTexto(e.target.value);}}
        style={{width:"100%",resize:"vertical",boxSizing:"border-box"}} />
      <div style={{fontSize:10,color:"var(--dim)",marginTop:5}}>La moneda y el importe se reemplazan automáticamente con los datos de la operación.</div>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:10}}>
        <div style={{fontSize:10.5,color:status==="error"?"var(--red)":"var(--dim)"}}>{status==="saving"?"Guardando…":status==="saved"?"Guardado ✓":status==="error"?"No se pudo guardar. Intentá de nuevo.":""}</div>
        <div style={{display:"flex",gap:7}}>
          <Btn s="sm" v="ghost" onClick={onCancel}>Cancelar</Btn>
          <Btn s="sm" onClick={guardar} disabled={status==="saving"}>Guardar</Btn>
        </div>
      </div>
    </div>
  );
...
function ReservaBloquesPanel({bloques, onUpdate, onMove, clausulasLib, operaciones, puedeEditar}){
  ...
  const [mostrarVistaControl,setMostrarVistaControl]=useState(false);
  var bloquesSeguros = Array.isArray(bloques) ? bloques : [];
  var ordenados = bloquesSeguros.slice().sort(function(a,b){return (a.orden||0)-(b.orden||0);});
  ...
  // La vista de control es pesada y trabaja con documentos completos. Se calcula
  // únicamente cuando el usuario la pide, para que nunca pueda bloquear la edición
  // de los bloques ni la apertura de esta pantalla.
  var comparacion = null;
  if (mostrarVistaControl) {
    try {
      comparacion = compararGeneradoresReserva(opComparacion, bloquesSeguros, clausulasLib||[]);
    } catch(e) {
      comparacion = {error:true, filas:[], conformidadOk:false, encabezadoOk:false, todoOk:false, docViejo:null, docNuevo:null};
    }
  }
  ...
  return (
    <div style={{marginBottom:20}}>
      ...
      <div style={{fontSize:11,color:"var(--dim)",marginBottom:12,lineHeight:1.5}}>
        Este es el cuerpo estándar del documento. La inmobiliaria puede adaptar el texto de cada bloque. Los campos entre {"{{llaves}}"} son variables conectadas al formulario de Operaciones: si cambiás el texto pero conservás una variable, el Broker seguirá cargando ese dato normalmente. El PDF y Word usan estos bloques.
      </div>
      ...
      {ordenados.map(function(b,idx){
        ...
        return (
          <div key={b.id} style={{padding:"10px 12px",borderRadius:10,border:"1px solid var(--border2)",marginBottom:6,background:"var(--card)"}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:12.5,fontWeight:600,color:"var(--text)"}}>{b.sinTitulo?"Bloque sin título":((b.titulo&&String(b.titulo).trim())?String(b.titulo).trim():"Bloque sin título")}</span>
              ...
              {puedeEditar&&!editando&&(
                <div style={{display:"flex",gap:3,flexShrink:0}}>
                  ...
                  {b.id==="reserva.precio" ? <button onClick={function(){setPrecioEditando(true);setEditId(null);}} title="Personalizar cómo se narra el precio" style={{padding:"4px 7px",borderRadius:6,background:"rgba(212,168,83,0.08)",border:"1px solid rgba(212,168,83,0.25)",color:"var(--gold)",cursor:"pointer",fontSize:12}}>✎ Precio</button> : <button onClick={function(){abrirEditor(b);}} title="Editar" style={{padding:"4px 7px",borderRadius:6,background:"rgba(255,255,255,0.04)",border:"1px solid var(--border)",color:"var(--muted)",cursor:"pointer",fontSize:12}}>✏</button>}
                </div>
              )}
            </div>
            {editando&&(
            ...
            {precioEditando && b.id==="reserva.precio" && puedeEditar && (
              <PrecioNarrativaEditor bloque={b} onSave={function(cambios){
                var r=onUpdate(b.id,cambios);
                return Promise.resolve(r);
              }} onCancel={function(){setPrecioEditando(false);}} />
            )}
          </div>
        );
      })}

      {/* La vista de control queda separada de la edición. Nunca se monta al abrir
          el cuerpo editable; solo se ejecuta si el usuario la solicita. */}
      <div style={{marginTop:18,paddingTop:14,borderTop:"1px dashed var(--border2)"}}>
        <button type="button" onClick={function(){setMostrarVistaControl(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}>
          <span style={{display:"flex",alignItems:"center",gap:7}}>
            <span style={{fontSize:14}}>🧪</span>
            <span style={{fontSize:12,fontWeight:650}}>Vista de control del documento</span>
          </span>
          <span style={{fontSize:10.5,color:"var(--dim)"}}>{mostrarVistaControl?"▲ Ocultar":"▼ Mostrar"}</span>
        </button>
        {mostrarVistaControl&&<div style={{marginTop:10}}>
          {comparacion && comparacion.error ? (
            <div style={{padding:"10px 12px",borderRadius:10,border:"1px solid var(--border)",background:"rgba(255,255,255,.02)",color:"var(--dim)",fontSize:11}}>La vista de control no está disponible para esta configuración, pero la plantilla se puede editar normalmente.</div>
          ) : comparacion && (
            <React.Fragment>
              {opsReserva.length>0&&(<select value={origenComparacion} onChange={function(e){setOrigenComparacion(e.target.value);}} style={{marginBottom:10,padding:"7px 10px",borderRadius:8,border:"1px solid var(--border)",background:"var(--card)",color:"var(--text)",fontSize:12}}>
              ...
              <div style={{padding:"10px 12px",borderRadius:10,border:"1px solid "+(comparacion.todoOk?"rgba(34,197,94,0.3)":"rgba(248,113,113,0.3)"),background:comparacion.todoOk?"rgba(34,197,94,0.06)":"rgba(248,113,113,0.06)"}}>
                ...
                {!comparacion.todoOk&&<div style={{marginTop:8,fontSize:11,color:"var(--muted)"}}>
                  ...
                  {Array.isArray(comparacion.filas)&&comparacion.filas.filter(function(f){return !f.ok;}).map(function(f){return <div key={f.idx} style={{marginBottom:4}}>⚠ Cláusula #{f.idx+1}: {f.actual?("actual: "+String(f.actual.titulo||"")):"no existe en el actual"} — {f.bloques?("bloques: "+String(f.bloques.titulo||"")):"no existe en bloques"}</div>;})}
                </div>}
              </div>
              {comparacion.docNuevo && <div style={{marginTop:18,paddingTop:14,borderTop:"1px dashed var(--border2)"}}>
                <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:8}}>
                  <span style={{fontSize:14}}>👁</span>
                  <p style={{fontFamily:"DM Serif Display,serif",fontSize:14,color:"var(--text)",margin:0}}>Vista previa completa del documento</p>
                </div>
...
function buildDocSections(op, clausulasLib, tipo, bloquesPlantilla) {
  if (tipo === "reserva" && Array.isArray(bloquesPlantilla) && bloquesPlantilla.length) {
    return buildDocSectionsPorBloques(op, bloquesPlantilla, clausulasLib || []);
  }
...
function DocumentViewer({op,clausulas,perfil,operaciones,bloquesReserva,onToggleTarea,tab,onChangeTab,autoAction,onAutoActionDone}){
  ...
  function getDoc(){
    ...
    return aplicarEncabezadoPersonalizado(buildDocSections(opParaDocumento(op), clausulas, tab, bloquesReserva), op, tab, perfil);
  }
  ...
  return (
    <div>
      ...
      {tab==="plantillas"&&(function(){
        ...
        var customMapPreview = (perfil&&perfil.encabezados_custom)||{};
        ...
        return <div>
          <div className="card" style={{padding:13,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div><div style={{fontFamily:"DM Serif Display,serif",fontSize:18,color:"var(--text)"}}>Plantillas</div><div style={{fontSize:10.9,color:"var(--dim)",marginTop:3}}>Cada plantilla parte de un modelo estándar que la inmobiliaria puede adaptar a su gusto, sin romper los campos del formulario de Operaciones.</div></div>
              {defaultsLocked?<SmallPill tone="green">🔒 Plantilla protegida</SmallPill>:<SmallPill tone="gold">⚙ En edición</SmallPill>}
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"minmax(210px,.72fr) minmax(0,1.7fr)",gap:10,alignItems:"start"}}>
            <div className="card" style={{padding:9}}>
              <div style={{fontSize:10.2,color:"var(--dim)",fontWeight:700,letterSpacing:".06em",textTransform:"uppercase",padding:"5px 7px 8px"}}>Modelos de documento</div>
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {PLANTILLA_TIPOS.map(function(tipo){
                  ...
                  return <button key={tipo} onClick={function(){setTipoDocSel(tipo);setPlantillaForm(null);setDragClausulaId(null);setEncabezadoEditing(false);setConfiguracionPlantillaAbierta(false);setMostrarPreviewConfiguracion(false);setMostrarBloquesConfiguracion(false);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,width:"100%",border:active?"1px solid rgba(212,168,83,.25)":"1px solid transparent",borderRadius:9,padding:"9px 10px",background:active?"rgba(212,168,83,.10)":"transparent",color:active?"var(--text)":"var(--muted)",cursor:"pointer",textAlign:"left"}}>
                    <span style={{display:"flex",alignItems:"center",gap:7,fontSize:11.2,fontWeight:active?650:500}}><span>{TIPO_ICON[tipo]||"📄"}</span>{TIPOS[tipo]}</span>
                    <span style={{fontSize:9.7,color:cfg.locked?"var(--green)":"var(--dim)"}}>{cfg.locked?"🔒":""} {count}</span>
                  </button>;
                })}
              </div>
            </div>
            <div>
              {renderForm()}
              <div className="card" style={{padding:13,marginBottom:9}}>
                <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
                  <div><div style={{fontSize:13.5,fontWeight:650,color:"var(--text)"}}>Cláusulas de esta plantilla</div><div style={{fontSize:10.7,color:"var(--dim)",marginTop:3}}>{TIPOS[tipoDocSel]} · {listaPlantilla.length} cláusula{listaPlantilla.length===1?"":"s"}</div></div>
                  {defaultsLocked&&<SmallPill tone="green">🔒 Fijada</SmallPill>}
                </div>
                <div style={{fontSize:10.5,color:"var(--dim)",lineHeight:1.5,margin:"9px 0 11px"}}>La plantilla define el documento completo: qué bloques tiene, qué texto usa y en qué orden. Podés adaptar el texto estándar sin modificar los campos que el Broker carga en Operaciones.</div>
                ...
                {puedeAgregar&&<div style={{marginTop:12,paddingTop:11,borderTop:"1px solid var(--border)"}}>
                  ...
                  <div style={{display:"flex",gap:7,alignItems:"center"}}>
                    <select className="inp" style={{flex:1,minWidth:0}} defaultValue="" onChange={function(e){if(e.target.value){agregarClausula(e.target.value);e.target.value="";}}}><option value="">Seleccioná una cláusula de la Biblioteca…</option>{disponibles.map(function(c){return <option key={c.id} value={c.id}>{(c.titulo&&String(c.titulo).trim())?String(c.titulo).trim():"Cláusula sin título"}{CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;})?" · Estándar":" · Propia"}</option>;})}</select>
                    <Btn s="sm" onClick={nuevaClausula}>＋ Nueva</Btn>
                  </div>
                </div>}
              </div>
              <div className="card" style={{padding:13}}>
                <button type="button" onClick={function(){setConfiguracionPlantillaAbierta(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}>
                  <span style={{fontSize:12.2,fontWeight:650}}>Configuración del documento</span>
                  <span style={{fontSize:11,color:"var(--dim)"}}>{configuracionPlantillaAbierta?"▲ Ocultar":"▼ Mostrar"}</span>
                </button>
                {configuracionPlantillaAbierta&&<div style={{marginTop:12,display:"flex",flexDirection:"column",gap:10}}>
                  {!puedeEditarPlantillas&&<div style={{padding:"8px 10px",borderRadius:9,background:"rgba(251,191,36,.07)",border:"1px solid rgba(251,191,36,.17)",color:"var(--amber)",fontSize:10.8}}>🔒 Solo Dueño y Administrador pueden modificar esta configuración.</div>}
                  ...
                  <div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:5,flexWrap:"wrap"}}>
                      <div style={{fontSize:11.8,fontWeight:650,color:"var(--text)"}}>Encabezado del documento</div>
                    ...
                    <div style={{fontSize:10.4,color:"var(--dim)",lineHeight:1.45,marginBottom:9}}>Es el texto introductorio que aparece al comienzo del documento. Podés escribirlo libremente y conservar los campos entre {"{{llaves}}"} para que DocWorks los complete automáticamente.</div>
                    {puedeEditarPlantillas && !encabezadoEditing && <div style={{display:"flex",justifyContent:"flex-end",gap:6,marginBottom:8}}>
                      <button type="button" onClick={function(){setEncabezadoDraft(customTextoPreview);setEncabezadoEditing(true);}} style={{height:30,padding:"0 10px",borderRadius:8,border:"1px solid rgba(212,168,83,.25)",background:"rgba(212,168,83,.07)",color:"var(--gold)",cursor:"pointer",fontSize:10.5}}>✏ Editar encabezado</button>
                      {customTextoPreview&&<button type="button" onClick={function(){var mapa=Object.assign({},customMapPreview);delete mapa[tipoDocSel];var next=Object.assign({},perfil,{encabezados_custom:mapa});Promise.resolve(onChange(next)).then(function(){setEncabezadoDraft("");setEncabezadoEditing(false);}).catch(function(){});}} style={{height:30,padding:"0 10px",borderRadius:8,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:10.5}}>Restablecer</button>}
                    </div>}
                    {encabezadoEditing&&<div>
                      <textarea value={encabezadoDraft} onChange={function(e){setEncabezadoDraft(e.target.value);}} rows={6} placeholder="Escribí el encabezado que querés usar para este documento…" style={{width:"100%",boxSizing:"border-box",resize:"vertical",padding:"9px 10px",borderRadius:8,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",fontSize:11.5,lineHeight:1.5}} />
                      <div style={{fontSize:9.8,color:"var(--dim)",marginTop:5,lineHeight:1.45}}>Variables útiles: {"{{comprador_nombre}} · {{vendedor_nombre}} · {{inmueble_direccion}} · {{precio_letras}} · {{fecha_operacion_larga}} · {{inmobiliaria_nombre}} · {{inmobiliaria_matricula}}"}</div>
                      <div style={{display:"flex",justifyContent:"flex-end",gap:6,marginTop:8}}>
                        <button type="button" onClick={function(){setEncabezadoDraft(customTextoPreview);setEncabezadoEditing(false);}} style={{height:30,padding:"0 10px",borderRadius:8,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:10.5}}>Cancelar</button>
                        <button type="button" onClick={function(){var mapa=Object.assign({},customMapPreview,{[tipoDocSel]:encabezadoDraft});var next=Object.assign({},perfil,{encabezados_custom:mapa});Promise.resolve(onChange(next)).then(function(){setEncabezadoEditing(false);}).catch(function(){});}} style={{height:30,padding:"0 11px",borderRadius:8,border:"1px solid rgba(74,222,128,.25)",background:"rgba(74,222,128,.08)",color:"var(--green)",cursor:"pointer",fontSize:10.5,fontWeight:650}}>Guardar encabezado</button>
                      </div>
                    </div>}
                  ...
                  {/* VISTA PREVIA — se abre bajo demanda para que Configuración nunca dependa de su render inicial */}
                  <div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}>
                    <button type="button" onClick={function(){setMostrarPreviewConfiguracion(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}>
                      <span style={{fontSize:11.8,fontWeight:650}}>Vista previa del documento</span>
                      <span style={{fontSize:10.5,color:"var(--dim)"}}>{mostrarPreviewConfiguracion?"▲ Ocultar":"▼ Mostrar"}</span>
                    </button>
                    {mostrarPreviewConfiguracion&&<div style={{marginTop:9}}>
                      <div style={{fontSize:10.4,color:"var(--dim)",marginBottom:8}}>Así se verá el documento con datos de ejemplo, sin modificar ninguna operación real.</div>
                      <PlantillaPreview tipo={tipoDocSel} encabezado={encabezadoEditing?encabezadoDraft:customTextoPreview} clausulas={listaPlantilla}/>
                    </div>}
                  </div>

                  {/* BLOQUES — se mantienen fuera del render inicial de Configuración */}
                  {tipoDocSel==="reserva"&&<div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}>
                    <button type="button" onClick={function(){setMostrarBloquesConfiguracion(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}>
                      <span style={{fontSize:11.8,fontWeight:650}}>Cuerpo editable de Reserva</span>
                      <span style={{fontSize:10.5,color:"var(--dim)"}}>{mostrarBloquesConfiguracion?"▲ Ocultar":"▼ Mostrar"}</span>
                    </button>
                    {mostrarBloquesConfiguracion&&<div style={{marginTop:9}}>
                      <div style={{fontSize:10.4,color:"var(--dim)",lineHeight:1.45,marginBottom:9}}>Acá se modifica el texto de los bloques del documento. Los datos del formulario de Operaciones siguen conectados automáticamente.</div>
                      ...
                      {(!bloquesLoading||bloquesReserva.length>0)&&<ReservaBloquesPanel bloques={Array.isArray(bloquesReserva)?bloquesReserva:[]} onUpdate={onUpdateBloqueReserva} onMove={onMoveBloqueReserva} clausulasLib={Array.isArray(clausulas)?clausulas:[]} operaciones={Array.isArray(operaciones)?operaciones:[]} puedeEditar={puedeEditarPlantillas}/>} 
                    </div>}
                  </div>}
                </div>}
              </div>
...
const DEFAULT_PERFIL={nombre:"",nombre_usuario:"",matricula:"",telefono:"",email:"",logoDataUrl:"",logoPosicion:"derecha",cuit:"",direccion:"",web:"",descripcion:"",rol:"dueno",pie_pagina_texto:"",pie_pagina_logo_debajo:false,clausulas_default_ids:[],plantillas:{},colorPrimario:"#142a4d",colorSecundario:"#c9a227",plantillaEstilo:"corporativo",encabezados_custom:{}};
...
function useDocWorksSession(){
  ...
  function limpiarAuthUrl(){
    try{ window.history.replaceState({},document.title,window.location.pathname+window.location.search.replace(/([?&])type=(?:invite|recovery)(&?)/i,function(_,a,b){return b?a:"";})); }catch(e){}
  }
  ...
  return(
    <>
      ...
      <Modal open={!!viewDoc} onClose={function(){setViewDoc(null);setDocAutoAction(null);}} title={"DocWorks v"+DOCWORKS_VERSION} wide={true}
        ...
        {viewDoc&&<DocumentViewer op={viewDoc} clausulas={clausulas} perfil={perfil} operaciones={operaciones} bloquesReserva={bloquesReserva} onToggleTarea={toggleTarea} tab={docTab} onChangeTab={setDocTab} autoAction={docAutoAction} onAutoActionDone={function(){setDocAutoAction(null);}}/>}
      </Modal>
      <ConfirmModal
        open={!!draftPrompt}
