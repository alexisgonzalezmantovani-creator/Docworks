...
async function generarPDF(doc, logoDataUrl, perfil) {
  try {
    ...
    function addFooter(){
      ...
      var tieneLeyenda = !!(perfil && perfil.pie_pagina_texto && !tieneLogoPie);
      ...
      if(tieneLogoPie){
        var miniScale=(perfil&&perfil.pie_pagina_logo_scale)||100; var miniMaxH=Math.max(6,Math.min(30,12*(miniScale/100))), logoAr2=logoInfo.width/logoInfo.height, miniW=miniMaxH*logoAr2;
        var miniY=Math.max(2,nextY-2);
        var miniX=(W-miniW)/2;
        ...
        if(tieneLeyenda){
          nextY-=11;
          text(((W-MR-ML)/2+ML)-(latin1Safe(perfil.pie_pagina_texto).length*1.75),nextY,latin1Safe(perfil.pie_pagina_texto),7,false);
        }
      }
    }
...
async function generarDOCX(doc, perfil) {
  try {
    ...
    // Footer: si se usa logo de pie, queda como único identificador visual del pie (centrado).
    if (perfil && (perfil.nombre || perfil.pie_pagina_texto || (perfil.pie_pagina_logo_debajo && logoInfo))) {
      ...
      if (perfil.pie_pagina_logo_debajo && logoInfo) {
        ...
        body += '<w:p><w:pPr><w:jc w:val="center"/><w:spacing w:before="40" w:after="20"/></w:pPr><w:r><w:drawing>'
          + '<wp:inline distT="0" distB="0" distL="0" distR="0">'
        ...
        if (perfil.nombre) body += px((perfil.nombre||"")+(perfil.matricula?" — Mat. "+perfil.matricula:""), {size:8, color:"888888", center:true, italic:true, spacing:{before:0,after:20}});
        body += px("Pagina 1", {size:8, color:"aaaaaa", center:true, spacing:{before:0,after:0}});
        if (perfil.pie_pagina_texto) body += px(perfil.pie_pagina_texto, {size:8, color:"999999", center:true, italic:true, spacing:{before:60,after:0}});
      }
    }
    ...
    return <div className="cuenta-doc-preview-wrap">
      ...
      <div className="cuenta-doc-page" style={{fontFamily:serif}}>
        ...
        <div className="cuenta-doc-footer">
          ...
          {localPerfil.pie_pagina_logo_debajo&&localPerfil.logoDataUrl ? <><div style={{textAlign:"center",marginTop:5}}><img src={localPerfil.logoDataUrl} style={{display:"inline-block",height:Math.max(12,Math.min(58,24*(localPerfil.pie_pagina_logo_scale||100)/100)),width:"auto",objectFit:"contain"}} alt="Logo pie"/></div><div style={{textAlign:"center",marginTop:2}}>Página 1</div></> : <><div className="cuenta-doc-footer-row"><span>{localPerfil.nombre||"Tu inmobiliaria"}</span><span>Página 1</span></div>{localPerfil.pie_pagina_texto&&<div className="cuenta-doc-footer-note">{localPerfil.pie_pagina_texto}</div>}</>}
        </div>
      </div>
    </div>;
  }
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div className="plantilla-appearance-grid" style={{display:"grid",gridTemplateColumns:"minmax(250px,.9fr) minmax(320px,1.1fr)",gap:12,alignItems:"start",marginBottom:10}}>
                      <div style={{border:"1px solid var(--border)",borderRadius:10,padding:12,background:"rgba(255,255,255,.012)"}}>
                        ...
                        <div className="preview-control-section"><div className="preview-control-title">📄 Pie de página</div><Txa label="Leyenda" value={localPerfil.pie_pagina_texto||""} onChange={fP("pie_pagina_texto")} placeholder="Ej: Documento emitido por..." rows={2}/><label style={{display:"flex",alignItems:"center",gap:7,marginTop:8,padding:"8px 9px",borderRadius:8,border:"1px solid var(--border)",background:localPerfil.pie_pagina_logo_debajo?"rgba(212,168,83,.08)":"transparent",cursor:localPerfil.logoDataUrl?"pointer":"not-allowed",opacity:localPerfil.logoDataUrl?1:.5,fontSize:10.8,color:"var(--text)"}}><input type="checkbox" disabled={!localPerfil.logoDataUrl} checked={!!localPerfil.pie_pagina_logo_debajo} onChange={function(e){setLocalPerfil(function(p){return Object.assign({},p,{pie_pagina_logo_debajo:e.target.checked});});}}/> Repetir logo debajo del pie</label>{localPerfil.logoDataUrl&&localPerfil.pie_pagina_logo_debajo&&<><div style={{fontSize:10.5,fontWeight:600,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".08em",marginTop:10,marginBottom:6,display:"flex",justifyContent:"space-between"}}><span>Tamaño logo del pie</span><span style={{color:"var(--gold)"}}>{localPerfil.pie_pagina_logo_scale||100}%</span></div><input type="range" min="50" max="200" step="5" value={localPerfil.pie_pagina_logo_scale||100} onChange={function(e){setLocalPerfil(function(p){return Object.assign({},p,{pie_pagina_logo_scale:parseInt(e.target.value,10)});});}} style={{width:"100%",accentColor:"var(--gold)"}}/><div style={{display:"flex",justifyContent:"space-between",fontSize:9.5,color:"var(--dim)"}}><span>Chico</span><span>Normal</span><span>Grande</span></div></>}</div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {entries.map(function(en){
              ...
              return <div key={en.id} className="card" style={{padding:"10px 12px",borderColor:abierta?"rgba(212,168,83,.30)":"var(--border)"}}>
                ...
                {abierta&&cambios.length>0&&<div style={{marginTop:10,paddingTop:10,borderTop:"1px solid var(--border)"}}>
                  <div style={{fontSize:9.5,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:7}}>Qué cambió</div>
                  <div style={{display:"flex",flexDirection:"column",gap:5}}>
                    {cambios.map(function(c,i){return <div key={i} style={{display:"grid",gridTemplateColumns:"minmax(100px,145px) minmax(0,1fr)",gap:8,alignItems:"start",padding:"7px 9px",borderRadius:8,border:"1px solid var(--border2)",background:"rgba(255,255,255,.012)"}}>
                      <div style={{fontSize:10.8,fontWeight:700,color:"var(--text)"}}>{c.label}</div>
                      <div style={{fontSize:10.5,lineHeight:1.45,color:"var(--muted)"}}>
                        <span style={{color:"var(--red)",fontWeight:600}}>Antes:</span> {cajaValor(c.antes,!c.antes)} <span style={{color:"var(--dim)",margin:"0 5px"}}>→</span> <span style={{color:"var(--green)",fontWeight:600}}>Después:</span> {cajaValor(c.despues,!c.despues)}
                      </div>
                    </div>})}
                  </div>
                </div>}
