...
// DocWorks 3.31 FINAL — edición premium, visor unificado, acciones, navegación y estabilidad.
// DocWorks 3.31 — personalización amigable del texto de PRECIO + activación de
            ...
            <div style={{padding:"20px 22px 22px"}}>
              <div style={{display:"grid",gridTemplateColumns:"minmax(250px,300px) minmax(0,1fr)",gap:22,alignItems:"start"}}>
                ...
                <div className="cuenta-info-grid" style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:"12px 14px",alignItems:"start"}}>
                  ...
                  <div style={{gridColumn:"2 / span 2",display:"flex",justifyContent:"flex-start",alignItems:"flex-end",minHeight:38,paddingTop:18,alignSelf:"start"}}>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}>
                      <label style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,fontSize:12,color:"var(--text)",cursor:"pointer"}}>
                        ...
                        <span><strong>Mostrar introducción</strong><span style={{display:"block",fontSize:10.5,color:"var(--dim)",marginTop:2}}>Preámbulo legal al inicio (ej. “Recibimos de…” en Reserva). Aplica a todos los documentos.</span></span>
                      </label>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:5,flexWrap:"wrap"}}><div style={{fontSize:11.8,fontWeight:650,color:"var(--text)"}}>Texto de la introducción (editable por tipo de documento)</div><span className="badge" style={{background:customTextoPreview?"rgba(212,168,83,.10)":"rgba(74,222,128,.08)",color:customTextoPreview?"var(--gold)":"var(--green)"}}>{customTextoPreview?"Personalizado":"Predeterminado"}</span></div>
                      ...
                      {puedeEditarPlantillas&&!encabezadoEditing&&<div style={{display:"flex",justifyContent:"flex-end",gap:6,marginBottom:8}}><button type="button" onClick={function(){setEncabezadoDraft(getEncabezadoTemplate(tipoDocSel, localPerfil));setEncabezadoEditing(true);}} style={{height:30,padding:"0 10px",borderRadius:8,border:"1px solid rgba(212,168,83,.25)",background:"rgba(212,168,83,.07)",color:"var(--gold)",cursor:"pointer",fontSize:10.5}}><span style={{display:"inline-flex",alignItems:"center",gap:6}}><DWIcon name="edit" size={13}/>Editar introducción</span></button>{customTextoPreview&&<button type="button" onClick={function(){var mapa=Object.assign({},customMapPreview);delete mapa[tipoDocSel];var next=Object.assign({},localPerfil,{encabezados_custom:mapa});setLocalPerfil(next);persistirPerfil(next).then(function(res){if(!(res&&res.error)){setEncabezadoDraft("");setEncabezadoEditing(false);}});}} style={{height:30,padding:"0 10px",borderRadius:8,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:10.5}}>Restablecer</button>}</div>}
                      {encabezadoEditing&&(
                        <EncabezadoEditor
                          tipo={tipoDocSel}
                      ...
                      {!encabezadoEditing&&<div style={{marginTop:6,padding:"9px 10px",borderRadius:8,background:"rgba(255,255,255,.025)",border:"1px solid var(--border2)",fontSize:10.8,color:"var(--muted)",lineHeight:1.5,whiteSpace:"pre-wrap"}}>{(customTextoPreview||DEFAULT_ENCABEZADOS[tipoDocSel]||"Sin introducción predeterminada para este tipo.").slice(0,420)}{(customTextoPreview||DEFAULT_ENCABEZADOS[tipoDocSel]||"").length>420?"…":""}</div>}
                    </div>
