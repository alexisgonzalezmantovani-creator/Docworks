...
// DocWorks 3.42 — edición premium, visor unificado, acciones, navegación y estabilidad.
// DocWorks 3.42 — personalización amigable del texto de PRECIO + activación de
...
function DashboardView({operaciones,clausulas,perfil,onNew,onEdit,onDelete,onChangeEstado,onConvertir,onConvertirComodato,onConvertirRefuerzo,onConvertirBoleto,onConvertirDevolucion,onDuplicar,onViewDoc,onPerfil,searchOverride,papelera,onRestore,onDeletePermanente,onVaciarPapelera,confirmVaciarPapelera,setConfirmVaciarPapelera,papeleraJump,error,loading}){
  ...
  function renderOpRow(op,nested){
    ...
    // El menú "⋯" concentra todo lo que no es la acción principal de la fila
    // (Abrir) ni la acción secundaria de uso frecuente (Compartir, que por
    // eso tiene su propio botón visible en vez de vivir acá adentro).
    const menuItems=[
      {label:"Editar",icon:<DWIcon name="edit" size={13}/>,onClick:function(){onEdit(op);}},
    ...
    const acciones=(
      <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
        <Btn v="primary" s="sm" onClick={function(){onViewDoc(op);}}><DWIcon name="open" size={12} strokeWidth={2}/> Abrir</Btn>
        <Btn v="ghost" s="sm" onClick={function(){shareWhatsApp(op);}} disabled={compartiendo} title="Compartir documento (PDF)">
          {compartiendo?"...":<DWIcon name="share" size={13}/>}
        </Btn>
