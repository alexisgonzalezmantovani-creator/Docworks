...
async function generarPDF(doc, logoDataUrl, perfil) {
  try {
    ...
    function addFooter(){
      // Pie configurable por elemento: leyenda, logo y número de página pueden
      // activarse/desactivarse y cada uno puede ubicarse izquierda/centro/derecha.
      ...
      function drawCenteredOrAlignedText(value,pos,y,size,bold){
        if(!value)return;
        var txt=latin1Safe(value), tw=textWidthPt(txt,size,!!bold), x=alignX(pos,tw);
        text(x,y,txt,size,!!bold);
      }
      // Leyenda: siempre en una sola línea centrada por pedido de diseño premium.
      if(leyendaActiva) drawCenteredOrAlignedText(perfil.pie_pagina_texto,"centro",footerY-11,7,false);
      // Logo: posición independiente de la leyenda y del número de página.
      if(pieLogoActivo){
        var miniScale=(perfil&&perfil.pie_pagina_logo_scale)||100;
        var miniMaxH=Math.max(6,Math.min(24,12*(miniScale/100)));
      ...
      // Número de página: posición independiente.
      if(numeroActivo) drawCenteredOrAlignedText("Pagina "+pageNum,perfil.pie_pagina_numero_posicion||"derecha",footerY-11,8,false);
    }
...
function derivedTokensGenerales(op) {
  ...
  var monedaTxt = moneda === "USD" ? "DÓLARES ESTADOUNIDENSES BILLETE" : "PESOS ARGENTINOS BILLETE";
          ...
          <div key={String(b.id||idx)} style={{padding:"10px 12px",borderRadius:10,border:"1px solid var(--border2)",marginBottom:6,background:"var(--card)"}}>
            ...
            {editando&&(b.id==="reserva.precio"&&precioEditando ? (
              ...
              <div style={{marginTop:8,paddingTop:8,borderTop:"1px dashed var(--border2)"}}>
                ...
                <TokensProtegidosDocWorks tokens={tokensDeTextoDocWorks((b.contenido||""))} />
...
var BODY_TEMPLATE_PRESETS = {
  ...
  boleto: [
    ...
    {id:"boleto.precio",titulo:"PRECIO",contenido:"El precio total y definitivo de la presente compraventa se fija en {{moneda_txt}} {{precio_letras}}, que las partes declaran justo y conveniente a sus intereses.-"},
  ...
  exclusividad: [
    ...
    {id:"exclusividad.precio",titulo:"PRECIO",contenido:"Precio de venta autorizado: {{moneda_txt}} {{precio_letras}}. El PROPIETARIO acepta recibir ofertas a partir de dicho valor.-"},
  ...
  refuerzo_reserva: [
    ...
    {id:"refuerzo.objeto",titulo:"OBJETO DEL REFUERZO",contenido:"En este acto el COMPRADOR entrega, en calidad de REFUERZO de la Reserva oportunamente suscripta, la suma de {{moneda_txt}} {{precio_letras}}, en concepto de ampliación del monto originalmente entregado.-"},
  ...
  devolucion_reserva: [
    ...
    {id:"devolucion.devolucion",titulo:"DEVOLUCIÓN",contenido:"El VENDEDOR / PROPIETARIO devuelve en este acto al COMPRADOR / OFERENTE la suma de {{moneda_txt}} {{precio_letras}}, recibiendo el COMPRADOR plena conformidad respecto de la devolución.-"},
...
async function generarDOCX(doc, perfil) {
  try {
    ...
    if (perfil && (perfil.nombre || perfil.matricula || perfil.web || (perfil.logo_encabezado_activo !== false && logoInfo))) {
    ...
    // Footer configurable: leyenda centrada, logo y número con posiciones independientes.
    if (perfil && (perfil.nombre || perfil.pie_pagina_texto || (perfil.pie_pagina_logo_debajo && logoInfo) || perfil.pie_pagina_numero_activo!==false)) {
      body += px("", {border:true, spacing:{before:40,after:30}});
      if (perfil.pie_pagina_logo_debajo && logoInfo) {
        var miniScalePct = (perfil&&perfil.pie_pagina_logo_scale)||100; var miniMaxHpx = Math.max(28,Math.min(114,57*(miniScalePct/100))), miniFitScale = Math.min(1, miniMaxHpx/logoInfo.height);
        ...
        body += '<w:p><w:pPr><w:jc w:val="'+miniJc+'"/><w:spacing w:before="40" w:after="20"/></w:pPr><w:r><w:drawing>'
          + '<wp:inline distT="0" distB="0" distL="0" distR="0"><wp:extent cx="'+miniCx+'" cy="'+miniCy+'"/><wp:docPr id="2" name="LogoPie"/>'
          + '<a:graphic xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"><a:graphicData uri="http://schemas.openxmlformats.org/drawingml/2006/picture">'
          + '<pic:pic xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><pic:nvPicPr><pic:cNvPr id="2" name="LogoPie"/><pic:cNvPicPr/></pic:nvPicPr>'
          + '<pic:blipFill><a:blip r:embed="rIdLogo1"/><a:stretch><a:fillRect/></a:stretch></pic:blipFill><pic:spPr><a:xfrm><a:off x="0" y="0"/><a:ext cx="'+miniCx+'" cy="'+miniCy+'"/></a:xfrm><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></pic:spPr>'
          + '</pic:pic></a:graphicData></a:graphic></wp:inline></w:drawing></w:r></w:p>';
      }
      if (perfil.pie_pagina_leyenda_activa!==false && perfil.pie_pagina_texto) body += px(perfil.pie_pagina_texto, {size:8, color:"999999", center:true, italic:true, spacing:{before:40,after:10}});
      if (perfil.pie_pagina_numero_activo!==false) {
        ...
        body += '<w:p><w:pPr><w:jc w:val="'+pageJc+'"/></w:pPr>'+px("Pagina 1", {size:8, color:"aaaaaa"})+'</w:p>';
      }
    }
...
function NegociacionPanel({op,onUpdate}){
  ...
  return <div className="card" style={{padding:16,marginBottom:12,border:"1px solid rgba(96,165,250,.20)",background:"rgba(96,165,250,.035)"}}>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginBottom:10,flexWrap:"wrap"}}>
      <div><div style={{fontSize:11,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".08em"}}>Negociación</div><div style={{fontSize:12,color:"var(--muted)",marginTop:3}}>Registrá cada propuesta sin crear otra operación.</div></div>
      {ultimo&&<span style={{fontSize:11,fontWeight:700,color:"var(--gold)"}}>Último valor: {ultimo.moneda||"USD"} {ultimo.monto||"—"}</span>}
    </div>
    {historial.length>0&&<div style={{display:"flex",flexDirection:"column",gap:7,marginBottom:12}}>{historial.map(function(h,i){return <div key={h.id||i} style={{display:"grid",gridTemplateColumns:"92px 1fr auto",gap:8,alignItems:"center",padding:"7px 8px",border:"1px solid var(--border)",borderRadius:8,background:"rgba(255,255,255,.018)",fontSize:11.5}}><span style={{color:"var(--dim)"}}>{fmtD(h.fecha)}</span><span style={{color:"var(--text)"}}><b>{h.parte==="vendedor"?"Vendedor":"Comprador"}</b>{h.observacion?" · "+h.observacion:""}</span><span style={{color:"var(--gold)",fontWeight:700,whiteSpace:"nowrap"}}>{h.moneda||"USD"} {h.monto||"—"}{h.vencimiento?" · vence "+fmtD(h.vencimiento):""}</span></div>;})}</div>}
...
function WorkflowPanel({op,onUpdate,onAction}){
  if(!op||op.tipo!=="reserva") return null;
  var estado=op.proceso_estado||"pendiente_aceptacion"; var venc=op.reserva_aceptacion_vencimiento;
  var dias=venc?Math.ceil((new Date(venc+"T23:59:59").getTime()-Date.now())/86400000):null;
  function setEstado(st){onUpdate(Object.assign({},op,{proceso_estado:st}));}
  return <div className="card" style={{padding:16,marginBottom:12,border:"1px solid rgba(212,168,83,.22)",background:"linear-gradient(135deg,rgba(212,168,83,.045),transparent)"}}>
    <div style={{fontSize:11,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:10}}>Flujo de la operación</div>
    {estado==="rechazada" ? (
      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:12}}>
        <span style={{border:"1px solid rgba(248,113,113,.35)",background:"rgba(248,113,113,.08)",color:"var(--red)",borderRadius:20,padding:"6px 10px",fontSize:10.5,fontWeight:700}}>Reserva rechazada</span>
        <span style={{color:"var(--dim)",fontSize:11}}>El siguiente paso recomendado es la devolución de la reserva.</span>
      </div>
    ) : (
      <div style={{display:"flex",alignItems:"center",gap:4,flexWrap:"wrap",marginBottom:12}}>{[["pendiente_aceptacion","Reserva"],["en_negociacion","Negociación"],["aceptada","Aceptada"],["documentacion","Documentación"],["cerrada","Cerrada"]].map(function(st,i){var active=st[0]===estado;return <div key={st[0]} style={{display:"flex",alignItems:"center",gap:4}}><span style={{border:"1px solid "+(active?"var(--gold)":"var(--border2)"),background:active?"rgba(212,168,83,.10)":"transparent",color:active?"var(--gold)":"var(--dim)",borderRadius:20,padding:"6px 9px",fontSize:10.5}}>{st[1]}</span>{i<4&&<span style={{color:"var(--dim)"}}>→</span>}</div>;})}</div>
    )}
    {venc&&<div style={{padding:"10px 12px",borderRadius:10,background:dias!==null&&dias<0?"rgba(239,68,68,.08)":"rgba(212,168,83,.08)",border:"1px solid "+(dias!==null&&dias<0?"rgba(239,68,68,.25)":"rgba(212,168,83,.20)"),fontSize:12,color:dias!==null&&dias<0?"var(--red)":"var(--text)"}}><b>Vencimiento de aceptación:</b> {fmtD(venc)}{dias!==null&&<span style={{marginLeft:7,color:"var(--dim)"}}>{dias<0?"plazo vencido":dias===0?"vence hoy":dias===1?"vence mañana":"faltan "+dias+" días"}</span>}</div>}
    <div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:10}}>{estado!=="aceptada"&&estado!=="cerrada"&&<Btn v="success" s="sm" onClick={function(){setEstado("aceptada");}}>✓ Aceptar reserva</Btn>}{estado!=="cerrada"&&<Btn v="ghost" s="sm" onClick={function(){setEstado("en_negociacion");onAction&&onAction("negociacion");}}>↔ Ver negociación</Btn>}{estado!=="rechazada"&&estado!=="cerrada"&&<Btn v="danger" s="sm" onClick={function(){setEstado("rechazada");onAction&&onAction("devolucion");}}>↩ Rechazar y preparar devolución</Btn>}{estado==="aceptada"&&<Btn v="primary" s="sm" onClick={function(){onAction&&onAction("continuar");}}>Continuar con documentación →</Btn>}</div>
  </div>;
}
...
function FichaOperacionPanel({op,operaciones,onClose}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
      ...
      <Btn v="ghost" s="sm" onClick={onClose}>Cerrar</Btn>
    </div>
    ...
    {tab==="negociacion"&&<div>{historial.length?historial.slice().reverse().map(function(h,i){return <div key={h.id||i} style={{display:"grid",gridTemplateColumns:"86px 1fr auto",gap:8,alignItems:"center",padding:"9px 8px",borderBottom:"1px solid var(--border)",fontSize:11.5}}><span style={{color:"var(--dim)"}}>{fmtD(h.fecha)}</span><span style={{color:"var(--text)"}}><b>{h.parte==="vendedor"?"Vendedor":"Comprador"}</b>{h.observacion?" · "+h.observacion:""}{h.vencimiento&&<div style={{fontSize:10,color:"var(--dim)",marginTop:2}}>Vence {fmtD(h.vencimiento)}</div>}</span><span style={{color:"var(--gold)",fontWeight:800,whiteSpace:"nowrap"}}>{h.moneda||"USD"} {h.monto||"—"}</span></div>;}):<div className="card" style={{padding:20,textAlign:"center",color:"var(--dim)",fontSize:12}}>Todavía no hay movimientos registrados.</div>}</div>}
...
function DocSummaryPanel({op, perfil, tabsDocs, onLeer, operaciones, onToggleTarea, onUpdateOperation, onActionWorkflow}){
  var [showFicha,setShowFicha]=useState(false);
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",marginBottom:12}}><Btn v="secondary" s="sm" onClick={function(){setShowFicha(true);}}>👤 Ver ficha completa</Btn></div>
      <div className="card" style={{padding:16,marginBottom:12}}>
      ...
      {op.tipo==="reserva"&&<><WorkflowPanel op={op} onUpdate={onUpdateOperation} onAction={onActionWorkflow}/><NegociacionPanel op={op} onUpdate={onUpdateOperation}/></>}
      ...
      <Modal open={showFicha} onClose={function(){setShowFicha(false);}} title="Ficha completa de clientes y operación" wide={true}>
        {showFicha&&<FichaOperacionPanel op={op} operaciones={operaciones||[]} onClose={function(){setShowFicha(false);}}/>}
      </Modal>
    </div>
  );
}
  ...
  return(
    <div>
      {tabActual.esTareas ? (
        ...
        <><WorkflowPanel op={op} onUpdate={onUpdateOperation} onAction={onActionWorkflow}/><NegociacionPanel op={op} onUpdate={onUpdateOperation}/></>
      ) : tabActual.esResumen ? (
...
function parseClauseJson(text){
  ...
  }catch(e){ console.warn("DocWorks: respuesta de IA no válida",e); }
  return null;
}
...
async function narrarClausulaIA(ideaBreve){
  ...
  var res=await sb.functions.invoke("ia-docworks-gemini",{
...
function CalendarioView({operaciones,onViewOp}){
  ...
  var eventosFiltrados=eventos.filter(function(e){return filtro==="todos"||e.tipo===filtro;}).sort(function(a,b){return new Date(a.fecha)-new Date(b.fecha);});
...
function ConfiguracionView({ perfil, onChange, currentUserId, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  function CuentaDocumentoPreview(){
    ...
    var meta=[];
    if(showMat&&localPerfil.matricula)meta.push("Matrícula: "+localPerfil.matricula);
    if(showWeb&&localPerfil.web)meta.push(localPerfil.web);
    return <div className="cuenta-doc-preview-wrap">
      ...
      <div className="cuenta-doc-page" style={{fontFamily:serif}}>
        ...
        <div className="cuenta-doc-header" style={{background:headerBg,borderBottom:estilo==="clasico"?"2px solid #1a1a1a":"none"}}>
          {showName&&<div style={{fontSize:14,fontWeight:700,color:headerColor,letterSpacing:".02em",textAlign:estilo==="clasico"?"center":"left"}}>{(localPerfil.nombre||"TU INMOBILIARIA").toUpperCase()}{showMat&&localPerfil.matricula?" — Matrícula: "+localPerfil.matricula:""}</div>}
          ...
          {showWeb&&localPerfil.web&&<div style={{fontSize:9.5,color:estilo==="corporativo"?"#ffffffaa":"#64748b",marginTop:3,textAlign:estilo==="clasico"?"center":"left"}}>{localPerfil.web}</div>}
        </div>
        ...
        <div className="cuenta-doc-footer">
          ...
          <div style={{position:"relative",minHeight:42,marginTop:5}}>
            {localPerfil.pie_pagina_logo_debajo&&localPerfil.logoDataUrl&&<div style={{position:"absolute",top:0,left:localPerfil.pie_pagina_logo_posicion==="izquierda"?0:localPerfil.pie_pagina_logo_posicion==="derecha"?"auto":"50%",right:localPerfil.pie_pagina_logo_posicion==="derecha"?0:"auto",transform:localPerfil.pie_pagina_logo_posicion==="centro"?"translateX(-50%)":"none"}}><img src={localPerfil.logoDataUrl} style={{height:Math.max(12,Math.min(32,14*(localPerfil.pie_pagina_logo_scale||100)/100)),width:"auto",objectFit:"contain"}} alt="Logo pie"/></div>}
            {localPerfil.pie_pagina_leyenda_activa!==false&&localPerfil.pie_pagina_texto&&<div style={{position:"absolute",top:localPerfil.pie_pagina_logo_debajo&&localPerfil.logoDataUrl?22:4,left:"50%",transform:"translateX(-50%)",width:"80%",textAlign:"center",fontSize:9,color:"var(--dim)",fontStyle:"italic",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{localPerfil.pie_pagina_texto}</div>}
            {localPerfil.pie_pagina_numero_activo!==false&&<div style={{position:"absolute",top:4,left:localPerfil.pie_pagina_numero_posicion==="izquierda"?0:localPerfil.pie_pagina_numero_posicion==="derecha"?"auto":"50%",right:localPerfil.pie_pagina_numero_posicion==="derecha"?0:"auto",transform:localPerfil.pie_pagina_numero_posicion==="centro"?"translateX(-50%)":"none",fontSize:9,color:"var(--dim)"}}>Página 1</div>}
          </div>
        </div>
      </div>
    </div>;
    ...
    <div>
      ...
      {tab==="cuenta"&&(
        <div style={{maxWidth:980,margin:"0 auto"}}>
          <div style={{
            background:"linear-gradient(180deg,rgba(212,168,83,0.10),rgba(212,168,83,0.025))",
            border:"1px solid rgba(212,168,83,0.20)",borderRadius:18,padding:"28px 24px 22px",textAlign:"center",
            boxShadow:"0 12px 35px rgba(0,0,0,.08)",marginBottom:14
          }}>
            <div style={{width:112,height:112,borderRadius:20,background:"rgba(255,255,255,.78)",border:"1px solid rgba(212,168,83,.30)",margin:"0 auto 14px",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",boxShadow:"0 10px 28px rgba(0,0,0,.12)"}}>
              {localPerfil.logoDataUrl?<img src={localPerfil.logoDataUrl} style={{width:"88%",height:"88%",objectFit:"contain",borderRadius:12}} alt="Logo de la inmobiliaria"/>:<div style={{fontSize:40,color:"var(--gold)",fontWeight:700}}>{localPerfil.nombre?localPerfil.nombre[0].toUpperCase():"D"}</div>}
            </div>
            <div style={{fontFamily:"DM Serif Display,serif",fontSize:22,color:"var(--text)",marginBottom:4}}>{localPerfil.nombre||"Tu inmobiliaria"}</div>
            {localPerfil.matricula&&<div style={{fontSize:12,color:"var(--gold)",fontWeight:600}}>Mat. {localPerfil.matricula}</div>}
            <div style={{display:"flex",justifyContent:"center",gap:0,margin:"20px auto 0",maxWidth:540,paddingTop:16,borderTop:"1px solid var(--border)"}}>
              <div style={{flex:1,textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:24,color:"var(--text)"}}>{opsTotal}</div><div style={{fontSize:10,color:"var(--dim)",marginTop:2}}>Operaciones activas</div></div>
              <div style={{flex:1,textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:24,color:"var(--green)"}}>{opsActivas}</div><div style={{fontSize:10,color:"var(--dim)",marginTop:2}}>En operación</div></div>
              <div style={{flex:1,textAlign:"center"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:24,color:"var(--teal)"}}>{equipo.length}</div><div style={{fontSize:10,color:"var(--dim)",marginTop:2}}>Personas en equipo</div></div>
            </div>
          </div>

          <div className="card" style={{padding:"22px 22px 20px"}}>
            <div style={{textAlign:"center",marginBottom:18}}>
              <div style={{fontSize:16,fontFamily:"DM Serif Display,serif",color:"var(--text)",marginBottom:4}}>Información de la inmobiliaria</div>
              <div style={{fontSize:10.8,color:"var(--dim)",lineHeight:1.5}}>Datos de la cuenta y de la inmobiliaria que se utilizan para identificarte y completar tus documentos.</div>
            </div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"12px 14px",alignItems:"start"}}>
        ...
        return <div>
          <div className="card" style={{padding:0,marginBottom:12,overflow:"hidden"}}>
            ...
            <div style={{display:"flex",gap:4,padding:"0 8px",borderTop:"1px solid var(--border)",background:"rgba(148,163,184,.025)"}}>
              <button type="button" onClick={function(){setPlantillaVista("modelos");setPlantillaSubtab("clausulas");}} style={{border:0,borderBottom:plantillaVista==="modelos"?"2px solid var(--gold)":"2px solid transparent",background:"transparent",color:plantillaVista==="modelos"?"var(--text)":"var(--muted)",padding:"12px 15px",cursor:"pointer",fontSize:11.5,fontWeight:plantillaVista==="modelos"?700:500}}>▦ Modelos de documentos</button>
              <button type="button" onClick={function(){setPlantillaVista("aspecto");}} style={{border:0,borderBottom:plantillaVista==="aspecto"?"2px solid var(--gold)":"2px solid transparent",background:"transparent",color:plantillaVista==="aspecto"?"var(--text)":"var(--muted)",padding:"12px 15px",cursor:"pointer",fontSize:11.5,fontWeight:plantillaVista==="aspecto"?700:500}}>✦ Aspecto del documento</button>
            </div>
          </div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div className="plantilla-appearance-grid" style={{display:"grid",gridTemplateColumns:"minmax(250px,.9fr) minmax(320px,1.1fr)",gap:12,alignItems:"start",marginBottom:10}}>
                      <div style={{border:"1px solid var(--border)",borderRadius:10,padding:12,background:"rgba(255,255,255,.012)"}}>
                        ...
                        <div className="preview-control-section"><div className="preview-control-title">📌 Encabezado</div><div style={{display:"flex",flexDirection:"column",gap:6}}>{[{k:"encabezado_nombre",l:"Nombre de la inmobiliaria"},{k:"encabezado_matricula",l:"Matrícula / CMCPSI"},{k:"encabezado_web",l:"Sitio web"}].map(function(f){var checked=localPerfil[f.k]!==false;return <label key={f.k} style={{display:"flex",alignItems:"center",gap:7,fontSize:11,color:"var(--text)",cursor:"pointer"}}><input type="checkbox" checked={checked} onChange={function(e){var o={};o[f.k]=e.target.checked;setLocalPerfil(function(p){return Object.assign({},p,o);});}}/>{f.l}</label>;})}</div></div>
...
function usePerfilSupabase(ctx, perfil, setPerfil){
  ...
  function leer(sb){
    ...
    }).catch(function(){ perfilCargado.current=true; setPerfilNubeCargado(true); });
  }
  ...
  useEffect(function(){
    ...
    loadSupabaseJs().then(function(sb){ if(sb) leer(sb).catch(function(err){ dwNotify("error","No se pudo sincronizar la configuración con la nube."); }); });
  },[inmobId]);
  useEffect(function(){
    if(!inmobId || !perfilCargado.current) return;
    var t=setTimeout(function(){ guardarAhora(perfil).catch(function(err){ dwNotify("error","No se pudieron guardar los cambios de configuración en la nube."); }); },500);
    return function(){clearTimeout(t);};
  },[inmobId,perfil]);
...
// ── Onboarding inicial ──────────────────────────────────────────────────────
// Tour corto, integrado y descartable. Se guarda por usuario en localStorage
// para que cada integrante vea la introducción una sola vez en su dispositivo.
function DocWorksOnboarding({ open, step, onStep, onClose, onNavigate, role }) {
  if (!open) return null;
  var steps = [
    { key:"inicio", title:"Bienvenido a DocWorks", text:"En pocos minutos vas a conocer el flujo principal para gestionar operaciones, preparar documentos y tener todo ordenado desde un solo lugar.", icon:"✦" },
    { key:"operaciones", title:"1. Empezá por Operaciones", text:"Acá nacen y se siguen tus operaciones. Podés crear una reserva, editar sus datos, ver su estado y continuar el circuito documental sin duplicar información.", icon:"▦", view:"dashboard" },
    { key:"documentos", title:"2. Generá tus documentos", text:"Desde cada operación podés revisar el documento, editarlo y finalizarlo en los formatos disponibles. La información se reutiliza para evitar cargar todo de nuevo.", icon:"▤", view:"dashboard" },
    { key:"clausulas", title:"3. Cláusulas y plantillas", text:"Las cláusulas y los bloques de tus modelos se administran desde acá. Las opciones protegidas por el dueño o administrador se mantienen controladas para todo el equipo.", icon:"§", view:"clausulas" },
    { key:"seguimiento", title:"4. Seguimiento y calendario", text:"Usá el calendario para tener a la vista fechas importantes y volver rápidamente a las operaciones que necesitan seguimiento.", icon:"📅", view:"calendario" },
    { key:"estadisticas", title:"5. Estadísticas", text:"Consultá el movimiento de la inmobiliaria y los honorarios. Cada usuario ve la información que corresponde a su rol.", icon:"◎", view:"estadisticas" },
    { key:"configuracion", title:"6. Configuración y equipo", text:"Desde Configuración se administran los datos de la inmobiliaria, el equipo, permisos, plantillas, auditoría y respaldos.", icon:"⚙", view:"configuracion" },
    { key:"listo", title:"¡Ya estás listo!", text:"Ese es el circuito básico. Podés volver a ver este recorrido cuando quieras desde el botón de ayuda de la barra superior.", icon:"✓" }
  ];
  var current=steps[Math.max(0,Math.min(step,steps.length-1))];
  var last=step===steps.length-1;
  function avanzar(){
    if(last){ onClose(true); return; }
    var next=steps[step+1];
    if(next&&next.view&&onNavigate) onNavigate(next.view);
    onStep(step+1);
  }
  function retroceder(){
    if(step<=0) return;
    var prev=steps[step-1];
    if(prev&&prev.view&&onNavigate) onNavigate(prev.view);
    onStep(step-1);
  }
  return (
    <div style={{position:"fixed",inset:0,zIndex:5000,background:"rgba(3,8,18,0.76)",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:"20px"}}>
      <div role="dialog" aria-modal="true" aria-label="Introducción a DocWorks" style={{width:"min(620px,100%)",border:"1px solid rgba(212,168,83,0.35)",borderRadius:24,background:"linear-gradient(145deg,rgba(16,25,43,0.98),rgba(8,14,26,0.99))",boxShadow:"0 30px 90px rgba(0,0,0,0.55),0 0 40px rgba(212,168,83,0.08)",overflow:"hidden"}}>
        ...
        <div style={{padding:"26px 28px 24px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:12,marginBottom:24}}>
            <div style={{display:"flex",alignItems:"center",gap:11}}>
              <div style={{width:42,height:42,borderRadius:13,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(212,168,83,0.10)",border:"1px solid rgba(212,168,83,0.25)",color:"var(--gold)",fontSize:21}}>{current.icon}</div>
              <div>
                <div style={{fontSize:11,textTransform:"uppercase",letterSpacing:1.3,color:"var(--gold)",fontWeight:700}}>DocWorks · Guía rápida</div>
                <div style={{fontSize:12,color:"var(--muted)",marginTop:3}}>Paso {step+1} de {steps.length}</div>
              </div>
            </div>
            <button type="button" onClick={function(){onClose(false);}} aria-label="Cerrar recorrido" style={{width:34,height:34,borderRadius:10,border:"1px solid var(--border2)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:18}}>×</button>
          </div>

          <div style={{display:"flex",gap:5,marginBottom:25}}>
            {steps.map(function(_,i){ return <div key={i} style={{height:4,flex:1,borderRadius:10,background:i<=step?"var(--gold)":"rgba(255,255,255,0.09)",transition:"background .2s"}}/>; })}
          </div>

          <div style={{minHeight:185}}>
            <h2 style={{margin:"0 0 12px",fontSize:27,lineHeight:1.15,color:"var(--text)",letterSpacing:-.5}}>{current.title}</h2>
            <p style={{margin:0,fontSize:15,lineHeight:1.75,color:"var(--muted2)",maxWidth:540}}>{current.text}</p>
            {step===0&&<div style={{marginTop:20,padding:"12px 14px",borderRadius:12,background:"rgba(212,168,83,0.07)",border:"1px solid rgba(212,168,83,0.16)",fontSize:12.5,color:"var(--muted2)"}}>No necesitás memorizar nada. La idea es que puedas empezar a trabajar y volver a esta guía cuando quieras.</div>}
            {step===steps.length-1&&<div style={{marginTop:20,padding:"12px 14px",borderRadius:12,background:"rgba(91,198,125,0.07)",border:"1px solid rgba(91,198,125,0.16)",fontSize:12.5,color:"var(--muted2)"}}>Tip: el recorrido queda disponible desde el botón <strong style={{color:"var(--text)"}}>?</strong> de la barra superior.</div>}
          </div>

          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,marginTop:18}}>
            <button type="button" onClick={function(){onClose(false);}} style={{border:0,background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:12.5,padding:"10px 0"}}>Omitir recorrido</button>
            <div style={{display:"flex",gap:9}}>
              {step>0&&<button type="button" onClick={retroceder} style={{height:42,padding:"0 17px",borderRadius:11,border:"1px solid var(--border2)",background:"transparent",color:"var(--text)",fontWeight:600,cursor:"pointer"}}>Anterior</button>}
              <button type="button" onClick={avanzar} style={{height:42,padding:"0 20px",borderRadius:11,border:"1px solid rgba(212,168,83,.65)",background:"linear-gradient(135deg,var(--gold),var(--gold2))",color:"#101010",fontWeight:800,cursor:"pointer",boxShadow:"0 5px 18px rgba(212,168,83,.18)"}}>{last?"Empezar a usar DocWorks":"Siguiente"}</button>
            </div>
          </div>
        </div>
      </div>
...
export default function App(){
  ...
  function asegurarDatosInicialesInmobiliaria(){
    ...
    loadSupabaseJs().then(function(sb){
      if(!sb) return;
      sb.rpc("fn_clausulas_sembrar",{}).catch(function(err){ dwNotify("error","No se pudieron inicializar las cláusulas estándar."); });
      sb.rpc("fn_bloques_reserva_sembrar",{}).catch(function(err){ dwNotify("error","No se pudo inicializar el cuerpo base de Reserva."); });
    }).catch(function(err){ dwNotify("error","No se pudieron inicializar los datos base de la inmobiliaria."); });
  }
    ...
    <>
      ...
      <div className="app-wrap">
        ...
        <div className="topbar">
          ...
          <div className="topbar-right">
            <button type="button" onClick={abrirOnboarding} title="Ver guía rápida de DocWorks" aria-label="Ver guía rápida de DocWorks" style={{width:34,height:34,borderRadius:10,border:"1px solid var(--border2)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:16,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>?</button>
      ...
      <Modal open={!!viewDoc} onClose={function(){setViewDoc(null);setDocAutoAction(null);}} title={"DocWorks v"+DOCWORKS_VERSION} wide={true}
        headerExtra={viewDoc?<div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          <Btn v="secondary" s="sm" onClick={function(){var op=viewDoc;setViewDoc(null);setDocAutoAction(null);setEditOp(op);setFormTipo(op.tipo||"reserva");setShowForm(true);}}><span style={{display:"inline-flex",alignItems:"center",gap:6}}><DWIcon name="edit" size={14}/>Editar</span></Btn>
        </div>:null}>
