...
function useSuperAdmin(activo){
  ...
  function cargarDetalle(inmobiliariaId){
    ...
    rpc("fn_super_detalle_inmobiliaria", { p_inmobiliaria_id: inmobiliariaId }).then(function(res){
      ...
      if (res.error) { setDetalleError(res.error.message); setDetalle(null); return; }
      setDetalle(res.data);
    }).catch(function(err){ setDetalleLoading(false); setDetalleError(err && err.message ? err.message : "Error cargando el detalle."); });
...
function SAModal({open,onClose,title,children,wide}){
  if (!open) return null;
  return (
    <div style={{position:"fixed",inset:0,zIndex:500,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.45)",padding:16}} onClick={onClose}>
      <div onClick={function(e){e.stopPropagation();}} style={{width:"100%",maxWidth:wide?680:420,background:AUTH_CARD,border:"1px solid "+AUTH_BORDER,borderRadius:16,padding:22,maxHeight:"88vh",overflow:"auto"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,gap:10}}>
          <div style={{fontSize:15,fontWeight:700,color:AUTH_TEXT}}>{title}</div>
          <button onClick={onClose} style={{background:"transparent",border:"none",color:AUTH_MUTED,fontSize:18,cursor:"pointer",lineHeight:1,padding:4}}>&#x2715;</button>
        </div>
        {children}
      </div>
    </div>
...
function SABadge({children,color}){
  var c = color||AUTH_MUTED;
  return <span style={{padding:"3px 9px",borderRadius:20,fontSize:10.5,fontWeight:700,color:c,border:"1px solid "+c+"55",whiteSpace:"nowrap"}}>{children}</span>;
}
var ROL_COLOR = { dueno:AUTH_GOLD, admin:"var(--blue,#60a5fa)", broker:"var(--green,#4ade80)", vendedor:"var(--purple,#a78bfa)", viewer:AUTH_MUTED };
var ROLES_BASE = ["dueno","admin","broker","vendedor","viewer"];
function SAEmpty({children}){ return <div style={{fontSize:12.5,color:AUTH_MUTED,padding:"14px 0",textAlign:"center"}}>{children}</div>; }
function SASection({title,children,right}){
  return (
    <div style={{marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
        <div style={{fontSize:11.5,fontWeight:700,color:AUTH_MUTED,textTransform:"uppercase",letterSpacing:0.4}}>{title}</div>
        {right}
      </div>
      {children}
    </div>
...
function SuperAdminDashboard({ contexto, onLogout }){
  ...
  function abrirDetalle(id){ setDetalleId(id); setNuevoPlanDetalle(""); sa.cargarDetalle(id); }
  function cerrarDetalle(){ setDetalleId(null); setNuevoPlanDetalle(""); sa.limpiarDetalle(); }
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:400,overflow:"auto",background:AUTH_BG,color:AUTH_TEXT,padding:24,fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{maxWidth:1040,margin:"0 auto"}}>
          ...
          sa.dashboard ? (
            <div>
              ...
              {sa.dashboard.inmobiliarias_suspendidas>0 && (
                <SASection title={"Inmobiliarias que requieren atención (" + sa.inmobiliarias.filter(function(i){return i.estado==="suspendida"||i.estado==="vencida";}).length + ")"}>
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                        ...
                        <SACard key={i.inmobiliaria_id}>
                          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:8}}>
                              <SABadge color={ESTADO_MEMBRESIA_COLOR[i.estado]}>{i.estado}</SABadge>
                              <SAButton ghost onClick={function(){setTab("inmobiliarias"); abrirDetalle(i.inmobiliaria_id);}}>Revisar</SAButton>
                            </div>
                          </div>
                        </SACard>
                      );
          ...
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              <div style={{flex:"1 1 220px",minWidth:180}}>
                <SATextInput value={busqueda} onChange={function(e){setBusqueda(e.target.value);}} placeholder="Buscar por nombre..."/>
              </div>
              <div style={{width:170}}>
                <SASelect value={filtroEstado} onChange={function(e){setFiltroEstado(e.target.value);}}>
            ...
            <div style={{fontSize:11.5,color:AUTH_MUTED,marginBottom:10}}>
                ...
                return (
                  <SACard key={i.inmobiliaria_id}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <SABadge color={ESTADO_MEMBRESIA_COLOR[i.estado]}>{i.estado}</SABadge>
                        <SAButton ghost onClick={function(){abrirDetalle(i.inmobiliaria_id);}}>Ver detalle</SAButton>
                      </div>
                    </div>
                  </SACard>
                );
                  ...
                  <SACard key={p.id}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
                      <div>
                        ...
                        <div style={{fontSize:12,color:AUTH_MUTED,marginTop:2}}>
                          Hasta {p.limite_usuarios} usuarios · {p.precio_mensual?p.moneda+" "+p.precio_mensual+"/mes":"precio sin definir"} · {p.activo?"activo":"inactivo"} · {inmobsEnPlan} inmobiliaria{inmobsEnPlan===1?"":"s"} usándolo
                        </div>
                      </div>
        ...
        {tab==="auditoria" && (
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              <div style={{width:260}}>
                <SASelect value={auditFiltro} onChange={function(e){setAuditFiltro(e.target.value);}}>
              ...
              {sa.auditoriaLoading && <div style={{fontSize:11.5,color:AUTH_MUTED}}>Cargando…</div>}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {!sa.auditoriaLoading && sa.auditoria.length===0 && <SAEmpty>Sin actividad registrada.</SAEmpty>}
              {sa.auditoria.map(function(a){
                return (
                  <div key={a.id} style={{padding:"10px 14px",borderRadius:10,border:"1px solid "+AUTH_BORDER,background:AUTH_CARD,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:600}}>{a.accion} <span style={{fontWeight:400,color:AUTH_MUTED}}>· {a.entidad_tipo}</span></div>
                      <div style={{fontSize:11.5,color:AUTH_MUTED,marginTop:2}}>{a.inmobiliaria_nombre||"—"} · {a.usuario_email||"sistema"}</div>
                    </div>
                    <div style={{fontSize:11,color:AUTH_MUTED,whiteSpace:"nowrap"}}>{new Date(a.creada_en).toLocaleString("es-AR")}</div>
                  </div>
                );
              })}
            </div>
        ...
        {tab==="pagos" && (
          <div>
            <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap",alignItems:"center"}}>
              <div style={{width:260}}>
                <SASelect value={pagosFiltro} onChange={function(e){setPagosFiltro(e.target.value);}}>
                  <option value="">Todas las inmobiliarias</option>
                  {sa.inmobiliarias.map(function(i){return <option key={i.inmobiliaria_id} value={i.inmobiliaria_id}>{i.inmobiliaria_nombre}</option>;})}
                </SASelect>
              </div>
              {sa.pagosLoading && <div style={{fontSize:11.5,color:AUTH_MUTED}}>Cargando…</div>}
              <div style={{marginLeft:"auto",display:"flex",gap:8}}>
                {Object.keys(pagosTotales).map(function(k){
                  return <SABadge key={k} color={"var(--green,#4ade80)"}>{"Cobrado " + k + " " + pagosTotales[k].toLocaleString("es-AR",{minimumFractionDigits:2})}</SABadge>;
                })}
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              ...
              {sa.pagos.map(function(p){
                return (
                  <div key={p.id} style={{padding:"10px 14px",borderRadius:10,border:"1px solid "+AUTH_BORDER,background:AUTH_CARD,display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
                    <div>
                      <div style={{fontSize:13,fontWeight:600}}>{fmt$(p.monto,p.moneda)} <span style={{fontWeight:400,color:AUTH_MUTED}}>· {p.proveedor||"—"}</span></div>
                      <div style={{fontSize:11.5,color:AUTH_MUTED,marginTop:2}}>{p.inmobiliaria_nombre||"—"}{p.referencia_externa?" · ref "+p.referencia_externa:""}</div>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <SABadge color={p.estado==="aprobado"||p.estado==="pagado"?"var(--green,#4ade80)":(p.estado==="rechazado"?AUTH_RED:"var(--amber,#fbbf24)")}>{p.estado}</SABadge>
                      <div style={{fontSize:11,color:AUTH_MUTED,whiteSpace:"nowrap"}}>{new Date(p.creado_en).toLocaleDateString("es-AR")}</div>
                    </div>
                  </div>
                );
              })}
      ...
      <SAModal open={!!detalleId} onClose={cerrarDetalle} title={inmobiliariaDetalleResumen?inmobiliariaDetalleResumen.inmobiliaria_nombre:"Detalle"} wide>
        {sa.detalleLoading && <div style={{fontSize:13,color:AUTH_MUTED}}>Cargando…</div>}
        {sa.detalleError && <div style={{fontSize:12.5,color:AUTH_RED}}>{sa.detalleError}</div>}
        {det && !sa.detalleLoading && (
          <div>
            <SASection title="Plan y membresía">
              <div style={{display:"flex",flexWrap:"wrap",gap:16,fontSize:12.5,color:AUTH_MUTED,marginBottom:10}}>
                <div>Plan: <b style={{color:AUTH_TEXT}}>{det.membresia?det.membresia.plan_nombre:"—"}</b></div>
                <div>Alta: <b style={{color:AUTH_TEXT}}>{det.inmobiliaria&&det.inmobiliaria.creada_en?new Date(det.inmobiliaria.creada_en).toLocaleDateString("es-AR"):"—"}</b></div>
                <div>Vence: <b style={{color:AUTH_TEXT}}>{det.membresia&&det.membresia.fecha_vencimiento?new Date(det.membresia.fecha_vencimiento).toLocaleDateString("es-AR"):"—"}</b></div>
                <div>Cláusulas: <b style={{color:AUTH_TEXT}}>{det.clausulas_count}</b></div>
                <div>Bloques: <b style={{color:AUTH_TEXT}}>{det.bloques_count}</b></div>
              </div>
              <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
                <div style={{width:220}}>
                  <SASelect value={nuevoPlanDetalle} onChange={function(e){setNuevoPlanDetalle(e.target.value);}}>
                ...
                <SAButton disabled={!nuevoPlanDetalle} onClick={function(){sa.asignarPlan(detalleId, nuevoPlanDetalle).then(function(){setNuevoPlanDetalle("");});}}>Aplicar plan</SAButton>
              </div>
              <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:10}}>
                {ESTADOS_MEMBRESIA.map(function(e){
                  ...
                  return <button key={e} onClick={function(){sa.setEstadoMembresia(detalleId, e);}}
                    style={{padding:"6px 12px",borderRadius:20,border:"1px solid "+(ESTADO_MEMBRESIA_COLOR[e]||AUTH_MUTED)+"55",background:activo?(ESTADO_MEMBRESIA_COLOR[e]||AUTH_MUTED)+"22":"transparent",color:ESTADO_MEMBRESIA_COLOR[e]||AUTH_MUTED,fontSize:12,fontWeight:600,cursor:"pointer"}}>{e}</button>;
                })}
              </div>
            </SASection>
            ...
            <SASection title={"Usuarios (" + (det.usuarios?det.usuarios.length:0) + ")"}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(det.usuarios||[]).map(function(u){
                  return (
                    <div key={u.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,padding:"8px 10px",borderRadius:9,background:"rgba(255,255,255,0.03)"}}>
                      <div style={{minWidth:160}}>
                        <div style={{fontSize:13,fontWeight:600}}>{u.nombre||u.email}</div>
                        <div style={{fontSize:11,color:AUTH_MUTED}}>{u.email}</div>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <SABadge color={u.activo?"var(--green,#4ade80)":AUTH_RED}>{u.activo?"activo":"inactivo"}</SABadge>
                        <div style={{width:130}}>
                          <SASelect value={u.rol_base} onChange={function(e){cambiarRol(u.id, e.target.value);}}>
                            {ROLES_BASE.map(function(r){return <option key={r} value={r}>{r}</option>;})}
                          </SASelect>
                        </div>
                        <SAButton ghost disabled={rolBusy===u.id} onClick={function(){toggleActivo(u.id, u.activo);}}>{u.activo?"Desactivar":"Activar"}</SAButton>
                      </div>
                    </div>
                  );
                })}
            ...
            <SASection title={"Cláusulas (" + sa.detalleClausulas.length + ")"}>
              {sa.detalleClausulasLoading && <div style={{fontSize:12,color:AUTH_MUTED}}>Cargando…</div>}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:280,overflow:"auto"}}>
                {sa.detalleClausulas.map(function(c){
                  return (
                    <div key={c.id} style={{padding:"8px 10px",borderRadius:9,background:"rgba(255,255,255,0.03)"}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                        <div style={{fontSize:12.5,fontWeight:600}}>{c.titulo}</div>
                        <div style={{display:"flex",gap:6,alignItems:"center"}}>
                          {c.obligatoria && <SABadge color={AUTH_GOLD}>obligatoria</SABadge>}
                          <SABadge color={c.activo?"var(--green,#4ade80)":AUTH_MUTED}>{c.activo?"activa":"inactiva"}</SABadge>
                        </div>
                      </div>
                      <div style={{fontSize:11,color:AUTH_MUTED,marginTop:3}}>{c.categoria}</div>
                      <div style={{fontSize:11.5,color:AUTH_MUTED,marginTop:4,lineHeight:1.4}}>{(c.contenido||"").slice(0,160)}{(c.contenido||"").length>160?"…":""}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <SASection title={"Plantillas de documentos (" + sa.detallePlantillas.length + ")"}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {sa.detallePlantillas.map(function(p){
                  return (
                    <div key={p.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,padding:"8px 10px",borderRadius:9,background:"rgba(255,255,255,0.03)"}}>
                      <div>
                        <div style={{fontSize:12.5,fontWeight:600}}>{p.nombre} <span style={{color:AUTH_MUTED,fontWeight:400}}>· {p.tipo_documento}</span></div>
                        <div style={{fontSize:11,color:AUTH_MUTED,marginTop:2}}>v{p.version} · {p.bloques_count} bloque{p.bloques_count===1?"":"s"}</div>
                      </div>
                      <SABadge color={p.activa?"var(--green,#4ade80)":AUTH_MUTED}>{p.activa?"activa":"inactiva"}</SABadge>
                    </div>
                  );
                })}
              </div>
            ...
            <SASection title="Últimos pagos">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(det.ultimos_pagos||[]).map(function(p){
                  return (
                    <div key={p.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:12.5,padding:"6px 10px",borderRadius:8,background:"rgba(255,255,255,0.03)"}}>
                      <div>{fmt$(p.monto,p.moneda)} <span style={{color:AUTH_MUTED}}>· {p.proveedor||"—"}</span></div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <SABadge color={p.estado==="aprobado"||p.estado==="pagado"?"var(--green,#4ade80)":(p.estado==="rechazado"?AUTH_RED:"var(--amber,#fbbf24)")}>{p.estado}</SABadge>
                        <span style={{color:AUTH_MUTED,fontSize:11}}>{new Date(p.creado_en).toLocaleDateString("es-AR")}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <SASection title="Actividad reciente">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(det.ultima_auditoria||[]).map(function(a,idx){
                  return (
                    <div key={idx} style={{fontSize:12,padding:"6px 10px",borderRadius:8,background:"rgba(255,255,255,0.03)",display:"flex",justifyContent:"space-between",gap:8}}>
                      <span>{a.accion} <span style={{color:AUTH_MUTED}}>· {a.entidad_tipo} · {a.usuario_email||"sistema"}</span></span>
                      <span style={{color:AUTH_MUTED,whiteSpace:"nowrap"}}>{new Date(a.creada_en).toLocaleString("es-AR")}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <SAButton ghost onClick={cerrarDetalle}>Cerrar</SAButton>
            </div>
          </div>
        )}
      </SAModal>
...
function useOperacionesSupabase(ctx){
  ...
  function recargar(){
    ...
    }).catch(function(err){ setLoading(false); setError(err && err.message ? err.message : "Error cargando las operaciones."); });
  }
  ...
  function guardar(op){
    return rpc("fn_operacion_guardar", { p_id:op.id, p_tipo:op.tipo, p_estado:op.estado||"borrador", p_parent_id:op.parent_id||null, p_datos:op }).then(function(res){
      if (res.error) { setError(res.error.message); } else { recargar(); }
      return res;
    });
  }
  function eliminar(id){ return rpc("fn_operacion_eliminar", { p_id:id }).then(function(res){ if(res.error) setError(res.error.message); else recargar(); return res; }); }
  function restaurar(id){ return rpc("fn_operacion_restaurar", { p_id:id }).then(function(res){ if(res.error) setError(res.error.message); else recargar(); return res; }); }
  function eliminarPermanente(id){ return rpc("fn_operacion_eliminar_permanente", { p_id:id }).then(function(res){ if(res.error) setError(res.error.message); else recargar(); return res; }); }
  function vaciarPapelera(){ return rpc("fn_operaciones_vaciar_papelera", {}).then(function(res){ if(res.error) setError(res.error.message); else recargar(); return res; }); }
...
function useAuditoriaSupabase(ctx){
  ...
  function recargar(){
    ...
    loadSupabaseJs().then(function(sb){
      if (!sb) { setLoading(false); setError("No se pudo conectar con Supabase."); return; }
      return sb.from("auditoria").select("id,accion,entidad_tipo,valor_anterior,valor_nuevo,creada_en,usuarios(nombre,email,rol_base)").order("creada_en",{ascending:false}).limit(300).then(function(res){
        setLoading(false);
        if (res.error) { setError(res.error.message); return; }
        setEntries((res.data||[]).map(mapEntry));
      });
    }).catch(function(err){ setLoading(false); setError(err&&err.message?err.message:"Error cargando la auditoría."); });
  }
  useEffect(function(){ recargar(); }, [inmobId]);
...
function useBloquesReservaSupabase(ctx){
  ...
  function recargar(){
    ...
    rpc("fn_bloques_reserva_listar",{}).then(function(res){
      ...
      if(!res.data || res.data.length===0){
        rpc("fn_bloques_reserva_sembrar",{}).then(function(res2){
          if(res2.error){ setLoading(false); setError(res2.error.message); return; }
          rpc("fn_bloques_reserva_listar",{}).then(function(res3){
            setLoading(false);
            if(res3.error){ setError(res3.error.message); return; }
    ...
    }).catch(function(err){ setLoading(false); setError(err&&err.message?err.message:"Error cargando los bloques."); });
  }
  useEffect(function(){ recargar(); }, [inmobId]);
  function editar(dbId, titulo, contenido){ return rpc("fn_bloque_editar",{p_bloque_id:dbId,p_titulo:titulo,p_contenido:contenido}).then(function(res){ if(res.error) setError(res.error.message); else recargar(); return res; }); }
  function mover(dbId, nuevoOrden){ return rpc("fn_bloque_mover",{p_bloque_id:dbId,p_nuevo_orden:nuevoOrden}).then(function(res){ if(res.error) setError(res.error.message); return res; }); }
...
export default function App(){
  ...
  function recargarClausulas(){
    ...
    loadSupabaseJs().then(function(sb){
      ...
      return sb.from("clausulas").select("*").order("orden").then(function(res){
        if (res.error) { setClausulasLoading(false); setClausulasError(res.error.message); return; }
        var activas = (res.data||[]).filter(function(c){return c.activo!==false;});
        if (activas.length===0) {
          sb.rpc("fn_clausulas_sembrar",{}).then(function(){
            return sb.from("clausulas").select("*").order("orden").then(function(res2){
              ...
              if (res2.error) { setClausulasError(res2.error.message); return; }
              setClausulas((res2.data||[]).filter(function(c){return c.activo!==false;}));
            });
          });
        } else {
          setClausulasLoading(false);
    ...
    }).catch(function(err){ setClausulasLoading(false); setClausulasError(err&&err.message?err.message:"Error cargando cláusulas."); });
  }
  ...
  function addClausula(c){
    clausulaRpc("fn_clausula_crear", { p_titulo:c.titulo, p_contenido:c.contenido, p_categoria:c.categoria||"general", p_tipos:c.tipos||["todos"], p_obligatoria:!!c.obligatoria, p_proteccion:"B" }).then(function(res){
      if (res.error) { setClausulasError(res.error.message); return; }
      recargarClausulas();
    });
  }
  ...
  function moveClausula(id, dir, visibleIds){
    setClausulas(function(cs){
      ...
      clausulaRpc("fn_clausula_mover",{p_clausula_id:id,p_nuevo_orden:ordenB}).then(function(res){ if(res.error) setClausulasError(res.error.message); });
      clausulaRpc("fn_clausula_mover",{p_clausula_id:otherId,p_nuevo_orden:ordenA}).then(function(res){ if(res.error) setClausulasError(res.error.message); });
      var next = cs.slice();
  ...
  function saveOp(data,accion){
    ...
    opsSupabase.guardar(opGuardada);
