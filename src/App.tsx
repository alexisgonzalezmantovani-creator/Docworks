  ...
  function CuentaDocumentoPreview(){
    ...
    return <div className="cuenta-doc-preview-wrap">
      ...
      <div className="cuenta-doc-page" style={{fontFamily:serif}}>
        {localPerfil.logoDataUrl&&<div className="cuenta-doc-logo-row" style={{display:"flex",justifyContent:logoAlign==="izquierda"?"flex-start":logoAlign==="centro"?"center":"flex-end"}}><img src={localPerfil.logoDataUrl} style={{display:"block",maxWidth:logoWidth,maxHeight:logoMaxH,width:"auto",height:"auto",objectFit:"contain"}} alt="Logo"/></div>}
    ...
    <div>
      ...
      {tab==="equipo"&&(
        <div>
          ...
          <div style={{background:"linear-gradient(135deg,rgba(212,168,83,.07),transparent)",border:"1px solid rgba(212,168,83,.18)",borderRadius:16,padding:"16px 18px",marginBottom:20}}>
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:14,flexWrap:"wrap",marginBottom:13}}>
              <div><div style={{fontFamily:"DM Serif Display,serif",fontSize:16,color:"var(--text)"}}>Esquema de comisiones</div><div style={{fontSize:11,color:"var(--dim)",lineHeight:1.5,marginTop:3}}>Definí si todos cobran el mismo split o si cada broker tiene su propio porcentaje. El split se aplica a los honorarios de las operaciones que tenga asignadas.</div></div>
            ...
            <div style={{display:"flex",justifyContent:"flex-end",marginTop:10}}><Btn v="success" s="sm" onClick={function(){var next=Object.assign({},localPerfil,{broker_split_mode:splitMode,broker_split_pct:splitDefault});setLocalPerfil(next);savePerfil();}}>Guardar esquema de comisiones ✓</Btn></div>
          </div>
            ...
            <div className="card" style={{padding:0,overflow:"hidden"}}>
              ...
              <div style={{padding:13}}>
                ...
                {plantillaSubtab==="cuerpo"&&<div className="card" style={{padding:14,marginTop:10}}>
                  ...
                  {tipoDocSel==="reserva" ? ((!bloquesLoading||bloquesReserva.length>0)&&<ReservaBloquesPanel bloques={Array.isArray(bloquesReserva)?bloquesReserva:[]} onUpdate={onUpdateBloqueReserva} onMove={onMoveBloqueReserva} clausulasLib={Array.isArray(clausulas)?clausulas:[]} operaciones={Array.isArray(operaciones)?operaciones:[]} puedeEditar={puedeEditarPlantillas}/>) : <div style={{padding:16,borderRadius:12,border:"1px dashed var(--border)",background:"rgba(255,255,255,.012)"}}><div style={{fontSize:12.5,fontWeight:650,color:"var(--text)",marginBottom:4}}>Editor de cuerpo preparado para {TIPOS[tipoDocSel]||tipoDocSel}</div><div style={{fontSize:11,color:"var(--dim)",lineHeight:1.55}}>Este modelo ya tiene su cuerpo generado y sus datos dinámicos conectados a Operaciones. La edición por bloques se habilitará sobre este mismo espacio sin tocar la estructura ni perder variables.</div></div>}
                </div>}
              </div>
  ...
  function invitar(email, rolBase, perfilPermisos){
    return rpc("fn_invitar_usuario", { p_email: email, p_rol_base: rolBase, p_perfil_permisos: perfilPermisos }).then(function(res){
      if (!res.error) {
        ...
        rpc("fn_clausulas_sembrar",{}).catch(function(){});
        rpc("fn_bloques_reserva_sembrar",{}).catch(function(){});
      }
      return res;
    });
  }
...
export default function App(){
  ...
  function asegurarDatosInicialesInmobiliaria(){
    ...
    loadSupabaseJs().then(function(sb){
      if(!sb) return;
      sb.rpc("fn_clausulas_sembrar",{}).catch(function(){});
      sb.rpc("fn_bloques_reserva_sembrar",{}).catch(function(){});
    }).catch(function(){});
  }
  ...
  useEffect(function(){
    ...
    var timer=setTimeout(function(){
      setPerfil(function(cur){
        ...
        perfilNube.guardarAhora(next).catch(function(){});
        return next;
      });
    },1200);
