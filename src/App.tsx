...
async function generarPDF(doc, logoDataUrl, perfil) {
  try {
    ...
    function drawParagraph(prefixRuns, bodyText, opts){
      ...
      var maxW=W-ML-MR;
      // Rótulo: siempre termina en ": " (espacio) para no pegar el cuerpo.
      ...
      if(wrapped.length===0){
        if(runs&&runs.length){
          ensureSpace(gap);
          var curX0=ML;
          runs.forEach(function(r){ text(curX0,y,r.txt,size,r.bold,0,r.bold?labelColor:null); curX0+=mw(r.txt,size,r.bold); });
          y-=gap;
        }
      } else {
