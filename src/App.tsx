...
// DocWorks 3.64 PRO — edición profesional, correcciones de operaciones, comisiones, documentos, móvil y cláusulas.
// DocWorks 3.64 PRO — IA asistida con dictado por voz, revisión inteligente, búsqueda ampliada, cláusulas obligatorias y experiencia 100% en español.
      ...
      r.onload=function(ev){
        ...
        img.onload=async function(){
          ...
          try{
            var sb=await loadSupabaseJs();
            if(sb&&currentUserId){
              var up=await sb.from("usuarios").update({foto_url:data}).eq("id",currentUserId);
...
function useEquipoSupabase(ctx){
  ...
  function recargar(){
    ...
    loadSupabaseJs().then(function(sb){
      ...
      Promise.all([
        sb.from("usuarios").select("id,nombre,email,rol_base,perfil_permisos,activo,creado_en,foto_url").eq("inmobiliaria_id", inmobId),
        esAdmin
...
export default function App(){
  ...
  function elegirInstalar(){
    if(deferredInstallPrompt){
      ...
      lsSet("dw_install_choice_v1",true);
      setShowInstallChoice(false);
      dwNotify("info","Para instalarla en iPhone/iPad: Compartir → Agregar a pantalla de inicio. En algunos navegadores Android también aparece la opción Instalar app en el menú.");
    }
  }
      ...
      {showInstallChoice&&(
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(3,7,18,.58)",backdropFilter:"blur(7px)",display:"flex",alignItems:"center",justifyContent:"center",padding:18}}>
          <div role="dialog" aria-modal="true" style={{width:"min(430px,100%)",borderRadius:20,border:"1px solid rgba(212,168,83,.32)",background:"var(--card)",boxShadow:"0 24px 80px rgba(0,0,0,.38)",padding:22}}>
            ...
            <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.6,marginBottom:18}}>Estás entrando desde un teléfono o tablet. Podés instalar DocWorks para tener una experiencia más cómoda, o continuar usando la versión web.</div>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
              <button type="button" onClick={elegirInstalar} style={{height:42,border:0,borderRadius:10,background:"var(--gold)",color:"#111827",fontWeight:800,cursor:"pointer"}}>Instalar app</button>
