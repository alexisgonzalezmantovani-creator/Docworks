...
// DocWorks 3.65 PRO — edición profesional, correcciones de operaciones, comisiones, documentos, móvil y cláusulas.
// DocWorks 3.65 PRO — IA asistida con dictado por voz, revisión inteligente, búsqueda ampliada, cláusulas obligatorias y experiencia 100% en español.
...
function normalizarTextoDocumento(text) {
  ...
  // Identificación: si el número tiene 7/8 dígitos es DNI; si tiene 11,
  // es CUIT. Así los documentos descargados no muestran "DNI/CUIT" cuando
  // el dato cargado permite identificar el tipo con certeza.
  out = out.replace(/\bDNI\s*\/\s*CUIT\s*:?\s*([0-9][0-9.\-\s]{5,18}[0-9])\b/gi, function(_,raw){
    var digs=String(raw||"").replace(/\D/g,"");
    if(digs.length===7||digs.length===8) return "DNI: "+raw;
    if(digs.length===11) return "CUIT: "+raw;
    return "DNI/CUIT: "+raw;
  });
...
function derivedTokensGenerales(op) {
  ...
  return {
    ...
    alquiler_comision_locador_final: (op.alquiler_comision_locador!==undefined && op.alquiler_comision_locador!==null && String(op.alquiler_comision_locador)!=="") ? op.alquiler_comision_locador : ((op.alquiler_comision!==undefined && op.alquiler_comision!==null && String(op.alquiler_comision)!=="") ? op.alquiler_comision : "___"),
    alquiler_comision_locatario_final: (op.alquiler_comision_locatario!==undefined && op.alquiler_comision_locatario!==null && String(op.alquiler_comision_locatario)!=="") ? op.alquiler_comision_locatario : ((op.alquiler_comision!==undefined && op.alquiler_comision!==null && String(op.alquiler_comision)!=="") ? op.alquiler_comision : "___"),
    ...
    res_alq_plazo_cantidad: op.res_alq_plazo_cantidad || "___",
    res_alq_plazo_unidad: op.res_alq_plazo_unidad || "___",
    reserva_alquiler_inicio: fmtD(op.res_alq_inicio_estimado),
    reserva_alquiler_plazo_cantidad: op.res_alq_plazo_cantidad || "___",
    reserva_alquiler_plazo_unidad: op.res_alq_plazo_unidad || "___",
    reserva_alquiler_inicio_estimado: fmtD(op.res_alq_inicio_estimado),
    reserva_alquiler_monto_reserva: fmt$L(op.res_alq_monto_reserva,op.res_alq_moneda||"ARS"),
    reserva_alquiler_monto_mensual: fmt$L(op.res_alq_monto_mensual,op.res_alq_monto_mensual_moneda||op.res_alq_moneda||"ARS"),
    reserva_alquiler_aceptacion_dias: op.res_alq_aceptacion_dias || "2",
    reserva_alquiler_vigencia_dias: op.res_alq_vigencia_dias || "10",
    reserva_alquiler_comision_locador: (op.res_alq_comision_locador!==undefined && op.res_alq_comision_locador!==null && String(op.res_alq_comision_locador)!=="") ? op.res_alq_comision_locador : ((op.res_alq_comision!==undefined && op.res_alq_comision!==null && String(op.res_alq_comision)!=="") ? op.res_alq_comision : "___"),
    reserva_alquiler_comision_locatario: (op.res_alq_comision_locatario!==undefined && op.res_alq_comision_locatario!==null && String(op.res_alq_comision_locatario)!=="") ? op.res_alq_comision_locatario : ((op.res_alq_comision!==undefined && op.res_alq_comision!==null && String(op.res_alq_comision)!=="") ? op.res_alq_comision : "___"),
...
function resolveVars(text, op, extra) {
  ...
  var out=String(text).replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}\s*%(?:\s+del\s+(?:(?:primer)\s+)?canon\s+mensual)?/g, function(_, key) {
    ...
    if(key.indexOf("alquiler_comision_")===0) return (c.valor!=="" && c.valor!==undefined && c.valor!==null ? c.valor : "___")+"% del total del contrato";
    if(key.indexOf("res_alq_comision_")===0) return (c.valor!=="" && c.valor!==undefined && c.valor!==null ? c.valor : "___")+"% del canon mensual";
    return c.valor||"___";
  });
...
function totalContratoAlquiler(op){
  op=op||{};
  // La comisión de alquiler se calcula sobre el VALOR TOTAL DEL CONTRATO:
  // canon mensual × cantidad de meses (+ días prorrateados si existieran).
  var mensual=parseFloat(String(op.alquiler_monto_inicial==null?"":op.alquiler_monto_inicial).replace(/[^0-9.,-]/g,"").replace(/\.(?=\d{3}(?:\D|$))/g,"").replace(/,/g,"."));
  if(!isFinite(mensual)) mensual=0;
  var meses=parseInt(op.alquiler_plazo_meses,10);
  if(!isFinite(meses)||meses<0) meses=0;
  var dias=parseInt(op.alquiler_plazo_dias,10);
  if(!isFinite(dias)||dias<0) dias=0;
  if(mensual<=0 || (meses<=0 && dias<=0)) return 0;
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .topbar-inmob{display:flex;align-items:center;gap:8px;padding:5px 9px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.045);border-radius:10px;min-width:0;}
  .topbar-inmob img{height:34px;max-width:118px;width:auto;object-fit:contain;object-position:center;display:block;}
  .topbar-inmob .demo-badge{font-size:9px;font-weight:800;letter-spacing:.08em;color:var(--gold);padding:3px 6px;border:1px solid rgba(212,168,83,.35);border-radius:999px;background:rgba(212,168,83,.08);}
  .sesion-badge .logout-btn{color:#64748b !important;background:transparent !important;}
  .sesion-badge .logout-btn:hover{color:#475569 !important;background:var(--hover-bg) !important;}
  ...
  @media(max-width:640px){
    ...
    .main{min-height:calc(100dvh - 52px);min-height:calc(100svh - 52px);overflow:hidden;display:flex;flex-direction:column;}
    .content{padding:12px 12px calc(var(--mobile-nav-h) + 28px + env(safe-area-inset-bottom,0px)) 12px;max-height:none;min-height:0;flex:1 1 auto;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch;overscroll-behavior-y:auto;touch-action:pan-y;}
    ...
    .modal-box,.modal-box.wide,.modal-box.narrow{max-width:100%;width:100%;min-height:0;height:auto;border-radius:20px 20px 0 0;max-height:calc(100dvh - env(safe-area-inset-top,0px));max-height:calc(100svh - env(safe-area-inset-top,0px));}
  ...
  /* ═══════════════════════════════════════════
     DOCWORKS 3.65 — MOBILE / TABLET HARDENING
  ═══════════════════════════════════════════ */
  @media (max-width: 1100px){
    .content{min-width:0!important;overflow-x:hidden!important;}
    .card,.op-card,.modal-box,.cuenta-profile-card{max-width:100%;box-sizing:border-box;}
    .cuenta-dashboard-head,.cuenta-main-grid,.cuenta-info-grid,.cuenta-config-grid{min-width:0!important;}
    .cuenta-dashboard-stats{min-width:0!important;}
    .cuenta-dashboard-stat{min-width:0!important;}
    table{display:block;max-width:100%;overflow-x:auto;-webkit-overflow-scrolling:touch;}
    .btn,.inp,select,textarea,input{min-height:38px;box-sizing:border-box;}
  }
  @media (max-width: 760px){
    .content{padding:14px 12px calc(82px + env(safe-area-inset-bottom,0px))!important;max-height:none!important;}
    .main{overflow:visible!important;}
    .mobile-topbar{height:56px;padding:0 10px;gap:8px;}
    .mobile-topbar-logo{min-width:0;}
    .mobile-logo-text{font-size:16px;white-space:nowrap;}
    .cuenta-dashboard-head{display:block!important;}
    .cuenta-dashboard-stats{grid-template-columns:repeat(3,minmax(0,1fr))!important;margin-top:12px;}
    .cuenta-dashboard-stat{padding:8px 7px!important;}
    .cuenta-dashboard-stat-label{font-size:7.5px!important;}
    .cuenta-dashboard-stat-value{font-size:15px!important;}
    .cuenta-info-grid{grid-template-columns:1fr!important;}
    .cuenta-profile-card > div{padding-left:13px!important;padding-right:13px!important;}
    .cuenta-save-panel{flex-direction:column!important;align-items:stretch!important;}
    .cuenta-save-panel>div:last-child{width:100%!important;display:flex!important;justify-content:stretch!important;}
    .cuenta-save-panel button{flex:1!important;width:100%!important;}
    .op-card{padding:12px!important;}
    .op-card-side{min-width:0!important;max-width:none!important;border-left:0!important;border-top:1px solid var(--border);padding:10px 0 0!important;margin-top:10px;}
    .modal-overlay{padding:10px!important;align-items:flex-end!important;}
    .modal-box,.modal-box.wide,.modal-box.narrow{width:100%!important;max-width:100%!important;max-height:calc(100dvh - 10px)!important;border-radius:18px 18px 0 0!important;}
    .plantilla-appearance-grid,.cuenta-main-grid{grid-template-columns:1fr!important;}
    .preview-control-section{width:100%!important;}
    .sa-detail-row{flex-wrap:wrap!important;}
    .mobile-nav{padding-bottom:env(safe-area-inset-bottom,0px)!important;}
  }
  @media (min-width: 761px) and (max-width: 1100px){
    .content{padding:18px 16px 28px!important;}
    .cuenta-main-grid{grid-template-columns:1fr!important;}
    .cuenta-info-grid{grid-template-columns:repeat(2,minmax(0,1fr))!important;}
    .cuenta-dashboard-head{grid-template-columns:1fr!important;}
    .modal-box.wide{width:min(94vw,900px)!important;}
  }
`;
  ...
  return (
    <div style={{marginTop:12,paddingTop:12,borderTop:"1px dashed var(--border)"}}>
      ...
      {conColega&&<div style={{marginTop:10}}>
        ...
        {data.compartida_colega_id&&miembros.some(function(m){return m.id===data.compartida_colega_id;})&&(
          <div style={{display:"flex",alignItems:"center",gap:9,marginTop:8,padding:"8px 10px",borderRadius:10,border:"1px solid var(--border2)",background:"rgba(212,168,83,.055)"}}>
            <div style={{width:34,height:34,borderRadius:"50%",overflow:"hidden",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(145deg,var(--gold),var(--gold2))",color:"#111827",fontWeight:800,flexShrink:0}}>
              {((miembros.find(function(m){return m.id===data.compartida_colega_id;})||{}).fotoDataUrl)
                ? <img src={(miembros.find(function(m){return m.id===data.compartida_colega_id;})||{}).fotoDataUrl} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="Foto del colega"/>
                : (((miembros.find(function(m){return m.id===data.compartida_colega_id;})||{}).nombre||"?").charAt(0).toUpperCase())}
            </div>
            <div style={{minWidth:0}}>
              <div style={{fontSize:11.5,fontWeight:700,color:"var(--text)"}}>Operación compartida con {((miembros.find(function(m){return m.id===data.compartida_colega_id;})||{}).nombre)||"colega"}</div>
              <div style={{fontSize:10,color:"var(--dim)"}}>La foto de perfil se comparte con el equipo.</div>
            </div>
          </div>
        )}
...
function ComisionEditor({data,onChange,prefix,label,base,monedaBase,defaultPct,legacyKey}){
  ...
  // Si el usuario borra todos los dígitos, el input debe quedar vacío.
  // No volver a inyectar el valor legacy/default mientras escribe.
  var hasExplicitValue=Object.prototype.hasOwnProperty.call(data,prefix+"_valor");
  var valor=hasExplicitValue ? data[prefix+"_valor"] : (legacy!==undefined?legacy:defaultPct);
  ...
  return <div style={{padding:"11px 12px",borderRadius:11,border:"1px solid var(--border)",background:"rgba(255,255,255,.018)"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:modo==="fijo"?"1fr 1.2fr .8fr":"1fr 1.2fr",gap:7}}>
      ...
      <div style={{position:"relative",minWidth:0}}>
        <input className="inp" type="number" min="0" step="0.01" value={valor===0?"0":(valor||"")} onChange={function(e){onChange(prefix+"_valor",e.target.value);if(modo==="porcentaje")onChange(legacyKey||prefix,e.target.value);}} placeholder={modo==="fijo"?"Ej. 500000":"Ej. 4"} style={{paddingRight:modo==="porcentaje"?30:12}}/>
        {modo==="porcentaje"&&<span style={{position:"absolute",right:11,top:"50%",transform:"translateY(-50%)",color:"var(--dim)",fontWeight:700,pointerEvents:"none"}}>%</span>}
      </div>
  ...
  return(
    <div><div className="grid2">
      ...
      <div className="col2"><div style={{fontSize:11,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Comisión inmobiliaria</div>
        ...
        <div className="grid2">
          <ComisionEditor data={data} onChange={onChange} prefix="alquiler_comision_locador" legacyKey="alquiler_comision_locador" label="Locador" base={baseComision} monedaBase={data.alquiler_moneda||"ARS"} defaultPct={data.alquiler_comision!==undefined && data.alquiler_comision!==null && String(data.alquiler_comision)!=="" ? data.alquiler_comision : "1"}/>
          <ComisionEditor data={data} onChange={onChange} prefix="alquiler_comision_locatario" legacyKey="alquiler_comision_locatario" label="Locatario" base={baseComision} monedaBase={data.alquiler_moneda||"ARS"} defaultPct={data.alquiler_comision!==undefined && data.alquiler_comision!==null && String(data.alquiler_comision)!=="" ? data.alquiler_comision : "1"}/>
        </div>
        <CompartidaBlock data={data} onChange={onChange} equipo={equipo}/>
      </div>
    ...
    <div>
      ...
      <div className="grid2">
        ...
        <div className="col2"><div style={{fontSize:11,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Comisión inmobiliaria</div>
          ...
          <div className="grid2">
            <ComisionEditor data={data} onChange={onChange} prefix="res_alq_comision_locador" legacyKey="res_alq_comision_locador" label="Locador" base={parseFloat(data.res_alq_monto_mensual)||0} monedaBase={data.res_alq_moneda||"ARS"} defaultPct={data.res_alq_comision!==undefined && data.res_alq_comision!==null && String(data.res_alq_comision)!=="" ? data.res_alq_comision : "1"}/>
            <ComisionEditor data={data} onChange={onChange} prefix="res_alq_comision_locatario" legacyKey="res_alq_comision_locatario" label="Locatario" base={parseFloat(data.res_alq_monto_mensual)||0} monedaBase={data.res_alq_moneda||"ARS"} defaultPct={data.res_alq_comision!==undefined && data.res_alq_comision!==null && String(data.res_alq_comision)!=="" ? data.res_alq_comision : "1"}/>
          </div>
          <CompartidaBlock data={data} onChange={onChange} equipo={equipo}/>
        </div>
      </div>
...
function ClausulaFormBody({form,setForm,onCancel,onSubmit,editId}){
  ...
  form=form||FORM_CLAUSULA_EMPTY;
  var tiposSeguros=Array.isArray(form.tipos)&&form.tipos.length?form.tipos:["todos"];
  return(
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      ...
      <TipoSelector value={tiposSeguros} onChange={function(t){setForm(function(f){return Object.assign({},f,{tipos:t});});}}/>
      <label style={{display:"flex",alignItems:"flex-start",gap:8,padding:"9px 11px",borderRadius:8,border:"1px solid var(--border)",background:form.obligatoria?"rgba(180,83,9,0.08)":"transparent",cursor:"pointer",fontSize:12.5,color:"var(--text)"}}>
        ...
        <span>🔒 <b>OBLIGATORIA</b> — La definió el Dueño/Administrador de {typeof nombreInmobiliariaObligatoria!=="undefined"&&nombreInmobiliariaObligatoria?nombreInmobiliariaObligatoria:"tu inmobiliaria"} y no se puede quitar de esta operación.</span>
      </label>
      <div>
...
function DashboardView({operaciones,clausulas,perfil,bloquesReserva,onNew,onEdit,onDelete,onChangeEstado,onConvertir,onConvertirComodato,onConvertirRefuerzo,onConvertirBoleto,onConvertirDevolucion,onDuplicar,onViewDoc,onPerfil,searchOverride,papelera,onRestore,onDeletePermanente,onVaciarPapelera,confirmVaciarPapelera,setConfirmVaciarPapelera,papeleraJump,error,loading}){
  ...
  function getName(op){
      if(op.tipo==="alquiler"||op.tipo==="reserva_alquiler") return nombreConSufijo(op,"locatario","Locatario")+" (locatario) ← "+nombreConSufijo(op,"locador","Locador")+" (Locador)";
      return nombreConSufijo(op,"comprador","Comprador")+" (comprador) ← "+nombreConSufijo(op,"vendedor","Vendedor")+" (Vendedor)";
    }
  ...
  return(
    <div>
      ...
      <ConfirmModal
        ...
        message="¿Deseás eliminar esta operación? Va a quedar en la papelera durante 30 días y podrás restaurarla antes de que se elimine definitivamente."
        confirmLabel="Eliminar y enviar a papelera"
        danger={true}
        onCancel={function(){setConfirmEliminarId(null);}}
...
function PapeleraView({papelera,onRestore,onDeletePermanente,onVaciar,confirmVaciar,setConfirmVaciar,embedded}){
  ...
  function getName(op){if(op.tipo==="alquiler"||op.tipo==="reserva_alquiler")return nombreConSufijo(op,"locatario","Locatario")+" (locatario) ← "+nombreConSufijo(op,"locador","Locador")+" (Locador)";return nombreConSufijo(op,"comprador","Comprador")+" (comprador) ← "+nombreConSufijo(op,"vendedor","Vendedor")+" (Vendedor)";}
  return(
    <div>
...
function ConfiguracionView({ perfil, onChange, currentUserId, currentUserRole, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  const [supportSubject,setSupportSubject] = useState("Consulta / problema en DocWorks");
  const [supportMessage,setSupportMessage] = useState("");

  useEffect(function(){ setLocalPerfil(perfil); }, [perfil]);

  function enviarSoporte(){
    var email=(perfil&&perfil.soporte_email)||(perfil&&perfil.super_admin_email)||"";
    var asunto=(supportSubject||"Soporte DocWorks").trim();
    var cuerpo=["Usuario: "+(currentUserId||""),"Rol: "+(currentUserRole||perfil.rol||""),"","Mensaje:",supportMessage||""].join("\n");
    var url="mailto:"+encodeURIComponent(email)+"?subject="+encodeURIComponent(asunto)+"&body="+encodeURIComponent(cuerpo);
    try{window.location.href=url;setShowSupport(false);}catch(e){dwNotify("error","No se pudo abrir el correo de soporte.");}
  }
  ...
  async function handleMiFotoPerfil(e){
    var file=e.target.files&&e.target.files[0]; if(!file) return;
    if(file.size>5*1024*1024){dwNotify("error","La foto es demasiado grande. Elegí una imagen de hasta 5 MB.");e.target.value="";return;}
    try{
      var r=new FileReader();
      r.onload=function(ev){
        ...
        img.onload=async function(){
          ...
          var next=Object.assign({},localPerfil,{broker_fotos:Object.assign({},localPerfil.broker_fotos||{},currentUserId?{[currentUserId]:data}: {})});
          setLocalPerfil(next);
          try{
            await persistirPerfil(next);
            dwNotify("success","Foto de perfil actualizada. El equipo podrá verla en operaciones compartidas.");
          }catch(err){
            dwNotify("error","No se pudo sincronizar la foto con el equipo. Verificá tu conexión.");
          }
        };
        img.onerror=function(){dwNotify("error","No se pudo procesar la imagen.");};
        img.src=ev.target.result;
      };
      r.readAsDataURL(file);
    }catch(err){dwNotify("error","No se pudo cargar la foto.");}
    e.target.value="";
  }
  ...
  var rolActual = currentUserRole || perfil.rol || "dueno";
  var esBroker = rolActual==="broker" || rolActual==="vendedor" || rolActual==="viewer";
  var puedeEditarInstitucional = !esBroker;
  var puedeVerFacturacion = !esBroker && !!perfil && (perfil.rol==="dueno" || perfil.rol==="admin" || rolActual==="dueno" || rolActual==="admin");
  var TABS_CFG = esBroker ? [
    {id:"cuenta", label:"Mi perfil", icon:"👤"}
  ] : [
  ...
  return (
    <div>
      ...
      {tab==="cuenta"&&esBroker&&(
        <div style={{width:"100%",maxWidth:720,margin:"0 auto"}}>
          <div className="card cuenta-profile-card" style={{padding:0,overflow:"hidden",borderRadius:18}}>
            <div style={{padding:"18px 20px",borderBottom:"1px solid var(--border)",background:"linear-gradient(180deg,rgba(212,168,83,.075),rgba(212,168,83,.015))"}}>
              <div style={{fontSize:11,fontWeight:750,color:"var(--gold)",textTransform:"uppercase",letterSpacing:".10em"}}>Mi perfil de broker</div>
              <div style={{fontFamily:"DM Serif Display,serif",fontSize:22,color:"var(--text)",marginTop:4}}>Tu información personal</div>
              <div style={{fontSize:11,color:"var(--dim)",marginTop:4,lineHeight:1.5}}>Podés completar únicamente tus datos personales. La información de la inmobiliaria es administrada por el dueño o administrador.</div>
            </div>
            <div style={{padding:20,display:"flex",flexDirection:"column",gap:16}}>
              <div style={{display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
                <div style={{position:"relative",width:78,height:78,flexShrink:0}}>
                  <div style={{width:78,height:78,borderRadius:"50%",background:"linear-gradient(145deg,var(--gold),var(--gold2))",border:"2px solid rgba(255,255,255,.18)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",fontSize:25,fontWeight:800,color:"#0a0f1a"}}>{(localPerfil.broker_fotos&&currentUserId&&localPerfil.broker_fotos[currentUserId])?<img src={localPerfil.broker_fotos[currentUserId]} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="Foto de perfil"/>:(localPerfil.broker_perfiles&&currentUserId&&localPerfil.broker_perfiles[currentUserId]&&localPerfil.broker_perfiles[currentUserId].nombre||formatearNombreUsuarioVisible({usuario:{nombre:localPerfil.nombre||"Broker"}}).charAt(0).toUpperCase())}</div>
                  <label title="Cambiar foto de perfil" style={{position:"absolute",right:-3,bottom:-3,width:25,height:25,borderRadius:"50%",background:"var(--gold)",color:"#0a0f1a",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,fontWeight:900,border:"2px solid var(--card)"}}>+
                    <input type="file" accept="image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={handleMiFotoPerfil}/>
                  </label>
                </div>
                <div style={{minWidth:0,flex:1}}><div style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>{(localPerfil.broker_perfiles&&currentUserId&&localPerfil.broker_perfiles[currentUserId]&&localPerfil.broker_perfiles[currentUserId].nombre)||formatearNombreUsuarioVisible({usuario:{nombre:localPerfil.nombre||"Broker"}})}</div><div style={{fontSize:10.5,color:"var(--dim)",marginTop:4}}>Broker · {currentUserId?"Perfil sincronizado con el equipo":"Perfil local"}</div></div>
              </div>
              {function(){var bp=(localPerfil.broker_perfiles&&currentUserId&&localPerfil.broker_perfiles[currentUserId])||{}; function setBP(ch){var all=Object.assign({},localPerfil.broker_perfiles||{});all[currentUserId]=Object.assign({},bp,ch);setLocalPerfil(Object.assign({},localPerfil,{broker_perfiles:all}));} return <div className="cuenta-info-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:"12px 14px"}}>
                <Inp label="Nombre y apellido" value={bp.nombre||""} onChange={function(e){setBP({nombre:e.target.value});}} placeholder="Nombre y apellido"/>
                <Inp label="Teléfono personal" value={bp.telefono||""} onChange={function(e){setBP({telefono:e.target.value});}} placeholder="+54 9 11 1234-5678"/>
                <Inp label="Correo" value={(ctx&&ctx.usuario&&ctx.usuario.email)||bp.email||""} readOnly onChange={function(){}} />
                <Inp label="Rol" value={ROLES_DEF[rolActual]&&ROLES_DEF[rolActual].label||rolActual} readOnly onChange={function(){}} />
              </div>; }()}
              <div className="cuenta-save-panel"><div><div className="cuenta-save-title">Perfil personal</div><div className="cuenta-save-copy">Estos datos no modifican la cuenta ni la identidad de la inmobiliaria.</div></div><div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>{perfilSaveStatus==="saved"&&<span className="cuenta-save-state saved"><span>✓</span> Guardado</span>}<Btn v="secondary" onClick={function(){setShowSupport(true);}}>Contactar soporte</Btn><Btn v="success" onClick={savePerfil} disabled={perfilSaveStatus==="saving"}>{perfilSaveStatus==="saving"?"Guardando…":"Guardar perfil"}</Btn></div></div>
            </div>
          </div>
        </div>
      )}
            ...
            <div style={{padding:"20px 22px 18px"}}>
              <div className="cuenta-main-grid" style={{display:"grid",gridTemplateColumns:"minmax(250px,300px) minmax(0,1fr)",gap:22,alignItems:"start"}}>
                <div style={{display:"flex",flexDirection:"column",gap:12,minWidth:0}}>
                  <div style={{padding:"16px",borderRadius:14,border:"1px solid rgba(212,168,83,.24)",background:"linear-gradient(180deg,rgba(212,168,83,.08),rgba(255,255,255,.018))",minWidth:0}}>
                    <div style={{fontSize:10.5,fontWeight:750,color:"var(--gold)",textTransform:"uppercase",letterSpacing:".10em",marginBottom:11}}>Mi perfil</div>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      <div style={{position:"relative",width:72,height:72,flexShrink:0}}>
                        <div style={{width:72,height:72,borderRadius:"50%",background:"linear-gradient(145deg,var(--gold),var(--gold2))",border:"2px solid rgba(255,255,255,.18)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",fontSize:22,fontWeight:800,color:"#0a0f1a"}}>
                          {(localPerfil.broker_fotos&&currentUserId&&localPerfil.broker_fotos[currentUserId])
                            ? <img src={localPerfil.broker_fotos[currentUserId]} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="Foto de perfil"/>
                            : ((localPerfil.nombre||"Usuario").charAt(0).toUpperCase())}
                        </div>
                        <label title="Cambiar foto de perfil" style={{position:"absolute",right:-3,bottom:-3,width:24,height:24,borderRadius:"50%",background:"var(--gold)",color:"#0a0f1a",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:14,fontWeight:900,border:"2px solid var(--card)",boxSizing:"border-box"}}>
                          +
                          <input type="file" accept="image/png,image/jpeg,image/webp" style={{display:"none"}} onChange={handleMiFotoPerfil}/>
                        </label>
                      </div>
                      <div style={{minWidth:0}}>
                        <div style={{fontSize:14,fontWeight:700,color:"var(--text)"}}>Tu foto de perfil</div>
                        <div style={{fontSize:10.5,color:"var(--dim)",lineHeight:1.45,marginTop:4}}>Se sincroniza con tu usuario para que los integrantes del equipo puedan verla cuando compartan o gestionen una operación.</div>
                      </div>
                    </div>
                  </div>
                  <div style={{padding:"16px",borderRadius:14,border:"1px solid rgba(212,168,83,.24)",background:"linear-gradient(180deg,rgba(212,168,83,.08),rgba(255,255,255,.018))",minWidth:0}}>
                    <div style={{fontSize:10.5,fontWeight:750,color:"var(--gold)",textTransform:"uppercase",letterSpacing:".10em",marginBottom:11}}>Logo de la inmobiliaria</div>
                    <div style={{display:"flex",gap:12,alignItems:"center"}}>
                      ...
                      <div style={{minWidth:0}}>
                        ...
                        <div style={{fontSize:9.8,color:"var(--dim)",lineHeight:1.45,marginTop:7}}>Este logo se utiliza en encabezados y pies de documentos según la configuración visual.</div>
                      </div>
                    </div>
                    <input ref={fileRef} type="file" accept="image/*" style={{display:"none"}} onChange={handleLogo}/>
                  </div>
              ...
              <div className="cuenta-save-panel">
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
                  ...
                  <Btn v="secondary" onClick={function(){setShowSupport(true);}}>Contactar soporte</Btn><Btn v="success" onClick={savePerfil} style={{minWidth:170,height:40,padding:"0 18px",boxShadow:"0 7px 18px rgba(16,185,129,.14)"}} disabled={perfilSaveStatus==="saving"}>{perfilSaveStatus==="saving"?"Guardando…":"Guardar cambios"}</Btn>
                </div>
              </div>
            </div>
          </div>
      ...
      {showSupport&&<Modal open={true} onClose={function(){setShowSupport(false);}} title="Contactar soporte">
        <div style={{fontSize:12,color:"var(--dim)",lineHeight:1.55,marginBottom:12}}>Completá el motivo y el detalle. Se abrirá tu correo con el mensaje listo para enviar al canal de soporte configurado por el Super Admin.</div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <Inp label="Asunto" value={supportSubject} onChange={function(e){setSupportSubject(e.target.value);}}/>
          <Field label="Mensaje"><textarea className="inp" rows={7} value={supportMessage} onChange={function(e){setSupportMessage(e.target.value);}} placeholder="Contanos qué ocurrió, en qué pantalla y qué estabas haciendo…" style={{resize:"vertical",minHeight:140}}/></Field>
        </div>
        {!(perfil&&perfil.soporte_email)&&<div style={{marginTop:10,padding:"9px 11px",borderRadius:9,background:"rgba(251,191,36,.08)",border:"1px solid rgba(251,191,36,.22)",color:"var(--amber)",fontSize:10.5}}>Si el Super Admin todavía no configuró un correo de soporte, se abrirá el correo sin destinatario para que pueda completarse.</div>}
        <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:14,flexWrap:"wrap"}}><Btn v="ghost" onClick={function(){setShowSupport(false);}}>Cancelar</Btn><Btn v="success" onClick={enviarSoporte}>Abrir correo para enviar →</Btn></div>
      </Modal>}
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:14}}>
                    ...
                    <div style={{position:"relative",width:48,height:48,flexShrink:0}}>
                      <div style={{width:48,height:48,borderRadius:13,background:"linear-gradient(135deg,#1e2d42,#162032)",border:"1px solid var(--border2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:17,fontWeight:700,color:"var(--text)",overflow:"hidden"}}>
                        {esPendiente?"✉":((localPerfil.broker_fotos&&localPerfil.broker_fotos[m.id]||m.fotoDataUrl)?<img src={(localPerfil.broker_fotos&&localPerfil.broker_fotos[m.id])||m.fotoDataUrl} style={{width:"100%",height:"100%",objectFit:"cover"}} alt="Foto del asesor"/>:(m.nombre[0]?.toUpperCase()||"?"))}
                      </div>
...
var MOBILE_STABILITY_CSS = `
  /* Tablet/web stability: keep the whole shell scrollable and never clip
     controls when the viewport is between phone and desktop widths. */
  html,body,#root{max-width:100%;min-height:100%;}
  body{overflow-x:hidden;}
  .app-body,.main,.content{min-width:0;}
  @media (min-width:641px) and (max-width:1100px){
    .content{padding:18px 18px 28px !important;overflow-x:hidden;}
    .sidebar{width:64px;}
    .sidebar-bottom-icon{display:flex !important;align-items:center;justify-content:center;visibility:visible !important;opacity:1 !important;}
    .topbar{min-width:0;}
    .topbar button,.topbar .btn{flex-shrink:0;}
    .grid2{grid-template-columns:repeat(2,minmax(0,1fr));}
    .two-col-layout{grid-template-columns:minmax(0,1fr) minmax(0,1.15fr);}
  }
  ...
  return (
    <div className="sesion-badge" style={{position:"relative",display:"flex",alignItems:"center"}}>
      ...
      {abierto&&<div style={{position:"absolute",right:0,top:"calc(100% + 7px)",minWidth:190,zIndex:500,background:"var(--card)",border:"1px solid var(--border2)",borderRadius:12,boxShadow:"var(--shadow-lg)",padding:6}}>
        ...
        <button type="button" onClick={function(){setAbierto(false);onLogout();}} className="logout-btn" style={{width:"100%",textAlign:"left",padding:"9px 10px",border:0,borderRadius:8,background:"transparent",cursor:"pointer",fontSize:12,fontWeight:700}}>↪ Salir</button>
      </div>}
    </div>
  );
}
...
export default function App(){
  ...
  function instalarApp(){
    ...
    deferredInstallPrompt.userChoice.then(function(){ setDeferredInstallPrompt(null); }).catch(function(){ setDeferredInstallPrompt(null); });
  }
  useEffect(function(){
    function revisarDispositivo(){
      ...
      var tablet=/iPad|Android(?!.*Mobile)|Tablet/i.test(ua) || (touch && Math.min(window.innerWidth,window.innerHeight)>=600 && Math.min(window.innerWidth,window.innerHeight)<=1200);
    ...
    revisarDispositivo();
    window.addEventListener("resize",revisarDispositivo);
    return function(){window.removeEventListener("resize",revisarDispositivo);};
  },[]);
  useEffect(function(){
    if(!isMobileOrTablet||isStandalone) return;
    if(lsGet("dw_install_choice_v1",false)) return;
    var timer=setTimeout(function(){setShowInstallChoice(true);},1100);
    return function(){clearTimeout(timer);};
  },[isMobileOrTablet,isStandalone]);
  function continuarEnWeb(){
    lsSet("dw_install_choice_v1",true);
  ...
  function elegirInstalar(){
    if(deferredInstallPrompt){
      ...
      lsSet("dw_install_choice_v1",true);
      setShowInstallChoice(false);
      dwNotify("info","En iPhone/iPad: tocá Compartir → Agregar a pantalla de inicio → Agregar. Apple no permite que una web cree ese acceso directo automáticamente.");
    }
  }
  ...
  async function guardarBorradorYSalir(){
    ...
    if(draft&&draft.data&&draftTieneContenido(draft.data)){
      ...
      if(existingId){
        ...
        op=Object.assign({},existente||{},draft.data,{id:existingId,created_at:(existente&&existente.created_at)||new Date().toISOString(),estado:(existente&&existente.estado==="activo")?"activo":"borrador"});
      } else {
  ...
  useEffect(function(){
    if (equipoSupabase.equipo.length || !equipoSupabase.loading) {
      var fotos=(perfil&&perfil.broker_fotos)||{};
      setEquipo(equipoSupabase.equipo.map(function(m){ return Object.assign({},m,{fotoDataUrl:m.fotoDataUrl||fotos[m.id]||""}); }));
    }
  }, [equipoSupabase.equipo, perfil&&perfil.broker_fotos]);
  ...
  async function saveOp(data,accion,estadoFinal){
    ...
    var estadoElegido;
    var estadoOriginal=editOp ? (editOp.estado||"borrador") : null;
    if(editOp && estadoOriginal==="activo" && (!estadoFinal || estadoFinal==="borrador")){
      // Editar un documento ACTIVO no lo convierte accidentalmente en borrador.
      estadoElegido="activo";
    } else {
      estadoElegido=(estadoFinal==="activo"||estadoFinal==="cerrado"||estadoFinal==="borrador")?estadoFinal:(accion==="borrador"?"borrador":(opGuardada.estado||"activo"));
    }
  ...
  // "Eliminar" mueve la operación a la Papelera y la conserva 30 días.
  function deleteOp(id){
    var opConfirm=operaciones.find(function(o){return o.id===id;});
    var tituloConfirm=opConfirm ? (TIPOS[opConfirm.tipo]||"esta operación") : "esta operación";
    if(!window.confirm("¿Deseás eliminar "+tituloConfirm+"?\n\nLa operación va a quedar en la Papelera y podrá recuperarse durante 30 días.")) return;
    setOperaciones(function(ops){
  ...
  return(
    <>
      ...
      {showInstallChoice&&(
        <div style={{position:"fixed",inset:0,zIndex:1000,background:"rgba(3,7,18,.58)",backdropFilter:"blur(7px)",display:"flex",alignItems:"center",justifyContent:"center",padding:18}}>
          <div role="dialog" aria-modal="true" style={{width:"min(430px,100%)",borderRadius:20,border:"1px solid rgba(212,168,83,.32)",background:"var(--card)",boxShadow:"0 24px 80px rgba(0,0,0,.38)",padding:22}}>
            <div style={{fontSize:25,marginBottom:8}}>📱</div>
            <div style={{fontFamily:"DM Serif Display,serif",fontSize:23,color:"var(--text)",marginBottom:7}}>Usá DocWorks como app</div>
            {isIOSDevice ? <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.65,marginBottom:18}}>En iPhone y iPad Safari no permite que una web agregue sola un acceso directo. Hacelo en pocos pasos: <b style={{color:"var(--text)"}}>1.</b> Tocá <b style={{color:"var(--text)"}}>Compartir</b> · <b style={{color:"var(--text)"}}>2.</b> <b style={{color:"var(--text)"}}>Agregar a pantalla de inicio</b> · <b style={{color:"var(--text)"}}>3.</b> Tocá <b style={{color:"var(--text)"}}>Agregar</b>. Después DocWorks se abrirá como app.</div> : <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.6,marginBottom:18}}>Estás entrando desde un teléfono o tablet. Podés instalar DocWorks para tener una experiencia más cómoda, o continuar usando la versión web.</div>}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9}}>
              <button type="button" onClick={elegirInstalar} style={{height:42,border:0,borderRadius:10,background:"var(--gold)",color:"#111827",fontWeight:800,cursor:"pointer"}}>{isIOSDevice?"Ver cómo instalar":"Instalar app"}</button>
              <button type="button" onClick={continuarEnWeb} style={{height:42,border:"1px solid var(--border2)",borderRadius:10,background:"transparent",color:"var(--text)",fontWeight:700,cursor:"pointer"}}>Continuar en web</button>
            </div>
          </div>
        </div>
      )}
