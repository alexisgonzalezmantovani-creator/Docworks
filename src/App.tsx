...
function ReservaBloquesPanel({bloques, onUpdate, onMove, clausulasLib, operaciones, puedeEditar}){
  ...
  var ordenados=bloquesSeguros.filter(function(b){return b&&typeof b==="object";}).map(function(b){
    return Object.assign({id:"",titulo:"",contenido:"",orden:0,sinTitulo:false,obligatoria:false,condicion:""},b);
  ...
  function guardarEdicion(){
    ...
    try{ onUpdate(editId,{titulo:String(draft.titulo||""),contenido:String(draft.contenido||"")}); }catch(e){}
    cancelarEdicion();
  }
          ...
          <div key={String(b.id||idx)} style={{padding:"10px 12px",borderRadius:10,border:"1px solid var(--border2)",marginBottom:6,background:"var(--card)"}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:12.5,fontWeight:600,color:"var(--text)"}}>{b.sinTitulo?"Bloque sin título":(String(b.titulo||"").trim()||"Bloque sin título")}</span>
                ...
                {!editando&&<div style={{fontSize:11,color:"var(--muted)",marginTop:4,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{String(b.contenido||"").split("\n\n")[0]}</div>}
              </div>
              {puedeEditar&&!editando&&(
                <div style={{display:"flex",gap:3,flexShrink:0}}>
                  ...
                  <button type="button" onClick={function(){abrirEditor(b);}} title="Editar" style={{padding:"4px 7px",borderRadius:6,background:"rgba(255,255,255,0.04)",border:"1px solid var(--border)",color:"var(--muted)",cursor:"pointer",fontSize:12}}>✏</button>
                </div>
              )}
            </div>
            ...
            {editando&&(
              <div style={{marginTop:8,paddingTop:8,borderTop:"1px dashed var(--border2)"}}>
                {!b.sinTitulo&&<label className="field" style={{display:"block",marginBottom:8}}><span className="field-label">Título</span><input className="inp" value={draft.titulo} onChange={function(e){setDraft(function(d){return Object.assign({},d,{titulo:e.target.value});});}} /></label>}
                ...
                <div style={{fontSize:10.5,color:"var(--dim)",marginTop:4,marginBottom:8}}>Podés usar variables como {"{{moneda_txt}}"}, {"{{precio_letras}}"}, {"{{anticipo_letras}}"}, {"{{saldo_letras}}"}.</div>
