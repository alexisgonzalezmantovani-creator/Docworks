...
// DocWorks 3.24 FINAL — ajustes de Configuración, visor de documentos, acciones y estabilidad.
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .viewer-modal{max-width:980px!important;width:min(980px,calc(100vw - 28px));height:84vh;min-height:680px;max-height:84vh;}
  .viewer-modal .modal-head{background:linear-gradient(135deg,#0f2a52,#123b72);border-bottom-color:rgba(96,165,250,.28);}
  .viewer-modal .modal-title{color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
  .viewer-modal .modal-body{display:flex;flex-direction:column;min-height:0;overflow-y:auto;overflow-x:hidden;}
  .viewer-modal .doc-viewer-shell{min-height:0;display:flex;flex-direction:column;}
  .viewer-modal .doc-viewer-shell>*{flex-shrink:0;}
  .doc-header-actions{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap;min-width:0;}
  ...
  .viewer-modal .doc-header-actions .doc-action-docx,
  ...
  .viewer-modal .doc-header-actions .doc-action-docx:hover:not(:disabled),
  ...
  .viewer-modal .doc-header-actions .doc-action-pdf{background:linear-gradient(135deg,#c9962a,var(--gold),var(--gold2))!important;border:1px solid rgba(255,255,255,.18)!important;color:#08111f!important;box-shadow:0 2px 12px rgba(212,168,83,.24);}
  .viewer-modal .doc-header-actions .doc-action-pdf:hover:not(:disabled){color:#08111f!important;}
  ...
  .doc-bottom-tabs{position:sticky;bottom:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px;margin-top:14px;padding:8px 0 max(8px,env(safe-area-inset-bottom,0px));background:var(--card);border-top:1px solid var(--border);z-index:5;}
  .doc-bottom-tab{min-width:0;border:1px solid var(--border2);background:rgba(255,255,255,.03);color:var(--muted);border-radius:10px;padding:9px 8px;font:600 11.5px "DM Sans",sans-serif;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:5px;}
  ...
  @media(max-width:640px){.op-detail-hero{align-items:stretch;flex-direction:column;}.op-detail-title{font-size:19px;}.doc-viewer-tabs{margin-left:-2px;margin-right:-2px;}
    .viewer-modal{width:100%!important;max-width:100%!important;height:100dvh!important;min-height:0!important;max-height:100dvh!important;}
    .viewer-modal .modal-head{padding:12px 14px;}
    .doc-header-actions{width:100%;justify-content:flex-start;}
    .doc-header-actions .btn{min-height:36px;}
    .doc-header-actions .doc-action{padding:6px 9px;}
    .doc-header-actions .doc-action span{font-size:11px;}
...
function DocumentHeaderActions({onEdit,onPDF,onDOCX,onShare}){
  return (
    <div className="doc-header-actions">
      ...
      <Btn v="secondary" s="sm" className="doc-action doc-action-docx" onClick={onDOCX} title="Descargar DOCX"><DWIcon name="download" size={14}/> <span>DOCX</span></Btn>
      ...
      <Btn v="primary" s="sm" className="doc-action doc-action-pdf" onClick={onPDF} title="Descargar PDF"><DWIcon name="file" size={14}/> <span>PDF</span></Btn>
    </div>
  );
}
...
function DocumentViewer({op,clausulas,perfil,operaciones,bloquesReserva,onToggleTarea,onUpdateOperation,onActionWorkflow,tab,onChangeTab,autoAction,onAutoActionDone}){
  ...
  useEffect(function(){
    if(tabActual.esResumen || tabActual.esTareas || tabActual.esNegociacion) return;
    var cancelado=false;
  ...
  return(
    <div className="doc-viewer-shell">
      {op.tipo!=="reserva"&&<div className="doc-viewer-tabs"><DocTabsBar op={op} tab={tab} onChange={setTab}/></div>}
      {tabActual.esTareas ? (
      ...
      <div className="doc-viewer-export-block">
      {/* Export buttons */}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
      ...
      {op.tipo==="reserva"&&<DocumentBottomTabs op={op} tab={tab} onChange={changeTab}/>}
    ...
    <div>
      ...
      {tab==="cuenta"&&(
        <div style={{width:"100%",maxWidth:"none",margin:0}}>
          <div className="cuenta-config-grid" style={{display:"grid",gridTemplateColumns:"minmax(250px,.72fr) minmax(0,1.28fr)",gap:14,alignItems:"stretch"}}>
            <div style={{
              ...
              boxShadow:"0 10px 24px rgba(0,0,0,.07)",display:"flex",flexDirection:"column",minWidth:0
            }}>
              <div style={{display:"flex",alignItems:"center",gap:14,minWidth:0}}>
                <div style={{width:78,height:78,borderRadius:16,background:"rgba(255,255,255,.78)",border:"1px solid rgba(212,168,83,.30)",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",boxShadow:"0 8px 18px rgba(0,0,0,.10)",flexShrink:0}}>
                ...
                <div style={{minWidth:0}}>
                  <div style={{fontFamily:"DM Serif Display,serif",fontSize:19,color:"var(--text)",lineHeight:1.15,overflowWrap:"anywhere"}}>{localPerfil.nombre||"Tu inmobiliaria"}</div>
                  {localPerfil.matricula&&<div style={{fontSize:11,color:"var(--gold)",fontWeight:600,marginTop:5}}>Mat. {localPerfil.matricula}</div>}
                </div>
              </div>
              <div style={{fontSize:10.5,color:"var(--dim)",marginTop:13,lineHeight:1.45}}>Perfil visible en la aplicación y en los documentos generados.</div>
              <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:0,marginTop:"auto",paddingTop:16,borderTop:"1px solid var(--border)"}}>
                <div style={{textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:20,color:"var(--text)"}}>{opsTotal}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>Operaciones</div></div>
                <div style={{textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:20,color:"var(--green)"}}>{opsActivas}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>En operación</div></div>
                <div style={{textAlign:"center"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:20,color:"var(--teal)"}}>{equipo.length}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>Equipo</div></div>
              </div>
            </div>
