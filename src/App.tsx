...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  const [plantillaSubtab,setPlantillaSubtab] = useState("aspecto");
      ...
      {tab==="cuenta"&&(
        <div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {/* Avatar + stats */}
            <div style={{background:"linear-gradient(180deg,rgba(212,168,83,0.09),rgba(212,168,83,0.025))",border:"1px solid rgba(212,168,83,0.2)",borderRadius:16,padding:22,textAlign:"center"}}>
              <div style={{fontSize:10.2,color:"var(--dim)",fontWeight:700,letterSpacing:".08em",textTransform:"uppercase",marginBottom:12}}>Identidad de la inmobiliaria</div>
              <div style={{width:108,height:108,borderRadius:18,background:"rgba(255,255,255,.7)",border:"1px solid rgba(212,168,83,.28)",margin:"0 auto 14px",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",boxShadow:"0 8px 24px rgba(0,0,0,.10)"}}>
                {localPerfil.logoDataUrl?<img src={localPerfil.logoDataUrl} style={{width:"88%",height:"88%",objectFit:"contain",borderRadius:12}} alt="Logo de la inmobiliaria"/>:<div style={{fontSize:38,color:"var(--gold)",fontWeight:700}}>{localPerfil.nombre?localPerfil.nombre[0].toUpperCase():"D"}</div>}
              </div>
              <div style={{fontSize:11,color:"var(--dim)",fontWeight:700,letterSpacing:".06em",textTransform:"uppercase",marginBottom:5}}>Datos de la inmobiliaria</div>
              <div style={{fontFamily:"DM Serif Display,serif",fontSize:20,color:"var(--text)",marginBottom:3}}>{localPerfil.nombre||"Tu inmobiliaria"}</div>
              ...
              <div style={{display:"flex",justifyContent:"center",gap:20,marginTop:16,paddingTop:14,borderTop:"1px solid var(--border)"}}>
          ...
          <div className="card" style={{padding:20}}>
            <div style={{fontSize:15,fontFamily:"DM Serif Display,serif",color:"var(--text)",marginBottom:4}}>Información de la inmobiliaria</div><div style={{fontSize:10.8,color:"var(--dim)",marginBottom:16}}>Estos datos identifican a la inmobiliaria y se utilizan en los documentos.</div>
            ...
            <div className="card" style={{padding:14}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:7}}>
                {PLANTILLA_TIPOS.map(function(tipo){
                  ...
                  return <button key={tipo} onClick={function(){setTipoDocSel(tipo);setPlantillaSubtab("aspecto");setPlantillaForm(null);setDragClausulaId(null);setEncabezadoEditing(false);setConfiguracionPlantillaAbierta(false);setMostrarPreviewConfiguracion(false);setMostrarBloquesConfiguracion(false);}} style={{display:"flex",alignItems:"center",gap:9,width:"100%",minHeight:54,border:active?"1px solid rgba(212,168,83,.42)":"1px solid var(--border)",borderRadius:10,padding:"8px 10px",background:active?"rgba(212,168,83,.10)":"rgba(255,255,255,.012)",color:active?"var(--text)":"var(--muted)",cursor:"pointer",textAlign:"left",boxShadow:active?"0 4px 14px rgba(0,0,0,.08)":"none"}}>
            ...
            <div className="card" style={{padding:0,overflow:"hidden"}}>
              <div style={{padding:"5px 7px",borderBottom:"1px solid var(--border)",background:"rgba(148,163,184,.025)"}}>
                <div style={{display:"flex",gap:4,alignItems:"stretch",overflowX:"auto",WebkitOverflowScrolling:"touch"}}>
                  ...
                  <button type="button" onClick={function(){setPlantillaSubtab("aspecto");}} style={{flex:"0 0 auto",border:0,borderRadius:8,background:plantillaSubtab==="aspecto"?"var(--card)":"transparent",boxShadow:plantillaSubtab==="aspecto"?"0 1px 4px rgba(0,0,0,.10)":"none",color:plantillaSubtab==="aspecto"?"var(--text)":"var(--muted)",padding:"9px 13px",cursor:"pointer",fontSize:11.2,fontWeight:plantillaSubtab==="aspecto"?700:500,whiteSpace:"nowrap"}}>🎨 Aspecto</button>
                  {tipoDocSel==="reserva"&&<button type="button" onClick={function(){setPlantillaSubtab("reserva");}} style={{flex:"0 0 auto",border:0,borderRadius:8,background:plantillaSubtab==="reserva"?"var(--card)":"transparent",boxShadow:plantillaSubtab==="reserva"?"0 1px 4px rgba(0,0,0,.10)":"none",color:plantillaSubtab==="reserva"?"var(--text)":"var(--muted)",padding:"9px 13px",cursor:"pointer",fontSize:11.2,fontWeight:plantillaSubtab==="reserva"?700:500,whiteSpace:"nowrap"}}>✍️ Cuerpo de Reserva</button>}
                </div>
              </div>
              <div style={{padding:13}}>
                ...
                {plantillaSubtab==="reserva"&&tipoDocSel==="reserva"&&<div className="card" style={{padding:14,marginTop:10}}>
                  <div style={{fontSize:14,fontFamily:"DM Serif Display,serif",color:"var(--text)",marginBottom:4}}>Cuerpo editable de Reserva</div>
                  <div style={{fontSize:10.8,color:"var(--dim)",lineHeight:1.5,marginBottom:12}}>Modificá el texto de los bloques de la Reserva sin alterar los datos que se completan automáticamente desde Operaciones.</div>
