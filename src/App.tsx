...
async function generarPDF(doc, logoDataUrl, perfil) {
  try {
      ...
      if(encNombre||encMatricula||encWeb){
        if(estilo==="corporativo"){
          ...
          if(encNombre||encMatricula){
            var cabeceraCorp=encNombre?(perfil.nombre.toUpperCase()+(encMatricula&&perfil.matricula?" — Matricula: "+perfil.matricula:"")):("Matricula: "+perfil.matricula);
            text(ML+8,y+6,cabeceraCorp,12,true,0,blanco);
          }
          if(encWeb&&perfil.web) text(ML+8,y-9,perfil.web,7,false,0,blancoTenue);
        } else if(esClasico){
          // Caja con doble filete, sin color — estilo escribanía tradicional.
          hline(y+8,0.3,null,0.75);
          if(encNombre||encMatricula){
            var cabeceraClas=encNombre?(perfil.nombre.toUpperCase()+(encMatricula&&perfil.matricula?" — Matricula: "+perfil.matricula:"")):("Matricula: "+perfil.matricula);
            text(centeredX(cabeceraClas,12,true),y+6,cabeceraClas,12,true);
          }
          if(encWeb&&perfil.web) text(centeredX(perfil.web,7,false),y-9,perfil.web,7,false);
          hline(y-16,0.3,null,0.75);
        } else {
          // Minimalista: tinte muy suave del color de marca, sin bloque duro.
          rect(ML,y-6,W-ML-MR,28,rgbPdfStr(tintRgb(colorPrim,0.94)));
          if(encNombre||encMatricula){
            var cabeceraMin=encNombre?(perfil.nombre.toUpperCase()+(encMatricula&&perfil.matricula?" — Matricula: "+perfil.matricula:"")):("Matricula: "+perfil.matricula);
            text(ML+8,y+6,cabeceraMin,12,true,0,colorPrim);
          }
          if(encWeb&&perfil.web) text(ML+8,y-9,perfil.web,7,false);
        }
        y-=42;
      }
    }
    ...
    if(doc.conformidad&&doc.conformidad.items&&doc.conformidad.items.length){
      ...
      if(doc.conformidad.dni) text(ML,y-20,tipoIdentificacion(doc.conformidad.dni)+": "+doc.conformidad.dni,8,false);
    }
  ...
  try {
    ...
    if (perfil && perfil.nombre) {
      ...
      if (encNombreD || encMatriculaD) {
        ...
        body += px(cabeceraDocx, {bold:true, size:14, center:true, color:DOCX_COLOR_PRIM, spacing:{before:60,after:30}});
      }
    ...
    if (doc.conformidad && doc.conformidad.items && doc.conformidad.items.length) {
      ...
      if (doc.conformidad.dni) body += px(tipoIdentificacion(doc.conformidad.dni)+": "+doc.conformidad.dni, {size:9, color:"444444", spacing:{before:0,after:80}});
    }
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .app-wrap{position:relative;z-index:1;min-height:100vh;min-height:100dvh;display:flex;flex-direction:column;}

  /* ── Topbar ── */
  .topbar{height:62px;background:linear-gradient(115deg,#0b1730 0%,#122a58 55%,#0e2049 100%);border-bottom:1px solid rgba(255,255,255,0.08);display:flex;align-items:center;gap:14px;padding:0 20px;position:sticky;top:0;z-index:50;box-shadow:0 2px 16px rgba(0,0,0,0.25);}
  .topbar-logo{display:flex;align-items:center;gap:10px;width:220px;flex-shrink:0;}
  .topbar-logo img{height:62px;width:auto;object-fit:contain;filter:drop-shadow(0 0 8px rgba(212,168,83,0.35));}
  ...
  .sesion-badge span:first-child{color:#f1f5f9 !important;}
  ...
  /* ── Modal ── */
  .modal-overlay{position:fixed;inset:0;z-index:100;display:flex;align-items:center;justify-content:center;padding:12px;background:rgba(0,0,0,0.75);backdrop-filter:blur(10px);height:100vh;height:100dvh;}
  .modal-box{background:var(--card);border:1px solid var(--border2);border-radius:20px;width:100%;max-height:92vh;max-height:92dvh;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 32px 100px rgba(0,0,0,0.35);}
  ...
  .modal-body{padding:18px 20px 24px;overflow-y:auto;overscroll-behavior:contain;-webkit-overflow-scrolling:touch;min-height:0;}
  ...
  @media(max-width:640px){
    ...
    .main{min-height:calc(100dvh - 52px);overflow:visible;}
    .content{padding:12px 12px calc(var(--mobile-nav-h) + 20px + env(safe-area-inset-bottom,0px)) 12px;max-height:none;overflow:visible;}
    ...
    .modal-overlay{padding:0;align-items:flex-end;height:100dvh;min-height:100dvh;}
    .modal-box,.modal-box.wide,.modal-box.narrow{max-width:100%;width:100%;border-radius:20px 20px 0 0;max-height:calc(100dvh - env(safe-area-inset-top,0px));}
    .modal-body{padding:16px 16px calc(28px + env(safe-area-inset-bottom,0px));}
  ...
  return(
    <div>
      ...
      {step<=total&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:16,borderTop:"1px solid var(--border)",marginTop:14}}>
        ...
        {modoEdicion ? <Btn v="success" onClick={function(){handleFinalizarAccion("guardar");}}>💾 Guardar cambios</Btn> : (step<total?<Btn onClick={function(){setStep(function(s){return s+1;});}}>Siguiente →</Btn>:<Btn v="success" onClick={function(){setStep(total+1);}}>Revisar y finalizar →</Btn>)}
      </div>}
      {saveStatus&&<div style={{display:"flex",justifyContent:"flex-end",paddingTop:8}}><AutosaveIndicator status={saveStatus} compact={true}/></div>}
    </div>
  );
...
function DocumentViewer({op,clausulas,perfil,operaciones,onToggleTarea,tab,onChangeTab,autoAction,onAutoActionDone}){
  ...
  function getDoc(){
    if(tabActual.esTareas) return {titulo:"Centro de tareas",secciones:[]};
  ...
  useEffect(function(){
    if(tabActual.esResumen || tabActual.esTareas) return;
    var cancelado=false;
...
function ClausulasView({clausulas,onAdd,onEdit,onDelete,puedeEditar,rolLabel,error,loading}){
                  ...
                  <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3,flexWrap:"wrap"}}>
                        <span style={{fontSize:13,fontWeight:500,color:"var(--text)"}}>{c.titulo||"Todo OK"}</span>
...
function EstadisticasView({operaciones,equipo}){
  ...
  var total=operacionesRaiz.length;
  var now=new Date();
  ...
  for(var i=11;i>=0;i--){
    ...
    var ops=operacionesRaiz.filter(function(o){return (o.created_at||"").slice(0,7)===key;});
  ...
  for(var y=minAnio;y<=now.getFullYear();y++){
    var yops=operacionesRaiz.filter(function(o){return (o.created_at||"").slice(0,4)===String(y);});
  ...
  var brokers=(equipo||[]).map(function(m){
    var mops=operacionesRaiz.filter(function(o){return o.broker_id===m.id;});
...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  function savePerfil(){
    ...
    var resultado;
    try { resultado=onChange(localPerfil); } catch(e) { resultado={error:e}; }
    Promise.resolve(resultado).then(function(res){
      if(res&&res.error){ setPerfilSaveStatus("error"); return; }
      setPerfilSaveStatus("saved");
      setTimeout(function(){ setPerfilSaveStatus(null); },3000);
    }).catch(function(){ setPerfilSaveStatus("error"); });
  }
  ...
  return (
    <div>
        ...
        <div className="two-col-layout">
          ...
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            ...
            <div style={{background:"linear-gradient(135deg,rgba(212,168,83,0.08),rgba(212,168,83,0.02))",border:"1px solid rgba(212,168,83,0.2)",borderRadius:16,padding:20,textAlign:"center"}}>
              ...
              <div style={{fontSize:12,color:"var(--gold)"}}>{localPerfil.matricula?"Mat. "+localPerfil.matricula:(localPerfil.nombre_usuario||"Usuario") }</div>
          ...
          <div className="card" style={{padding:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:12}}>
              ...
              <div style={{paddingTop:8}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}><Btn v="success" onClick={savePerfil} style={{flex:1}} disabled={perfilSaveStatus==="saving"}>{perfilSaveStatus==="saving"?"Guardando…":"Guardar cambios ✓"}</Btn>{perfilSaveStatus==="saved"&&<span style={{fontSize:12,color:"var(--green)",fontWeight:600}}>✓ Guardado</span>}{perfilSaveStatus==="saving"&&<span style={{fontSize:12,color:"var(--gold)",fontWeight:600}}>Guardando…</span>}{perfilSaveStatus==="error"&&<span style={{fontSize:12,color:"var(--red)",fontWeight:600}}>⚠ No se pudo guardar</span>}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {tab==="plantillas"&&(function(){
        ...
        var estandar=lista.filter(function(c){return !!CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;});});
        var personalizadas=lista.filter(function(c){return !CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;});});
        var defaultsSeleccionados=perfil.clausulas_default_ids||[];
        var defaultsLocked=!!perfil.clausulas_default_locked;
        var defaultsLista=lista.filter(function(c){return defaultsSeleccionados.indexOf(c.id)!==-1;});
        var defaultsCount=defaultsLista.length;
        ...
        function nuevaClausula(){
          ...
          setClausulaOpenId(null); setPlantillaSubtab("mis");
        }
        function editarClausula(c){
          var esDefault=defaultsSeleccionados.indexOf(c.id)!==-1;
          var esEstandar=!!CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;});
          if(!puedeEditarPlantillas || esEstandar || (defaultsLocked && esDefault))return;
          setPlantillaForm({editId:c.id,titulo:c.titulo,categoria:c.categoria,contenido:c.contenido,tipos:c.tipos||["todos"],obligatoria:!!c.obligatoria});
          setClausulaOpenId(c.id); setPlantillaSubtab("mis");
        }
        function guardar(){
          if(!puedeEditarPlantillas)return;
        ...
        function toggleDefault(c){
          if(!puedeEditarPlantillas || defaultsLocked)return;
          var activo=defaultsSeleccionados.indexOf(c.id)!==-1;
          var next=activo?defaultsSeleccionados.filter(function(x){return x!==c.id;}):defaultsSeleccionados.concat([c.id]);
          var updated=Object.assign({},perfil,{clausulas_default_ids:next,clausulas_default_locked:false});
          onChange(updated);
          setLocalPerfil(function(p){return Object.assign({},p,{clausulas_default_ids:next,clausulas_default_locked:false});});
        }
        function lockDefaults(){ if(!puedeEditarPlantillas || defaultsLocked || defaultsCount===0)return; setConfirmLockDefaults(true); }
        function confirmLock(){
          var updated=Object.assign({},perfil,{clausulas_default_ids:defaultsSeleccionados,clausulas_default_locked:true});
          onChange(updated);
          setLocalPerfil(function(p){return Object.assign({},p,{clausulas_default_ids:defaultsSeleccionados,clausulas_default_locked:true});});
          setConfirmLockDefaults(false);
        }
        function unlockDefaults(){
          if(!puedeEditarPlantillas)return;
          var updated=Object.assign({},perfil,{clausulas_default_locked:false});
          onChange(updated);
          setLocalPerfil(function(p){return Object.assign({},p,{clausulas_default_locked:false});});
        }
        function toggleClausula(c){ setClausulaOpenId(function(id){return id===c.id?null:c.id;}); }
        ...
        function ClauseCard({c,mode}){
          var isOpen=clausulaOpenId===c.id;
          var esEstandar=!!CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;});
          var porDefecto=defaultsSeleccionados.indexOf(c.id)!==-1;
          var pos=lista.indexOf(c);
          var lockEdit=!puedeEditarPlantillas || esEstandar || (defaultsLocked&&porDefecto);
          var puedeMover=puedeEditarPlantillas && !esEstandar && !(defaultsLocked&&porDefecto);
          return <div key={c.id} style={{border:"1px solid "+(isOpen?"rgba(212,168,83,.28)":"var(--border)"),borderRadius:12,background:isOpen?"rgba(255,255,255,.025)":"rgba(255,255,255,.012)",overflow:"hidden",boxShadow:isOpen?"0 8px 24px rgba(0,0,0,.10)":"none"}}>
            <div draggable={puedeMover}
              onDragStart={function(e){if(!puedeMover)return;setDragClausulaId(c.id);try{e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",c.id);}catch(_){}}}
              onDragOver={function(e){if(puedeMover){e.preventDefault();}}}
              onDrop={function(e){if(!puedeMover)return;e.preventDefault();var fromId=dragClausulaId||((e.dataTransfer&&e.dataTransfer.getData("text/plain"))||"");if(fromId&&fromId!==c.id){moveClausulaTo(fromId,c.id,listaIds);}setDragClausulaId(null);}}
              onDragEnd={function(){setDragClausulaId(null);}}
              onClick={function(){toggleClausula(c);}}
              style={{display:"flex",alignItems:"center",gap:10,padding:"11px 12px",cursor:"pointer",opacity:dragClausulaId===c.id?.55:1}}>
              <div style={{width:30,height:30,borderRadius:9,background:porDefecto?"rgba(45,212,191,.10)":esEstandar?"rgba(212,168,83,.09)":"rgba(255,255,255,.035)",border:"1px solid "+(porDefecto?"rgba(45,212,191,.20)":"var(--border)"),display:"flex",alignItems:"center",justifyContent:"center",fontSize:10.5,fontWeight:700,color:porDefecto?"var(--teal)":"var(--muted)",flexShrink:0}}>{pos+1}</div>
              <div style={{minWidth:0,flex:1}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:12.7,fontWeight:650,color:"var(--text)"}}>{c.titulo||"Todo OK"}</span>
                  <SmallPill tone={esEstandar?"gold":"green"}>{esEstandar?"Estándar":"Propia"}</SmallPill>
                  {porDefecto&&<SmallPill tone="teal">✓ En mi configuración</SmallPill>}
                </div>
              ...
              {mode==="selector"&&puedeEditarPlantillas&&<div onClick={function(e){e.stopPropagation();}} style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}><input type="checkbox" checked={porDefecto} disabled={defaultsLocked} onChange={function(){toggleDefault(c);}}/><span style={{fontSize:10.3,color:porDefecto?"var(--teal)":"var(--muted)"}}>{porDefecto?"Incluida":"Agregar"}</span></div>}
              <span style={{fontSize:15,color:isOpen?"var(--gold)":"var(--dim)",width:16,textAlign:"center"}}>{isOpen?"⌃":"⌄"}</span>
            </div>
            {isOpen&&<div style={{borderTop:"1px solid var(--border)",padding:"11px 12px 12px"}} onClick={function(e){e.stopPropagation();}}>
              <div style={{fontSize:10.8,color:"var(--muted)",lineHeight:1.6,whiteSpace:"pre-wrap"}}>{c.contenido||"Sin contenido."}</div>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:11,paddingTop:9,borderTop:"1px solid var(--border)",flexWrap:"wrap"}}>
                <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                  <SmallPill tone={esEstandar?"gold":"muted"}>{esEstandar?"Biblioteca DocWorks":"Cláusula de la inmobiliaria"}</SmallPill>
                  {c.obligatoria&&<SmallPill tone="muted">Obligatoria</SmallPill>}
                </div>
                {!lockEdit&&<div style={{display:"flex",gap:5}}>
                  {puedeMover&&<><button onClick={function(){if(pos>0)moveClausulaTo(c.id,lista[pos-1].id,listaIds);}} disabled={pos<=0} style={{height:28,width:30,borderRadius:7,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:pos>0?"pointer":"not-allowed"}}>↑</button><button onClick={function(){if(pos<lista.length-1)moveClausulaTo(c.id,lista[pos+1].id,listaIds);}} disabled={pos>=lista.length-1} style={{height:28,width:30,borderRadius:7,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:pos<lista.length-1?"pointer":"not-allowed"}}>↓</button></>}
                  <button onClick={function(){editarClausula(c);}} style={{height:28,padding:"0 9px",borderRadius:7,border:"1px solid rgba(212,168,83,.22)",background:"rgba(212,168,83,.06)",color:"var(--gold)",cursor:"pointer",fontSize:10.3}}>Editar</button>
                  <button onClick={function(){onDuplicateClausula(c.id);}} style={{height:28,padding:"0 9px",borderRadius:7,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:10.3}}>Duplicar</button>
                  <button onClick={function(){onDeleteClausula(c.id);setClausulaOpenId(null);}} style={{height:28,padding:"0 9px",borderRadius:7,border:"1px solid rgba(248,113,113,.16)",background:"rgba(248,113,113,.05)",color:"var(--red)",cursor:"pointer",fontSize:10.3}}>Eliminar</button>
                </div>}
              </div>
              {lockEdit&&<div style={{marginTop:9,fontSize:10.2,color:"var(--dim)"}}>{esEstandar?"Cláusula estándar del sistema: se consulta desde esta biblioteca y no se modifica acá.":defaultsLocked&&porDefecto?"Cláusula incluida en la configuración bloqueada de la inmobiliaria.":"Solo lectura."}</div>}
            </div>}
          </div>;
        }

        function SectionHeader({title,subtitle,count}){
          return <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,marginBottom:11}}>
            <div><div style={{fontSize:13.2,fontWeight:650,color:"var(--text)"}}>{title}</div>{subtitle&&<div style={{fontSize:10.7,color:"var(--dim)",marginTop:3,lineHeight:1.45}}>{subtitle}</div>}</div>
            {count!=null&&<SmallPill tone="muted">{count}</SmallPill>}
          </div>;
        }
        ...
        return <div>
          <div className="card" style={{padding:13,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div><div style={{fontFamily:"DM Serif Display,serif",fontSize:18,color:"var(--text)"}}>Plantillas</div><div style={{fontSize:10.9,color:"var(--dim)",marginTop:3}}>Configurá una vez el modelo de cada documento y después usalo sin ruido.</div></div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>{defaultsLocked?<SmallPill tone="green">🔒 Configuración fija</SmallPill>:<SmallPill tone="gold">⚙ En configuración</SmallPill>}</div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8,marginTop:12}}>
              <span style={{fontSize:10.4,color:"var(--dim)",fontWeight:650}}>DOCUMENTO</span>
              <select value={tipoDocSel} onChange={function(e){setTipoDocSel(e.target.value);setPlantillaForm(null);setClausulaOpenId(null);setEncabezadoEditing(false);}} style={{flex:1,maxWidth:360,height:34,borderRadius:9,border:"1px solid var(--border)",background:"var(--surface)",color:"var(--text)",padding:"0 10px",fontSize:11.2}}>{Object.keys(TIPOS).map(function(k){return <option key={k} value={k}>{TIPOS[k]}</option>;})}</select>
            </div>
          </div>

          {!puedeEditarPlantillas&&<div style={{padding:"8px 11px",marginBottom:10,borderRadius:9,background:"rgba(251,191,36,.07)",border:"1px solid rgba(251,191,36,.17)",color:"var(--amber)",fontSize:10.8}}>🔒 Solo Dueño y Administrador pueden modificar esta configuración.</div>}

          <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:5,padding:4,background:"rgba(255,255,255,.018)",border:"1px solid var(--border)",borderRadius:10,marginBottom:10}}>
            {[{id:"defecto",label:"Mi configuración",meta:defaultsCount},{id:"estandar",label:"Estándar",meta:estandar.length},{id:"mis",label:"Mis cláusulas",meta:personalizadas.length},{id:"config",label:"Documento",meta:""}].map(function(t){var active=plantillaSubtab===t.id;return <button key={t.id} onClick={function(){setPlantillaSubtab(t.id);setPlantillaForm(null);setClausulaOpenId(null);}} style={{border:0,borderRadius:8,padding:"8px 7px",background:active?"rgba(212,168,83,.12)":"transparent",color:active?"var(--gold)":"var(--muted)",fontSize:10.7,fontWeight:active?650:500,cursor:"pointer"}}>{t.label}{t.meta!==""&&<span style={{opacity:.55,marginLeft:4}}>({t.meta})</span>}</button>;})}
          </div>

          {plantillaSubtab==="defecto"&&<div className="card" style={{padding:13}}>
            <SectionHeader title="Mi configuración por defecto" subtitle={defaultsLocked?"Este conjunto quedó fijado para las operaciones nuevas. Para cambiarlo hay que desbloquearlo explícitamente.":"Elegí las cláusulas que querés que la inmobiliaria use automáticamente en este tipo de documento. Cuando termines, bloqueá la configuración."} count={defaultsCount+" seleccionadas"}/>
            <div style={{padding:"10px 11px",borderRadius:10,background:defaultsLocked?"rgba(74,222,128,.05)":"rgba(212,168,83,.05)",border:"1px solid "+(defaultsLocked?"rgba(74,222,128,.16)":"rgba(212,168,83,.16)"),marginBottom:12}}>
              <div style={{fontSize:11.2,fontWeight:650,color:"var(--text)"}}>{defaultsLocked?"🔒 Configuración protegida":"Paso único de configuración"}</div>
              <div style={{fontSize:10.3,color:"var(--dim)",marginTop:2}}>{defaultsLocked?"La selección ya está lista para el equipo.":"Marcá o desmarcá cláusulas. La selección no cambia por abrir o editar una operación."}</div>
            </div>
            {defaultsLocked&&<div style={{display:"flex",flexDirection:"column",gap:7}}>{defaultsLista.length?defaultsLista.map(function(c){return <ClauseCard key={c.id} c={c} mode="locked"/>;}):<div style={{padding:18,textAlign:"center",border:"1px dashed var(--border)",borderRadius:10,color:"var(--dim)",fontSize:10.8}}>No hay cláusulas configuradas como predeterminadas.</div>}</div>}
            {!defaultsLocked&&<>
              <div style={{fontSize:10.5,color:"var(--dim)",fontWeight:650,textTransform:"uppercase",letterSpacing:".06em",marginBottom:7}}>Elegí las que querés dejar activas</div>
              <div style={{fontSize:10.2,color:"var(--dim)",marginBottom:8}}>Primero aparecen las estándar de DocWorks y debajo las cláusulas propias de la inmobiliaria.</div>
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {estandar.length>0&&<div>
                  <div style={{fontSize:9.8,color:"var(--gold)",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",margin:"2px 0 6px"}}>Estándar de DocWorks</div>
                  <div style={{display:"flex",flexDirection:"column",gap:7}}>{estandar.map(function(c){return <ClauseCard key={c.id} c={c} mode="selector"/>;})}</div>
                </div>}
                {personalizadas.length>0&&<div style={{marginTop:7}}>
                  <div style={{fontSize:9.8,color:"var(--teal)",fontWeight:700,textTransform:"uppercase",letterSpacing:".06em",margin:"2px 0 6px"}}>Propias de la inmobiliaria</div>
                  <div style={{display:"flex",flexDirection:"column",gap:7}}>{personalizadas.map(function(c){return <ClauseCard key={c.id} c={c} mode="selector"/>;})}</div>
                </div>}
              </div>
            </>}
            {puedeEditarPlantillas&&<div style={{display:"flex",justifyContent:"flex-end",gap:7,marginTop:13,paddingTop:11,borderTop:"1px solid var(--border)"}}>{defaultsLocked?<button onClick={unlockDefaults} style={{height:32,padding:"0 12px",borderRadius:8,border:"1px solid var(--border)",background:"transparent",color:"var(--muted)",cursor:"pointer",fontSize:10.7}}>Desbloquear para modificar</button>:<button onClick={lockDefaults} disabled={!defaultsCount} style={{height:32,padding:"0 12px",borderRadius:8,border:"1px solid rgba(74,222,128,.22)",background:defaultsCount?"rgba(74,222,128,.08)":"rgba(255,255,255,.02)",color:defaultsCount?"var(--green)":"var(--dim)",cursor:defaultsCount?"pointer":"not-allowed",fontSize:10.7,fontWeight:650}}>🔒 Finalizar y bloquear</button>}</div>}
          </div>}

          {plantillaSubtab==="estandar"&&<div className="card" style={{padding:13}}>
            <SectionHeader title="Cláusulas estándar" subtitle="Biblioteca base de DocWorks. Son de consulta y sirven como punto de partida para tu configuración." count={estandar.length}/>
            <div style={{fontSize:10.4,color:"var(--dim)",marginBottom:9}}>Estas cláusulas no se editan desde acá. Para una adaptación, duplicalas y trabajá sobre tu copia.</div>
            <div style={{display:"flex",flexDirection:"column",gap:7}}>{estandar.map(function(c){return <ClauseCard key={c.id} c={c} mode="readonly"/>;})}</div>
          </div>}

          {plantillaSubtab==="mis"&&<div className="card" style={{padding:13}}>
            <SectionHeader title="Mis cláusulas" subtitle="Cláusulas propias de la inmobiliaria. Acá creás y mantenés las adaptaciones que querés usar." count={personalizadas.length}/>
            {puedeEditarPlantillas&&<div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}><Btn s="sm" onClick={nuevaClausula}>＋ Nueva cláusula</Btn></div>}
            {plantillaForm&&puedeEditarPlantillas&&<div style={{border:"1px solid rgba(212,168,83,.22)",borderRadius:11,padding:12,marginBottom:12,background:"rgba(212,168,83,.035)"}}>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:9}}><div style={{fontSize:12.5,fontWeight:650,color:"var(--text)"}}>{plantillaForm.editId?"Editar cláusula":"Nueva cláusula"}</div><button onClick={function(){setPlantillaForm(null);}} style={{border:0,background:"transparent",color:"var(--dim)",fontSize:18,cursor:"pointer"}}>×</button></div>
              <ClausulaFormBody form={plantillaForm} setForm={function(updater){setPlantillaForm(function(f){return typeof updater==="function"?updater(f):updater;});}} editId={plantillaForm.editId} onSubmit={guardar} onCancel={function(){setPlantillaForm(null);}}/>
              <div style={{marginTop:10,paddingTop:10,borderTop:"1px solid var(--border)"}}><ClausulaIAComposer onDraft={function(draft){setPlantillaForm(function(f){return Object.assign({},f,{titulo:draft.titulo,categoria:draft.categoria||"general",contenido:draft.contenido});});}}/></div>
            </div>}
            {!personalizadas.length&&!plantillaForm&&<div style={{padding:22,textAlign:"center",border:"1px dashed var(--border)",borderRadius:10,color:"var(--dim)",fontSize:10.8}}>Todavía no tenés cláusulas propias. Creá una o duplicá una estándar.</div>}
            <div style={{display:"flex",flexDirection:"column",gap:7}}>{personalizadas.map(function(c){return <ClauseCard key={c.id} c={c} mode="mine"/>;})}</div>
          </div>}

          {plantillaSubtab==="config"&&<div style={{display:"flex",flexDirection:"column",gap:9}}>
            <div className="card" style={{padding:13}}><details><summary style={{fontSize:12.2,fontWeight:650,color:"var(--text)",cursor:"pointer"}}>Encabezado del documento</summary><div style={{marginTop:10}}><div style={{fontSize:10.7,color:"var(--dim)",marginBottom:8}}>Vista previa del encabezado para {TIPOS[tipoDocSel]}.</div><PlantillaPreview tipo={tipoDocSel} encabezado={encabezadoEditing?encabezadoDraft:customTextoPreview} clausulas={lista}/></div></details></div>
            {tipoDocSel==="reserva"&&<div className="card" style={{padding:13}}><details><summary style={{fontSize:12.2,fontWeight:650,color:"var(--text)",cursor:"pointer"}}>Bloques de Reserva</summary><div style={{marginTop:10}}>{bloquesError&&<div style={{marginBottom:10,padding:"8px 10px",borderRadius:8,background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.25)",color:"var(--red)",fontSize:11.2}}>{bloquesError}</div>}{bloquesLoading&&bloquesReserva.length===0&&<div style={{fontSize:11.5,color:"var(--dim)"}}>Cargando bloques…</div>}{(!bloquesLoading||bloquesReserva.length>0)&&<ReservaBloquesPanel bloques={bloquesReserva} onUpdate={onUpdateBloqueReserva} onMove={onMoveBloqueReserva} clausulasLib={clausulas} operaciones={operaciones} puedeEditar={puedeEditarPlantillas}/>}</div></details></div>}
          </div>}

          {confirmLockDefaults&&<Modal open={true} onClose={function(){setConfirmLockDefaults(false);}} title="Fijar configuración de la inmobiliaria">
            <div style={{fontSize:12.5,color:"var(--muted)",lineHeight:1.55,marginBottom:15}}>Vas a fijar las <b style={{color:"var(--text)"}}>{defaultsCount}</b> cláusulas elegidas para <b style={{color:"var(--text)"}}>{TIPOS[tipoDocSel]}</b>. Quedarán como la configuración por defecto de la inmobiliaria y para modificarlas después habrá que desbloquearlas explícitamente.</div>
            <div style={{display:"flex",justifyContent:"flex-end",gap:7}}><Btn v="ghost" onClick={function(){setConfirmLockDefaults(false);}}>Cancelar</Btn><Btn v="success" onClick={confirmLock}>Fijar configuración</Btn></div>
          </Modal>}
        </div>;
      })()}

      {/* ── TAB: AUDITORÍA (registro de cambios en cláusulas, bloques y operaciones) ── */}
        ...
        return (
          <div>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
              <div>
                <div style={{fontSize:15,fontWeight:600,color:"var(--text)"}}>Registro de auditoría</div>
                <div style={{fontSize:12,color:"var(--dim)"}}>Quién creó, editó, eliminó o restauró cláusulas, bloques y operaciones, y cuándo. Se guarda en la nube y no se puede borrar, para que quede como respaldo confiable.</div>
              </div>
            </div>
            {auditoriaSupabase&&auditoriaSupabase.error && <div style={{marginBottom:16,padding:"10px 14px",borderRadius:10,background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.25)",color:"var(--red)",fontSize:12.5}}>{auditoriaSupabase.error}</div>}
            <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
              ...
              <Slt value={auditAccion} onChange={function(e){setAuditAccion(e.target.value);}} style={{minWidth:160}}>
                ...
                <option value="restaurar">Restauraciones</option>
              </Slt>
            </div>
            {entries.length===0&&(
              <div className="card" style={{padding:30,textAlign:"center"}}>
                <p style={{color:"var(--dim)",fontSize:13}}>{auditoriaSupabase&&auditoriaSupabase.loading?"Cargando…":(auditLog.length===0?"Todavía no hay movimientos registrados.":"No hay resultados para ese filtro.")}</p>
              </div>
            )}
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
...
function LoginScreen({ onLogin, onRecover, error, loading, connError }){
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",background:AUTH_BG,padding:16,fontFamily:"'DM Sans',sans-serif"}}>
      <form onSubmit={submit} style={{width:"100%",maxWidth:380,background:AUTH_CARD,border:"1px solid "+AUTH_BORDER,borderRadius:16,padding:28,display:"flex",flexDirection:"column",gap:14,boxShadow:"0 8px 30px rgba(0,0,0,0.08)"}}>
        ...
        {!recoverMode && <div style={{display:"flex",flexDirection:"column",gap:5}}>
        ...
        <button type="submit" disabled={loading} style={{marginTop:4,padding:"11px 14px",borderRadius:10,border:"none",background:AUTH_GOLD,color:"#1a1206",fontWeight:700,fontSize:14,cursor:loading?"default":"pointer",opacity:loading?.6:1}}>
          {recoverMode ? (recoverLoading ? "Enviando…" : "Enviar enlace") : (loading ? "Ingresando…" : "Ingresar")}
        </button>
        <button type="button" onClick={function(){setRecoverMode(function(v){return !v;});setRecoverMsg(null);setPassword("");}} style={{background:"transparent",border:"none",color:AUTH_MUTED,cursor:"pointer",fontSize:12,textDecoration:"underline",padding:0}}>
          {recoverMode ? "Volver a iniciar sesión" : "¿Olvidaste tu contraseña? Recuperala"}
        </button>
      </form>
    </div>
  );
...
function useSuperAdmin(activo){
  ...
  return { dashboard:dashboard, inmobiliarias:inmobiliarias, planes:planes, loading:loading, error:error, recargar:recargar,
    ...
    pagos:pagos, pagosLoading:pagosLoading, cargarPagos:cargarPagos };
}
...
function SuperAdminDashboard({ contexto, onLogout }){
  ...
  function abrirDetalle(id, nombreActual){ setDetalleId(id); setNuevoPlanDetalle(""); setRenombrando(false); setNombreDetalle(nombreActual||""); sa.cargarDetalle(id); }
  ...
  var TABS_SA = [ ["resumen","Resumen","📊"], ["inmobiliarias","Inmobiliarias","🏢"], ["planes","Planes","📦"], ["auditoria","Auditoría","🕓"], ["pagos","Pagos","💳"] ];
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:400,overflow:"auto",background:"var(--bg)",color:"var(--text)",padding:24,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{maxWidth:1040,margin:"0 auto"}}>
        ...
        {tab==="auditoria" && (
          <div>
            <div className="filter-row" style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
              <div style={{width:260}}>
                <select className="inp" value={auditFiltro} onChange={function(e){setAuditFiltro(e.target.value);}}>
              ...
              {sa.auditoriaLoading && <div style={{fontSize:11.5,color:"var(--dim)"}}>Cargando…</div>}
            </div>
            <div className="sa-list">
              {!sa.auditoriaLoading && sa.auditoria.length===0 && <SAEmpty>Sin actividad registrada.</SAEmpty>}
              {sa.auditoria.map(function(a){
                return (
                  <div key={a.id} className="op-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,padding:"12px 16px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:12,minWidth:0}}>
                      <div className="sa-row-icon">{auditIcon(a.accion)}</div>
                      <div style={{minWidth:0}}>
                        <div style={{fontSize:13,fontWeight:600}}>{a.accion} <span style={{fontWeight:400,color:"var(--dim)"}}>· {a.entidad_tipo}</span></div>
                        <div style={{fontSize:11.5,color:"var(--dim)",marginTop:2}}>{a.inmobiliaria_nombre||"—"} · {a.usuario_email||"sistema"}</div>
                      </div>
                    </div>
                    <div style={{fontSize:11,color:"var(--dim)",whiteSpace:"nowrap"}}>{new Date(a.creada_en).toLocaleString("es-AR")}</div>
                  </div>
                );
              })}
            </div>
      ...
      {/* Detalle completo de inmobiliaria */}
      <Modal open={!!detalleId} onClose={cerrarDetalle} wide
        title={
          renombrando ? (
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <input className="inp" style={{fontSize:15,padding:"6px 10px"}} value={nombreDetalle} onChange={function(e){setNombreDetalle(e.target.value);}} autoFocus/>
            ...
            <span style={{display:"inline-flex",alignItems:"center",gap:8}}>
              {inmobiliariaDetalleResumen?inmobiliariaDetalleResumen.inmobiliaria_nombre:"Detalle"}
              <button title="Renombrar" onClick={function(){setRenombrando(true);}} style={{background:"transparent",border:"none",cursor:"pointer",fontSize:13,opacity:0.6,padding:2}}>✏️</button>
            </span>
          )
        }>
        ...
        {det && !sa.detalleLoading && (
          <div>
            ...
            <SASection title={"Usuarios (" + (det.usuarios?det.usuarios.length:0) + ")"}>
              ...
              <div className="sa-list">
                {(det.usuarios||[]).map(function(u){
                  return (
                    <div key={u.id} className="op-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10,padding:"10px 14px"}}>
                      <div style={{minWidth:160,display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div>
                          <div style={{fontSize:13,fontWeight:600}}>{u.nombre||u.email}</div>
                      ...
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <div style={{width:130}}>
                          <select className="inp" value={u.rol_base} onChange={function(e){cambiarRol(u.id, e.target.value);}}>
                            {ROLES_BASE.map(function(r){return <option key={r} value={r}>{r}</option>;})}
                          </select>
                        </div>
            ...
            <SASection title={"Cláusulas (" + sa.detalleClausulas.length + ")"}>
              {sa.detalleClausulasLoading && <div style={{fontSize:12,color:"var(--dim)"}}>Cargando…</div>}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:280,overflow:"auto"}}>
                {sa.detalleClausulas.map(function(c){
                  return (
                    <div key={c.id} className="op-card" style={{padding:"10px 12px",cursor:"default"}}>
                      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                        <div style={{fontSize:12.5,fontWeight:600}}>{c.titulo}</div>
                        <div style={{display:"flex",gap:6,alignItems:"center"}}>
                          {c.obligatoria && <SABadge tone="gold">obligatoria</SABadge>}
                          <SABadge tone={c.activo?"green":"dim"}>{c.activo?"activa":"inactiva"}</SABadge>
                        </div>
                      </div>
                      <div style={{fontSize:11,color:"var(--dim)",marginTop:3}}>{c.categoria}</div>
                      <div style={{fontSize:11.5,color:"var(--dim)",marginTop:4,lineHeight:1.4}}>{(c.contenido||"").slice(0,160)}{(c.contenido||"").length>160?"…":""}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <SASection title={"Plantillas de documentos (" + sa.detallePlantillas.length + ")"}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {sa.detallePlantillas.map(function(p){
                  return (
                    <div key={p.id} className="op-card" style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8,padding:"10px 12px",cursor:"default"}}>
                      <div>
                        <div style={{fontSize:12.5,fontWeight:600}}>{p.nombre} <span style={{color:"var(--dim)",fontWeight:400}}>· {p.tipo_documento}</span></div>
                        <div style={{fontSize:11,color:"var(--dim)",marginTop:2}}>v{p.version} · {p.bloques_count} bloque{p.bloques_count===1?"":"s"}</div>
                      </div>
                      <SABadge tone={p.activa?"green":"dim"}>{p.activa?"activa":"inactiva"}</SABadge>
                    </div>
                  );
                })}
              </div>
            ...
            <SASection title="Últimos pagos">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(det.ultimos_pagos||[]).map(function(p){
                  return (
                    <div key={p.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:12.5,padding:"8px 12px",borderRadius:9,background:"rgba(212,168,83,0.04)"}}>
                      <div>{fmt$(p.monto,p.moneda)} <span style={{color:"var(--dim)"}}>· {p.proveedor||"—"}</span></div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <SABadge tone={p.estado==="aprobado"||p.estado==="pagado"?"green":(p.estado==="rechazado"?"red":"amber")}>{p.estado}</SABadge>
                        <span style={{color:"var(--dim)",fontSize:11}}>{new Date(p.creado_en).toLocaleDateString("es-AR")}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <SASection title="Actividad reciente">
              ...
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                {(det.ultima_auditoria||[]).map(function(a,idx){
                  return (
                    <div key={idx} style={{fontSize:12,padding:"8px 12px",borderRadius:9,background:"rgba(212,168,83,0.04)",display:"flex",justifyContent:"space-between",gap:8}}>
                      <span>{auditIcon(a.accion)} {a.accion} <span style={{color:"var(--dim)"}}>· {a.entidad_tipo} · {a.usuario_email||"sistema"}</span></span>
                      <span style={{color:"var(--dim)",whiteSpace:"nowrap"}}>{new Date(a.creada_en).toLocaleString("es-AR")}</span>
                    </div>
                  );
                })}
              </div>
            ...
            <div style={{display:"flex",justifyContent:"flex-end"}}>
              <button className="btn btn-ghost" onClick={cerrarDetalle}>Cerrar</button>
            </div>
          </div>
        )}
      </Modal>
...
function useDocWorksSession(){
  ...
  function login(email, password){
    ...
    loadSupabaseJs().then(function(sb){
      ...
      sb.auth.signInWithPassword({ email: email, password: password }).then(function(res){
        ...
        if (res.error) { setLoginError(traducirError(traducirError(res.error.message))); return; }
      }).catch(function(err){
        setLoginLoading(false);
  ...
  return { ready:ready, session:session, contexto:contexto, contextoLoading:contextoLoading, superDashboard:superDashboard, loginError:loginError, loginLoading:loginLoading, connError:connError, login:login, recoverPassword:recoverPassword, logout:logout, retry:retry };
}

function SesionBadge({ contexto, onLogout }){
  if (!contexto || !contexto.usuario) return null;
  var usuario = (contexto.usuario.email||"").split("@")[0] || contexto.usuario.nombre || "Usuario";
  return (
...
function useAuditoriaSupabase(ctx){
  ...
  var [error,setError]=useState(null);
  var inmobId = ctx && ctx.inmobiliaria ? ctx.inmobiliaria.id : null;
  var ENTIDAD_LABEL = { clausula:"cláusula", bloque:"bloque", operacion:"operación" };
  function mapEntry(r){
    var val = r.valor_nuevo || r.valor_anterior || {};
    var titulo = val.titulo || val.nombre || val.tipo || (ENTIDAD_LABEL[r.entidad_tipo]||r.entidad_tipo||"");
    var u = r.usuarios || {};
    return { id:r.id, accion:r.accion, entidadTipo:r.entidad_tipo, clausulaTitulo:titulo, ts:r.creada_en, actorNombre:u.nombre||u.email||"—", actorRol:u.rol_base, cambios:[] };
  }
  function recargar(){
...
function usePerfilSupabase(ctx, perfil, setPerfil){
  ...
  function leer(sb){
    ...
    return sb.from("inmobiliarias").select("id,configuracion").eq("id",inmobId).maybeSingle().then(function(res){
      if(!res.error && res.data && res.data.configuracion && typeof res.data.configuracion === "object") {
        setPerfil(function(cur){ return Object.assign({},DEFAULT_PERFIL,cur,res.data.configuracion); });
        perfilCargado.current=true;
        return;
      }
      return sb.from("inmobiliarias").select("id,metadata").eq("id",inmobId).maybeSingle().then(function(r2){
        if(!r2.error && r2.data && r2.data.metadata && typeof r2.data.metadata === "object") {
          setPerfil(function(cur){ return Object.assign({},DEFAULT_PERFIL,cur,r2.data.metadata); });
        }
        perfilCargado.current=true;
      });
    }).catch(function(){ perfilCargado.current=true; });
  }
  function guardar(sb, p){
    if(!inmobId) return Promise.resolve({error:{message:"Sin inmobiliaria activa."}});
    return sb.from("inmobiliarias").update({configuracion:p}).eq("id",inmobId).then(function(res){
      if(!res.error) return res;
      return sb.from("inmobiliarias").update({metadata:p}).eq("id",inmobId);
    });
  }
  ...
  useEffect(function(){
    perfilCargado.current=false;
    if(!inmobId) return;
    loadSupabaseJs().then(function(sb){ if(sb) leer(sb).catch(function(){}); });
  },[inmobId]);
  useEffect(function(){
    if(!inmobId || !perfilCargado.current) return;
    var t=setTimeout(function(){ guardarAhora(perfil).catch(function(){}); },500);
    return function(){clearTimeout(t);};
  },[inmobId,perfil]);
  return {guardarAhora:guardarAhora};
}
...
export default function App(){
  ...
  function recargarClausulas(){
    ...
    loadSupabaseJs().then(function(sb){
      ...
      return sb.from("clausulas").select("*").order("orden").then(function(res){
        ...
        var activas = (res.data||[]).filter(function(c){return c.activo!==false;}).map(function(c){ return Object.assign({},c,{titulo:c.titulo||"Todo OK"}); });
        if (activas.length===0) {
          sb.rpc("fn_clausulas_sembrar",{}).then(function(){
            return sb.from("clausulas").select("*").order("orden").then(function(res2){
              ...
              setClausulas((res2.data||[]).filter(function(c){return c.activo!==false;}).map(function(c){ return Object.assign({},c,{titulo:c.titulo||"Todo OK"}); }));
            });
          });
        } else {
          setClausulasLoading(false);
  ...
  function updateBloqueReserva(id, cambios){
    ...
    if (actual && actual._dbId) {
      bloquesSupabase.editar(actual._dbId, cambios.titulo!==undefined?cambios.titulo:actual.titulo, cambios.contenido!==undefined?cambios.contenido:actual.contenido);
    }
  }
  ...
  async function saveOp(data,accion){
    ...
    try {
      ...
      setOperaciones(function(ops){
        return esNueva ? [opGuardada].concat(ops) : ops.map(function(o){return o.id===editOp.id?opGuardada:o;});
      });
      reportSave("saved");
  ...
  function changeEstado(id,estado){
    setOperaciones(function(ops){
      ...
      var nuevas = ops.map(function(o){ if(o.id!==id) return o; actualizado=Object.assign({},o,{estado:estado}); return actualizado; });
      if (actualizado) opsSupabase.guardar(actualizado);
      return nuevas;
    });
  }
  ...
  function toggleTarea(opId,tareaId){
    setOperaciones(function(ops){return ops.map(function(o){
      ...
      var actualizado=Object.assign({},o,{tareas_done:td});
      opsSupabase.guardar(actualizado);
      return actualizado;
    });});
  ...
  if (!docworksAuth.session) return <><GStyles/><LoginScreen onLogin={docworksAuth.login} onRecover={docworksAuth.recoverPassword} error={docworksAuth.loginError} loading={docworksAuth.loginLoading} connError={docworksAuth.connError}/></>;
  ...
  return(
    <>
      ...
      <div className="app-wrap">
        ...
        <div className="topbar">
          <div className="topbar-logo">
            <img src={LOGO_B64} alt="DocWorks" style={{height:62,width:"auto",objectFit:"contain",filter:"drop-shadow(0 0 12px rgba(212,168,83,0.5))"}}/>
      ...
      <Modal open={!!viewDoc} onClose={function(){setViewDoc(null);setDocAutoAction(null);}} title={"DocWorks v"+DOCWORKS_VERSION} wide={true}
