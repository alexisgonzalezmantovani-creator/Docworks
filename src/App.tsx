...
// DocWorks 3.52 PRO — edición premium, visor unificado, acciones, navegación y estabilidad.
// DocWorks 3.52 PRO — personalización amigable del texto de PRECIO + activación de
...
async function docworksMercadoPago(action, payload) {
  ...
  if (res.error) throw new Error(res.error.message || "No se pudo conectar con Mercado Pago.");
...
function initPWA() {
  try {
    ...
    if (typeof navigator !== "undefined" && navigator.serviceWorker && typeof location !== "undefined" && (location.protocol === "https:" || location.hostname === "localhost")) {
...
function derivedTokensGenerales(op) {
  ...
  return {
    ...
    alquiler_comision_locador_texto: textoComision(op,"alquiler_comision_locador","LOCADOR","primer canon mensual",parseFloat(op.alquiler_monto_inicial||0),op.alquiler_moneda||"ARS","alquiler_comision_locador",op.alquiler_comision||"1"),
    alquiler_comision_locatario_texto: textoComision(op,"alquiler_comision_locatario","LOCATARIO","primer canon mensual",parseFloat(op.alquiler_monto_inicial||0),op.alquiler_moneda||"ARS","alquiler_comision_locatario",op.alquiler_comision||"1"),
...
function resolveVars(text, op, extra) {
  ...
  var fixedLegacyKeys={};
  var out=String(text).replace(/\{\{\s*([a-zA-Z0-9_]+)\s*\}\}\s*%/g, function(_, key) {
    ...
    if(key.indexOf("comision_vendedor")==0 || key.indexOf("comision_comprador")==0){
      ...
      base=parseFloat((op||{}).alquiler_monto_inicial||0); moneda=(op||{}).alquiler_moneda||"ARS"; def=(op||{}).alquiler_comision||"1";
    } else {
...
function EncabezadoEditor({tipo, value, onChange, onCancel, onSave, onRestablecer}){
  ...
  async function conIA(){
    ...
    try{
      ...
      setStatus("ia-error"); setMsg((e&&e.message)||"No se pudo usar la IA");
    }
  }
...
function getComisionComponents(op){
  ...
  if(tipo==="reserva" || tipo==="boleto") {
    ...
    var baseA=parseFloat(op.alquiler_monto_inicial||0), monedaA=op.alquiler_moneda||"ARS";
...
function finalizeDoc(doc, op, clausulasLib) {
  ...
  // Leyenda de responsabilidad del Broker: se incorpora al documento final
  // para dejar expresamente asentado que la verificación del contenido de las
  // cláusulas corresponde al Broker antes de firmar, emitir o descargar.
  var brokerResponsabilidad =
    "El Broker es responsable de verificar que las cláusulas incorporadas a esta operación sean correctas, adecuadas y correspondan a lo acordado entre las partes. La revisión asistida por DocWorks no reemplaza la verificación final del Broker.";
  if (!doc.secciones) doc.secciones = [];
  var yaTieneResponsabilidadBroker = doc.secciones.some(function(sec){
    return String(sec && sec.titulo || "").trim().toUpperCase() === "VERIFICACIÓN DEL BROKER";
  });
  if (!yaTieneResponsabilidadBroker) {
    doc.secciones.push({
      titulo: "VERIFICACIÓN DEL BROKER",
      items: [brokerResponsabilidad]
    });
  }
...
async function mejorarEncabezadoIA(textoActual, tipo) {
  ...
  function normalizarErrorIA(err){
    ...
    return m || "No se pudo usar el asistente.";
  }
  var res, lastErr;
...
function buildDocSectionsStandard(op, clausulasLib, tipo) {
  ...
  doc = {
    ...
    secciones:[
      ...
      {titulo:"10. HONORARIOS DEL LOCADOR", items:[textoComision(op,"alquiler_comision_locador","LOCADOR","primer canon mensual",parseFloat(op.alquiler_monto_inicial||0),op.alquiler_moneda||"ARS","alquiler_comision_locador",op.alquiler_comision||"1")]},
      {titulo:"11. HONORARIOS DEL LOCATARIO", items:[textoComision(op,"alquiler_comision_locatario","LOCATARIO","primer canon mensual",parseFloat(op.alquiler_monto_inicial||0),op.alquiler_moneda||"ARS","alquiler_comision_locatario",op.alquiler_comision||"1")]},
    ],
    firmas:[
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .topbar-search-icon{position:absolute;left:14px;top:50%;transform:translateY(-50%);color:rgba(241,245,249,0.42);font-size:14px;pointer-events:none;}
    ...
    <div>
      ...
      <div className="grid2">
        ...
        {data.tipo==="reserva"&&<div className="col2" style={{marginTop:2,padding:"12px 14px",borderRadius:12,border:"1px solid rgba(212,168,83,.22)",background:"rgba(212,168,83,.045)"}}>
          ...
          <div style={{fontSize:10.5,color:"var(--dim)",marginBottom:9}}>El broker elige cómo vence la aceptación: por horas desde la firma, por días, o en una fecha fija. Se refleja en la cláusula y en el Calendario.</div>
  ...
  return(
    <div><div className="grid2">
      ...
      <div className="col2"><div style={{fontSize:11,fontWeight:700,color:"var(--dim)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Comisión inmobiliaria</div>
        ...
        <div className="grid2">
          <ComisionEditor data={data} onChange={onChange} prefix="alquiler_comision_locador" legacyKey="alquiler_comision_locador" label="Locador" base={monto} monedaBase={data.alquiler_moneda||"ARS"} defaultPct={data.alquiler_comision||"1"}/>
          <ComisionEditor data={data} onChange={onChange} prefix="alquiler_comision_locatario" legacyKey="alquiler_comision_locatario" label="Locatario" base={monto} monedaBase={data.alquiler_moneda||"ARS"} defaultPct={data.alquiler_comision||"1"}/>
        </div>
        <CompartidaBlock data={data} onChange={onChange} equipo={equipo}/>
      </div>
...
function StepClausulas({data,onChange,clausulas,esAlquiler,onAddClausula}){
  ...
  return(
    <div>
      <p style={{fontSize:13,color:"var(--muted)",marginBottom:8}}>Seleccioná las cláusulas a incluir. Las marcadas 🔒 <b>OBLIGATORIA</b> las definió el Dueño/Administrador y no se pueden quitar de esta operación.</p>
      <div style={{padding:"9px 11px",marginBottom:12,borderRadius:9,border:"1px solid var(--border2)",background:"var(--surface2)",fontSize:11.5,color:"var(--dim)",lineHeight:1.45}}>👤 <b style={{color:"var(--text)"}}>Responsabilidad del Broker:</b> el Broker es el encargado de verificar que las cláusulas sean correctas, adecuadas y correspondan a lo acordado entre las partes. La asistencia de DocWorks no reemplaza esta verificación final.</div>
  ...
  return(
    <div>
      ...
      <Tarjeta t="Condiciones económicas">
        ...
        {(data.tipo==="alquiler"||data.tipo==="reserva_alquiler")&&<Fila k="Comisión" v={getComisionComponents(data).map(function(c){return resumenComision(data,c.prefix,c.rol,c.prefix.indexOf("res_alq_")===0?"canon mensual":"primer canon mensual",c.base,c.baseMoneda,c.prefix,(c.prefix.indexOf("res_alq_")===0?data.res_alq_comision:data.alquiler_comision)||"1");}).join(" · ")}/>}
      </Tarjeta>
...
function EstadisticasView({operaciones,equipo,perfil,currentUserId,currentUserRole}){
  ...
  var rankingBase=(equipo||[]).filter(function(m){return m.rol==="broker";}).map(function(m){
    ...
    return {id:m.id,nombre:m.nombre||"Broker",foto:m.fotoDataUrl||"",operaciones:mops.length,facturacion:fact};
        ...
        {ESTADO_TABS.map(function(t){
          return(
            <button key={t.id} onClick={function(){setEstadoTab(t.id);}} style={{
              ...
              {t.label}
              <span style={{fontSize:11,padding:"1px 7px",borderRadius:20,background:estadoTab===t.id?"rgba(212,168,83,0.15)":"var(--surface2)",color:estadoTab===t.id?"var(--gold)":"var(--dim)"}}>{t.count}</span>
            </button>
          );
        })}
      </div>
  ...
  function renderOpRow(op,nested){
    ...
    return(
      <div key={op.id} className="op-card fade-up" style={Object.assign({},{marginTop:hijosOp.length?8:0},removiendo?{opacity:0,transform:"translateX(28px) scale(0.96)",pointerEvents:"none"}:{})}>
        {/* Meta: estado + tipo + fecha + banderas de colaboración (poco frecuentes) */}
        ...
        {/* Título = lo que identifica la operación de un vistazo: el inmueble */}
        <div style={{fontSize:14.5,fontWeight:700,color:"var(--text)",lineHeight:1.35,display:"flex",alignItems:"center",gap:5,minWidth:0}}>
          <span style={{color:"var(--gold)",flexShrink:0,display:"inline-flex"}}><DWIcon name="pin" size={12}/></span>
          <span style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{op.inmueble_direccion||"Sin dirección"}{op.inmueble_partido?" — "+op.inmueble_partido:""}</span>
        </div>
        {/* Subtítulo = las partes + el monto, en una sola línea */}
        <div style={{fontSize:12.5,color:"var(--muted)",marginTop:3,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>
          {getName(op)}{monto?<span style={{color:"var(--gold)",fontWeight:700}}> · {monto}</span>:null}
        </div>
        {(()=>{var pp=getProximoPasoOperacion(op,operaciones);return <div style={{marginTop:6,fontSize:11,color:pp.tone,fontWeight:700,display:"flex",alignItems:"center",gap:5}}><span style={{width:5,height:5,borderRadius:"50%",background:pp.tone,display:"inline-block",flexShrink:0}}/>{pp.texto}</div>;})()}
        {/* Acciones: Abrir es la única realmente frecuente y ahora se ve como tal */}
          ...
          {(esReserva||esResAlq)?(function(){
            ...
            return (
              <Btn v={faltan>0?"orange":"secondary"} s="sm" onClick={function(){setAddDocFor(op);}}>
                <DWIcon name="link" size={12} strokeWidth={2}/> {hijosOp.length>0?"Vinculados ("+hijosOp.length+")":"Agregar documento"}
              </Btn>
            );
          })():<span/>}
          {acciones}
          ...
          return <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div className="card" style={{padding:18,borderRadius:16,background:"linear-gradient(135deg,rgba(212,168,83,.09),rgba(255,255,255,.015))"}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
                <div><div style={{fontSize:10.5,color:"var(--gold)",fontWeight:750,textTransform:"uppercase",letterSpacing:".1em"}}>Cuenta premium</div><div style={{fontFamily:"DM Serif Display,serif",fontSize:21,color:"var(--text)",marginTop:4}}>Facturación y suscripción</div><div style={{fontSize:11,color:"var(--dim)",marginTop:4}}>Administrá el plan de tu inmobiliaria, medios de pago, comprobantes y estado de la suscripción.</div></div>
            ...
            <div style={{display:"grid",gridTemplateColumns:"minmax(0,1.15fr) minmax(280px,.85fr)",gap:12,alignItems:"start"}}>
              <div className="card" style={{padding:16}}>
                ...
                <div style={{marginTop:12,padding:12,borderRadius:10,background:"rgba(74,222,128,.045)",border:"1px solid rgba(74,222,128,.15)"}}><div style={{fontSize:10.5,fontWeight:700,color:"var(--green)"}}>🔐 Mercado Pago conectado</div><div style={{fontSize:10,color:"var(--dim)",marginTop:3,lineHeight:1.45}}>El checkout se procesa en Mercado Pago. DocWorks guarda únicamente identificadores, estados y referencias; nunca números completos de tarjeta.</div><button type="button" className="btn btn-primary" style={{marginTop:10,width:"100%"}} onClick={async function(){try{var code=fact.plan_codigo||localPerfil.plan_codigo||localPerfil.plan||"profesional";var r=await docworksMercadoPago("create_checkout",{plan_codigo:code,payer_email:fact.email||localPerfil.email||"" ,back_url:window.location.href});if(r.init_point){window.location.href=r.init_point;}else{throw new Error("Mercado Pago no devolvió el enlace de checkout.");}}catch(e){dwNotify("error",e.message||String(e));}}}>Continuar con Mercado Pago →</button><button type="button" className="btn btn-secondary" style={{marginTop:7,width:"100%"}} onClick={async function(){try{var r=await docworksMercadoPago("sync",{});if(r.subscription){var s=r.subscription;var next=Object.assign({},localPerfil,{facturacion:Object.assign({},fact,{estado_pago:s.status||fact.estado_pago,proximo_vencimiento:s.next_payment_date||fact.proximo_vencimiento,metodo_pago:s.payment_method_id||fact.metodo_pago})});setLocalPerfil(next);onChange(next);dwNotify("success","Estado de Mercado Pago actualizado.");}else{dwNotify("info","Todavía no hay una suscripción de Mercado Pago asociada.");}}catch(e){dwNotify("error",e.message||String(e));}}}>↻ Actualizar estado</button></div>
              </div>
              ...
              <div className="card" style={{padding:16}}>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  ...
                  <label style={{fontSize:10.5,color:"var(--dim)"}}>Email de facturación<input className="inp" style={{marginTop:4}} value={fact.email||localPerfil.email||""} onChange={function(e){guardarFact({email:e.target.value});}} placeholder="administracion@inmobiliaria.com" /></label>
      ...
      <Modal open={showCheckout} onClose={function(){setShowCheckout(false);}} title="Nueva suscripción · PRO">
        <div style={{display:"flex",flexDirection:"column",gap:15}}>
          <div style={{padding:14,borderRadius:12,border:"1px solid rgba(212,168,83,.25)",background:"rgba(212,168,83,.06)"}}>
            <div style={{fontWeight:800,fontSize:14}}>Checkout DocWorks</div>
          ...
          <div style={{display:"flex",justifyContent:"flex-end",gap:8}}><button className="btn btn-ghost" onClick={function(){setShowCheckout(false);}}>Cancelar</button><button className="btn btn-primary" onClick={function(){setShowCheckout(false);dwNotify("success","Checkout preparado. Falta conectar la pasarela de pago para procesar el cobro real.");}}>Continuar al pago</button></div>
        </div>
      </Modal>
...
class DocWorksErrorBoundary extends Component {
  constructor(props){
    super(props);
  ...
  componentDidCatch(error,info){
    try{ console.error("DocWorks: error de renderizado",error,info); }catch(e){}
  }
  recargar(){
  ...
  render(){
    ...
    var msg=this.state.error&&this.state.error.message ? String(this.state.error.message) : "Error inesperado de renderizado.";
    return (
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:24,background:"#eef2f7",fontFamily:"DM Sans,Arial,sans-serif",color:"#0f172a",boxSizing:"border-box"}}>
        <div style={{width:"min(560px,100%)",background:"#fff",border:"1px solid rgba(15,23,42,.12)",borderRadius:18,padding:28,boxShadow:"0 18px 50px rgba(15,23,42,.12)"}}>
          <div style={{fontSize:30,marginBottom:10}}>⚠️</div>
          <div style={{fontSize:22,fontWeight:800,marginBottom:8}}>DocWorks detectó un error</div>
          <div style={{fontSize:13,lineHeight:1.6,color:"#475569",marginBottom:16}}>La aplicación evitó mostrar una página en blanco. Podés recargar y volver a intentar.</div>
          <div style={{fontSize:11.5,lineHeight:1.5,color:"#7f1d1d",background:"#fef2f2",border:"1px solid #fecaca",borderRadius:10,padding:"10px 12px",wordBreak:"break-word",marginBottom:18}}>{msg}</div>
          <button type="button" onClick={()=>this.recargar()} style={{height:40,padding:"0 16px",border:0,borderRadius:10,background:"#142a4d",color:"#fff",fontWeight:800,cursor:"pointer"}}>Recargar DocWorks</button>
        </div>
      </div>
    );
  }
...
function App(){
  ...
  function recargarClausulas(){
    ...
    if (!inmobId) { setClausulas([]); setClausulasLoading(false); return Promise.resolve(); }
      ...
      <div className="app-wrap">
        ...
        <div className="topbar">
          ...
          <div className="topbar-search">
            <span className="topbar-search-icon" aria-hidden="true"><DWIcon name="search" size={16} strokeWidth={2}/></span>
            <input
              value={topSearch}
...
export default function DocWorksRoot(){
  return <DocWorksErrorBoundary><App/></DocWorksErrorBoundary>;
}
