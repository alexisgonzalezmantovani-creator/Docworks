...
// DocWorks 3.22 — documento consistente, guardado protegido y exportación prolija.
...
function normalizarTextoDocumento(text) {
  // Tipografía consistente: cuando una etiqueta termina en ":" debe existir
  // un espacio antes del contenido. No toca horas (10:30), URLs ni otros
  // patrones numéricos porque solo corrige dos puntos seguidos de letra.
  return String(text || "").replace(/:([A-Za-zÁÉÍÓÚÜÑáéíóúüñ])/g, ": $1");
}
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  @media(max-width:900px){.cuenta-config-grid{grid-template-columns:1fr !important;}.cuenta-config-grid > div:first-child{grid-template-columns:auto minmax(0,1fr) !important;}.cuenta-config-grid > div:first-child > div:nth-child(3){grid-column:1 / -1;}}
...
function Modal({open,onClose,title,children,wide,formTipo,headerExtra,headerRight}){
  if(!open)return null;
  return(
  ...
  return (
    <div className="doc-summary-panel">
      <div className="op-detail-hero">
        ...
        <span className={"badge "+(estadoBadgeCls[op.estado]||"badge-activo")} style={{flexShrink:0}}>{ESTADOS[op.estado]||op.estado}</span>
      </div>
      ...
      {timeline.length>0&&(
        <div className="card op-clean-card" style={{padding:16,marginBottom:16}}>
    ...
    <div>
      ...
      {tab==="cuenta"&&(
        <div style={{width:"100%",maxWidth:"none",margin:0}}>
          <div className="cuenta-config-grid" style={{display:"grid",gridTemplateColumns:"minmax(0,1fr)",gap:14,alignItems:"start"}}>
            <div style={{
              background:"linear-gradient(180deg,rgba(212,168,83,0.10),rgba(212,168,83,0.025))",
              border:"1px solid rgba(212,168,83,0.20)",borderRadius:16,padding:"18px 20px 16px",textAlign:"left",
              boxShadow:"0 10px 24px rgba(0,0,0,.07)",display:"grid",gridTemplateColumns:"auto minmax(220px,1fr) minmax(280px,520px)",gap:18,alignItems:"center"
            }}>
              <div style={{width:74,height:74,borderRadius:16,background:"rgba(255,255,255,.78)",border:"1px solid rgba(212,168,83,.30)",margin:"0",display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",boxShadow:"0 8px 18px rgba(0,0,0,.10)"}}>
                {localPerfil.logoDataUrl?<img src={localPerfil.logoDataUrl} style={{width:"86%",height:"86%",objectFit:"contain",borderRadius:10}} alt="Logo de la inmobiliaria"/>:<div style={{fontSize:28,color:"var(--gold)",fontWeight:700}}>{localPerfil.nombre?localPerfil.nombre[0].toUpperCase():"D"}</div>}
              </div>
              <div style={{fontFamily:"DM Serif Display,serif",fontSize:18,color:"var(--text)",marginBottom:3,overflowWrap:"anywhere"}}>{localPerfil.nombre||"Tu inmobiliaria"}</div>
              {localPerfil.matricula&&<div style={{fontSize:11,color:"var(--gold)",fontWeight:600}}>Mat. {localPerfil.matricula}</div>}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:0,margin:0,paddingTop:0,borderTop:0}}>
                <div style={{textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:19,color:"var(--text)"}}>{opsTotal}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>Operaciones</div></div>
                <div style={{textAlign:"center",borderRight:"1px solid var(--border)"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:19,color:"var(--green)"}}>{opsActivas}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>En operación</div></div>
                <div style={{textAlign:"center"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:19,color:"var(--teal)"}}>{equipo.length}</div><div style={{fontSize:8.8,color:"var(--dim)",marginTop:1,lineHeight:1.2}}>Equipo</div></div>
              </div>
            </div>

            <div className="card" style={{padding:"18px 18px 16px",width:"100%",boxSizing:"border-box"}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:10,flexWrap:"wrap",marginBottom:12}}>
                <div><div style={{fontSize:16,fontFamily:"DM Serif Display,serif",color:"var(--text)",marginBottom:3}}>Información de la inmobiliaria</div><div style={{fontSize:10.5,color:"var(--dim)",lineHeight:1.45}}>Estos son los datos existentes de tu cuenta. Editalos solo desde acá cuando corresponda.</div></div>
                {perfilSaveStatus==="saved"&&<span style={{fontSize:11.5,color:"var(--green)",fontWeight:600}}>✓ Guardado</span>}
              </div>
              <div className="cuenta-info-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:"10px 12px",alignItems:"start"}}>
                ...
                <div style={{display:"flex",alignItems:"stretch",minHeight:38,paddingTop:16}}>
                  <Btn v="success" onClick={savePerfil} style={{width:"100%"}} disabled={perfilSaveStatus==="saving"}>{perfilSaveStatus==="saving"?"Guardando…":"Guardar cambios ✓"}</Btn>
                </div>
              </div>
              {perfilSaveStatus==="error"&&<div style={{marginTop:10,padding:"8px 10px",borderRadius:8,background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.20)",color:"var(--red)",fontSize:11.2}}>No se pudieron guardar los cambios.</div>}
            </div>
          </div>
        </div>
      )}
  ...
  return(
    <>
      ...
      <Modal open={!!viewDoc} onClose={function(){setViewDoc(null);setDocAutoAction(null);}} title={"DocWorks v"+DOCWORKS_VERSION} wide={true}
        headerRight={viewDoc?<Btn v="secondary" s="sm" onClick={function(){var op=viewDoc;setViewDoc(null);setDocAutoAction(null);setEditOp(op);setFormTipo(op.tipo||"reserva");setShowForm(true);}}><span style={{display:"inline-flex",alignItems:"center",gap:6}}><DWIcon name="edit" size={14}/>Editar</span></Btn>:null}>
