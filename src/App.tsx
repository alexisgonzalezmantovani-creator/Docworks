...
function PrecioNarrativaEditor({bloque, onSave, onCancel}){
  ...
  function syncTexto(){
    if(editorRef.current) setTexto(textoNarrativaPrecioDocWorks(editorRef.current));
  }
    ...
    <div>
      ...
      {tab==="plantillas"&&(function(){
        ...
        return <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"minmax(185px,195px) minmax(0,1fr)",gap:10,alignItems:"start"}}>
            ...
            <div>
              ...
              <div className="card" style={{padding:13,marginBottom:9}}>
                <div style={{display:"flex",gap:6,borderBottom:"1px solid var(--border)",marginBottom:12}}>
                  <button type="button" onClick={function(){setPlantillaSubtab("clausulas");}} style={{flex:1,border:0,borderBottom:plantillaSubtab==="clausulas"?"2px solid var(--gold)":"2px solid transparent",background:"transparent",color:plantillaSubtab==="clausulas"?"var(--text)":"var(--muted)",padding:"9px 8px",cursor:"pointer",fontSize:11.5,fontWeight:plantillaSubtab==="clausulas"?700:500}}>Cláusulas de esta plantilla</button>
                  <button type="button" onClick={function(){setPlantillaSubtab("configuracion");}} style={{flex:1,border:0,borderBottom:plantillaSubtab==="configuracion"?"2px solid var(--gold)":"2px solid transparent",background:"transparent",color:plantillaSubtab==="configuracion"?"var(--text)":"var(--muted)",padding:"9px 8px",cursor:"pointer",fontSize:11.5,fontWeight:plantillaSubtab==="configuracion"?700:500}}>Configuración del documento</button>
                </div>
                ...
                {plantillaSubtab==="configuracion"&&<div>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    ...
                    <div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}><button type="button" onClick={function(){setMostrarPreviewConfiguracion(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}><span style={{fontSize:11.8,fontWeight:650}}>Vista previa del documento</span><span style={{fontSize:10.5,color:"var(--dim)"}}>{mostrarPreviewConfiguracion?"▲ Ocultar":"▼ Mostrar"}</span></button>{mostrarPreviewConfiguracion&&<div style={{marginTop:9}}><div style={{fontSize:10.4,color:"var(--dim)",marginBottom:8}}>Así se verá el documento con datos de ejemplo, sin modificar ninguna operación real.</div><PlantillaPreview tipo={tipoDocSel} encabezado={encabezadoEditing?encabezadoDraft:customTextoPreview} clausulas={listaPlantilla}/></div>}</div>
                    {tipoDocSel==="reserva"&&<div style={{border:"1px solid var(--border)",borderRadius:10,padding:11,background:"rgba(255,255,255,.012)"}}><button type="button" onClick={function(){setMostrarBloquesConfiguracion(function(v){return !v;});}} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",border:0,background:"transparent",padding:0,color:"var(--text)",cursor:"pointer",textAlign:"left"}}><span style={{fontSize:11.8,fontWeight:650}}>Cuerpo editable de Reserva</span><span style={{fontSize:10.5,color:"var(--dim)"}}>{mostrarBloquesConfiguracion?"▲ Ocultar":"▼ Mostrar"}</span></button>{mostrarBloquesConfiguracion&&<div style={{marginTop:9}}><div style={{fontSize:10.4,color:"var(--dim)",lineHeight:1.45,marginBottom:9}}>Acá se modifica el texto de los bloques del documento. Los datos del formulario de Operaciones siguen conectados automáticamente.</div>{bloquesError&&<div style={{marginBottom:10,padding:"8px 10px",borderRadius:8,background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.25)",color:"var(--red)",fontSize:11.2}}>{bloquesError}</div>}{bloquesLoading&&bloquesReserva.length===0&&<div style={{fontSize:11.5,color:"var(--dim)"}}>Cargando bloques…</div>}{(!bloquesLoading||bloquesReserva.length>0)&&<ReservaBloquesPanel bloques={Array.isArray(bloquesReserva)?bloquesReserva:[]} onUpdate={onUpdateBloqueReserva} onMove={onMoveBloqueReserva} clausulasLib={Array.isArray(clausulas)?clausulas:[]} operaciones={Array.isArray(operaciones)?operaciones:[]} puedeEditar={puedeEditarPlantillas}/>}</div>}</div>}
                  </div>
                </div>}
              </div>
      ...
      {tab==="auditoria"&&(function(){
        ...
        var [auditEntidad,setAuditEntidad]=useState("todas");
        ...
        return <div>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:10}}>
            <div><div style={{fontSize:15,fontWeight:600,color:"var(--text)"}}>Registro de auditoría</div><div style={{fontSize:12,color:"var(--dim)",lineHeight:1.45}}>Cada cambio queda asociado a la persona, fecha, módulo y registro afectado. En las ediciones se conserva exactamente el valor anterior y el nuevo.</div></div>
            <span className="badge" style={{background:"rgba(74,222,128,.08)",color:"var(--green)"}}>● Historial protegido</span>
          </div>
          {auditoriaSupabase&&auditoriaSupabase.error&&<div style={{marginBottom:16,padding:"10px 14px",borderRadius:10,background:"rgba(248,113,113,.08)",border:"1px solid rgba(248,113,113,.25)",color:"var(--red)",fontSize:12.5}}>{auditoriaSupabase.error}</div>}
          <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
            <Inp value={auditQuery} onChange={function(e){setAuditQuery(e.target.value);}} placeholder="Buscar persona, módulo o registro…" style={{flex:1,minWidth:220}}/>
            <Slt value={auditEntidad} onChange={function(e){setAuditEntidad(e.target.value);}} style={{minWidth:150}}><option value="todas">Todos los módulos</option><option value="clausula">Cláusulas</option><option value="bloque">Bloques</option><option value="operacion">Operaciones</option><option value="plantilla">Plantillas</option><option value="configuracion">Configuración</option><option value="usuario">Usuarios</option></Slt>
            <Slt value={auditAccion} onChange={function(e){setAuditAccion(e.target.value);}} style={{minWidth:145}}><option value="todas">Todas las acciones</option><option value="crear">Creaciones</option><option value="editar">Ediciones</option><option value="eliminar">Eliminaciones</option><option value="restaurar">Restauraciones</option><option value="importar">Importaciones</option></Slt>
          </div>
          {entries.length===0&&<div className="card" style={{padding:30,textAlign:"center"}}><p style={{color:"var(--dim)",fontSize:13}}>{auditoriaSupabase&&auditoriaSupabase.loading?"Cargando…":(auditLog.length===0?"Todavía no hay movimientos registrados.":"No hay resultados para ese filtro.")}</p></div>}
          <div style={{display:"flex",flexDirection:"column",gap:8}}>{entries.map(function(en){
            var abierta=auditExpandida===en.id;
            return <div key={en.id} className="card" style={{padding:"12px 14px"}}>
              <button type="button" onClick={function(){setAuditExpandida(abierta?null:en.id);}} style={{width:"100%",border:0,background:"transparent",padding:0,color:"inherit",cursor:"pointer",textAlign:"left"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}>
                  <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap"}}><span style={{fontSize:10.5,fontWeight:700,padding:"3px 8px",borderRadius:20,color:ACCION_COLOR[en.accion]||"var(--dim)",background:"rgba(148,163,184,.10)"}}>{ACCION_LABEL[en.accion]||en.accion}</span><span className="badge" style={{fontSize:9.5}}>{en.entidadLabel}</span><span style={{fontSize:13,fontWeight:650,color:"var(--text)"}}>{en.clausulaTitulo||"(sin título)"}</span></div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:10.5,color:"var(--dim)"}}>{new Date(en.ts).toLocaleString("es-AR")}</span><span style={{fontSize:12,color:"var(--dim)"}}>{abierta?"▲":"▼"}</span></div>
                </div>
                <div style={{fontSize:11.5,color:"var(--muted)",marginTop:5}}>Por <b style={{color:"var(--text)"}}>{en.actorNombre}</b>{en.actorRol?" — "+(ROLES_DEF[en.actorRol]?.label||en.actorRol):""} · {resumenCambio(en)}</div>
              </button>
              {abierta&&<div style={{marginTop:11,paddingTop:11,borderTop:"1px solid var(--border)",display:"flex",flexDirection:"column",gap:12}}>
                {(!en.cambios||!en.cambios.length)&&<div style={{fontSize:11.5,color:"var(--dim)"}}>No hay diferencias de campos para mostrar.</div>}
                {(en.cambios||[]).map(function(c,i){return <div key={i}>
                  <div style={{fontSize:11,fontWeight:700,color:"var(--text)",marginBottom:6}}>{c.label}</div>
                  <div style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:8}}>
                    <div><div style={{fontSize:9.5,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:"var(--red)",marginBottom:4}}>Anterior</div>{cajaValor(c.antes,!c.antes)}</div>
                    <div><div style={{fontSize:9.5,fontWeight:700,textTransform:"uppercase",letterSpacing:".05em",color:"var(--green)",marginBottom:4}}>Nuevo</div>{cajaValor(c.despues,!c.despues)}</div>
                  </div>
                </div>})}
              </div>}
            </div>;
