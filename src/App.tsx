...
var APP_VERSION = "2_39";
var APP_TITLE = "DocWorks 2_39";
...
async function invokeDocworksAI(payload){
  var client = await loadSupabaseJs();
  if(!client) throw new Error("No se pudo inicializar Supabase.");
  var result = await client.functions.invoke("ia-docworks", {body:payload});
  if(result.error){
    var msg = result.error.message || "No se pudo conectar con la IA.";
    if(result.data && result.data.error) msg = result.data.error;
    throw new Error(msg);
  }
  if(!result.data) throw new Error("La IA no devolvió una respuesta.");
  return result.data;
}
  ...
  return(
    <div style={{
      ...
      <div style={{animation:"splashPop 0.7s cubic-bezier(0.175,0.885,0.32,1.275) both",display:"flex",flexDirection:"column",alignItems:"center",gap:0}}>
        ...
        <div style={{fontFamily:"DM Serif Display,serif",fontSize:36,color:"#f1f5f9",marginTop:-4,letterSpacing:"-0.5px"}}>{APP_TITLE}</div>
...
async function narrarClausulaIA(ideaBreve){
  var data = await invokeDocworksAI({
    mode:"clause",
...
function BotIA({onClose,operaciones,clausulas,onAddClausula}){
  ...
  async function callClaude(newMsgs, systemExtra){
    var data = await invokeDocworksAI({
      mode: systemExtra ? "clause" : "chat",
      messages:newMsgs.map(function(m){
        return {role:m.role,content:(m.apiContent!==undefined?m.apiContent:m.content)};
      })
    });
    ...
    <div>
      ...
      {tab==="cuenta"&&(
        <div className="card" style={{padding:20,marginTop:16}}>
          ...
          <div style={{fontSize:12.5,color:"var(--dim)",marginBottom:16,lineHeight:1.6}}>Los datos de negocio se guardan en Supabase. Podés exportar un respaldo JSON del estado actual para conservar una copia; al importar, los datos vuelven a guardarse en Supabase.</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:400,display:"flex",alignItems:"center",justifyContent:"center",background:AUTH_BG,padding:16,fontFamily:"'DM Sans',sans-serif"}}>
      <form onSubmit={submit} style={{width:"100%",maxWidth:340,background:AUTH_CARD,border:"1px solid "+AUTH_BORDER,borderRadius:16,padding:28,display:"flex",flexDirection:"column",gap:14,boxShadow:"0 8px 30px rgba(0,0,0,0.08)"}}>
        <div style={{textAlign:"center",marginBottom:6}}>
          <div style={{fontSize:22,fontWeight:800,color:AUTH_TEXT}}>{APP_TITLE}</div>
    ...
    <div style={{position:"fixed",inset:0,zIndex:400,overflow:"auto",background:"var(--bg)",color:"var(--text)",padding:24,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <Modal open={showNueva} onClose={function(){setShowNueva(false);}} title="Nueva inmobiliaria">
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <Field label="Email del dueño/admin (recibe la invitación)"><input className="inp" type="email" value={nuevaForm.email} onChange={function(e){setNuevaForm(function(f){return Object.assign({},f,{email:e.target.value});});}} placeholder="dueno@inmobiliaria.com"/></Field>
...
// ── PERFIL DE LA INMOBILIARIA: fuente de verdad en Supabase ─────────────────
// Los datos comerciales/de presentación de la inmobiliaria viven en una sola
// fila por tenant. El nombre del usuario actual se sincroniza con `usuarios`.
function usePerfilSupabase(ctx){
  var [perfil,setPerfil]=useState(DEFAULT_PERFIL);
  ...
  function recargar(){
    if (!inmobId) { setPerfil(DEFAULT_PERFIL); setLoading(false); return; }
    setLoading(true); setError(null);
    rpc("fn_perfil_inmobiliaria_obtener", {}).then(function(res){
      setLoading(false);
      if (res.error) { setError(traducirError(res.error.message)); return; }
      var data = res.data || {};
      var base = Object.assign({}, DEFAULT_PERFIL, data);
      base.nombre_usuario = (usuario && usuario.nombre) || data.nombre_usuario || "";
      base.rol = (usuario && usuario.rol_base) || data.rol || "dueno";
      setPerfil(base);

      // Migración única del perfil viejo. Si Supabase todavía no tenía perfil,
      // aprovechamos el dato local existente y, una vez guardado, lo eliminamos.
      if (data._cloud_missing === true) {
        var legacy = lsGet("perfil", null);
        if (legacy && legacy.nombre) {
          var migrado = Object.assign({}, base, legacy, { rol:base.rol });
          rpc("fn_perfil_inmobiliaria_guardar", { p_datos:migrado }).then(function(r){
            if (!r.error) { lsRemove("perfil"); setPerfil(migrado); }
          });
        }
      }
    }).catch(function(err){
      setLoading(false); setError(traducirError(err && err.message ? err.message : "Error cargando el perfil."));
    });
  }

  useEffect(function(){ recargar(); }, [inmobId, usuario && usuario.id, usuario && usuario.nombre, usuario && usuario.rol_base]);

  function guardar(nextPerfil){
    var payload = Object.assign({}, nextPerfil || perfil, { rol:(usuario && usuario.rol_base) || perfil.rol });
    setSaving(true); setError(null);
    return rpc("fn_perfil_inmobiliaria_guardar", { p_datos:payload }).then(function(res){
      setSaving(false);
      if (res.error) { setError(traducirError(res.error.message)); return res; }
      var data = Object.assign({}, DEFAULT_PERFIL, res.data || payload);
      data.nombre_usuario = (usuario && usuario.nombre) || data.nombre_usuario || "";
      data.rol = (usuario && usuario.rol_base) || data.rol;
      setPerfil(data);
      return res;
    }).catch(function(err){
      setSaving(false); setError(traducirError(err && err.message ? err.message : "Error guardando el perfil."));
      return { error:{ message:err && err.message ? err.message : "Error guardando el perfil." } };
    });
  }

  return { perfil:perfil, loading:loading, saving:saving, error:error, recargar:recargar, guardar:guardar };
}
...
function useAuditoriaSupabase(ctx){
  ...
  function recargar(){
    ...
    loadSupabaseJs().then(function(sb){
      if (!sb) { setLoading(false); setError("No se pudo conectar con Supabase."); return; }
      return sb.from("auditoria").select("id,accion,entidad_tipo,valor_anterior,valor_nuevo,creada_en,usuarios!auditoria_usuario_id_fkey(nombre,email,rol_base)").eq("inmobiliaria_id", inmobId).order("creada_en",{ascending:false}).limit(300).then(function(res){
        setLoading(false);
        if (res.error) { setError(traducirError(res.error.message)); return; }
  ...
  function vaciar(){
    return loadSupabaseJs().then(function(sb){
      if (!sb) return {error:{message:"No se pudo conectar con Supabase."}};
      return sb.rpc("fn_auditoria_vaciar", {}).then(function(res){
        if (!res.error) setEntries([]);
        else setError(traducirError(res.error.message));
        return res;
      });
    });
  }
  return { entries:entries, loading:loading, error:error, recargar:recargar, vaciar:vaciar };
}
...
export default function App(){
  ...
  const perfilSupabase=usePerfilSupabase(docworksAuth.contexto);
  const perfil=perfilSupabase.perfil;
  const [equipo,setEquipo]=useState([]);
  useEffect(function(){ if (!equipoSupabase.loading) setEquipo(equipoSupabase.equipo); }, [equipoSupabase.equipo, equipoSupabase.loading]);
  // La auditoría es 100% cloud. No mantenemos una segunda copia local.
  const auditLog=auditoriaSupabase.entries;
  ...
  // Guardado: los datos de negocio se persisten en Supabase. localStorage queda
  // reservado para preferencias de UI y borradores temporales.
  ...
  // ── Exportar / Importar respaldo completo (JSON) ──
  // Exporta el estado actual que viene de Supabase. La importación ya no
  // escribe estados locales: perfil/cláusulas/operaciones vuelven a Supabase.
  ...
  async function importarRespaldo(rawText, modo) {
    ...
    if (d.perfil) {
      var pr=await perfilSupabase.guardar(Object.assign({}, DEFAULT_PERFIL, d.perfil));
      if (pr && pr.error) errores.push("Perfil: "+traducirError(pr.error.message));
    }

    // Las cláusulas se importan mediante sus RPC para respetar RLS/permisos.
    // En modo fusionar se agregan como copias; en reemplazar se agregan también
    // porque la app no hace DELETE físico de cláusulas protegidas.
    if (Array.isArray(d.clausulas)) {
      for (var ci=0; ci<d.clausulas.length; ci++) {
        var c=d.clausulas[ci];
        var cr=await clausulaRpc("fn_clausula_crear", { p_titulo:c.titulo, p_contenido:c.contenido, p_categoria:c.categoria||"general", p_tipos:c.tipos||["todos"], p_obligatoria:!!c.obligatoria, p_proteccion:c.proteccion||"B" });
        if (cr && cr.error) errores.push("Cláusula "+(c.titulo||"sin título")+": "+traducirError(cr.error.message));
      }
      recargarClausulas();
    }

    if (Array.isArray(d.operaciones)) {
      for (var oi=0; oi<d.operaciones.length; oi++) {
        var op=d.operaciones[oi];
        var or=await opsSupabase.guardar(op);
        if (or && or.error) errores.push("Operación "+(op.id||"sin ID")+": "+traducirError(or.error.message));
      }
    }

    // Equipo y auditoría no se importan desde un JSON: son datos administrados
    // por Auth/RPC y el historial debe permanecer íntegro.
    if (errores.length) return { ok:false, error:errores.join("\n") };
    return { ok:true, warning:"El equipo y la auditoría no se importan; ambos son administrados por Supabase." };
  }
    ...
    <>
      ...
      <div className="mobile-topbar">
        <div className="mobile-topbar-logo">
          ...
          <span className="mobile-logo-text">{APP_TITLE}</span>
        </div>
      ...
      <div className="app-wrap">
        ...
        <div className="topbar">
          <div className="topbar-logo">
            ...
            <div className="topbar-logo-text">{APP_TITLE}</div>
          </div>
