...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .topbar-logo img{height:46px;width:auto;object-fit:contain;filter:drop-shadow(0 0 8px rgba(212,168,83,0.35));}
      ...
      {tab==="plantillas"&&(function(){
        ...
        return(
          <div>
            ...
            {tipoDocSel==="reserva" && (<>
              {bloquesError && <div style={{marginBottom:14,padding:"10px 14px",borderRadius:10,background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.25)",color:"var(--red)",fontSize:12.5}}>{bloquesError}</div>}
              {bloquesLoading && bloquesReserva.length===0 && <div style={{marginBottom:14,fontSize:12.5,color:"var(--dim)"}}>Cargando bloques…</div>}
            ...
            {puedeEditarPlantillas && lista.length>0 && (
              <div style={{fontSize:11,color:"var(--dim)",marginBottom:10,display:"flex",alignItems:"flex-start",gap:6}}>
                <span>☑</span>
                <span>Tildá «Incluir por defecto» en las cláusulas que querés que ya vengan marcadas al crear una operación nueva de este tipo. Marcá 🔒 «Obligatoria» en el formulario si el broker nunca debe poder sacarla. Usá ↑ ↓ o arrastrá cada cláusula para ordenarlas — el orden acá define el orden en el documento final.</span>
              </div>
            )}
            ...
            <div style={{display:"flex",flexDirection:"column",gap:7}}>
              ...
              {lista.map(function(c,idxLista){
                ...
                return(
                  <div key={c.id} className="op-card" draggable={puedeEditarPlantillas}
                    ...
                    onDrop={function(e){
                      ...
                      var from=listaIds.indexOf(fromId), to=listaIds.indexOf(c.id);
                      if(from<0||to<0||from===to)return;
                      var dir=from<to?"down":"up";
                      var steps=Math.abs(to-from);
                      var currentIds=listaIds.slice();
                      for(var z=0;z<steps;z++){
                        var idx=currentIds.indexOf(fromId);
                        var swap=dir==="up"?idx-1:idx+1;
                        if(idx<0||swap<0||swap>=currentIds.length)break;
                        onMoveClausula(fromId,dir,currentIds);
                        var tmp=currentIds[idx]; currentIds[idx]=currentIds[swap]; currentIds[swap]=tmp;
                      }
                      setDragClausulaId(null);
                    }}
                    onDragEnd={function(){setDragClausulaId(null);}}
                    style={{cursor:puedeEditarPlantillas?(dragClausulaId===c.id?"grabbing":"grab"):"default",opacity:dragClausulaId===c.id?0.65:1}}>
                    <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:8}}>
                      <div style={{flex:1}}>
                        <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:3,flexWrap:"wrap"}}>
                          <span style={{fontSize:13,fontWeight:500,color:"var(--text)"}}>{c.titulo||"Todo OK"}</span>
...
export default function App(){
  ...
  // Reordena una cláusula dentro de la biblioteca. Es un reordenamiento
  // visual local (la tabla `clausulas` no tiene columna de orden todavía),
  // así que no persiste entre dispositivos — queda como mejora pendiente.
  function moveClausula(id, dir, visibleIds){
    setClausulas(function(cs){
      ...
      var ordenA=cs[i1].orden, ordenB=cs[i2].orden;
      clausulaRpc("fn_clausula_mover",{p_clausula_id:id,p_nuevo_orden:ordenB}).then(function(res){ if(res.error) setClausulasError(traducirError(res.error.message)); });
      clausulaRpc("fn_clausula_mover",{p_clausula_id:otherId,p_nuevo_orden:ordenA}).then(function(res){ if(res.error) setClausulasError(traducirError(res.error.message)); });
      var next = cs.slice();
      next[i1]=Object.assign({},cs[i1],{orden:ordenB});
      next[i2]=Object.assign({},cs[i2],{orden:ordenA});
      return next;
    });
  }
