...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  const [plantillaSubtab,setPlantillaSubtab] = useState("configuracion");
        ...
        return <div>
          <div className="card" style={{padding:13,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:18,color:"var(--text)"}}>Plantillas</div><SmallPill tone="gold">Plantilla por defecto</SmallPill></div><div style={{fontSize:10.9,color:"var(--dim)",marginTop:3}}>Cada plantilla parte de un modelo estándar que la inmobiliaria puede adaptar a su gusto, sin romper los campos del formulario de Operaciones.</div></div>
            ...
            <div className="card" style={{padding:9}}>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:4}}>
                {PLANTILLA_TIPOS.map(function(tipo){
                  ...
                  return <button key={tipo} onClick={function(){setTipoDocSel(tipo);setPlantillaSubtab("configuracion");setPlantillaForm(null);setDragClausulaId(null);setEncabezadoEditing(false);setConfiguracionPlantillaAbierta(false);setMostrarPreviewConfiguracion(false);setMostrarBloquesConfiguracion(false);}} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,width:"100%",border:active?"1px solid rgba(212,168,83,.25)":"1px solid transparent",borderRadius:9,padding:"9px 10px",background:active?"rgba(212,168,83,.10)":"transparent",color:active?"var(--text)":"var(--muted)",cursor:"pointer",textAlign:"left"}}>
