...
function ConfiguracionView({ perfil, onChange, darkMode, onToggleDark, equipo, equipoSupabase, operaciones, clausulas, onAddClausula, onEditClausula, onDeleteClausula, onMoveClausula, onDuplicateClausula, bloquesReserva, onUpdateBloqueReserva, onMoveBloqueReserva, bloquesError, bloquesLoading, papelera, auditLog, auditoriaSupabase, onExportarRespaldo, onImportarRespaldo, onVaciarAuditoria, puedeInstalar, onInstalar }) {
  ...
  function CuentaDocumentoPreview(){
    ...
    return <div className="cuenta-doc-preview-wrap">
      <div className="cuenta-doc-preview-label"><span>Vista previa · primera hoja</span><span style={{color:"var(--gold)"}}>El tamaño y posición se actualizan en vivo</span></div>
      <div className="cuenta-doc-page" style={{fontFamily:serif}}>
        <div className="cuenta-doc-header" style={{background:headerBg,borderBottom:estilo==="clasico"?"2px solid #1a1a1a":"none"}}>
          {localPerfil.logoDataUrl&&<div style={{textAlign:logoAlign,marginBottom:10}}><img src={localPerfil.logoDataUrl} style={{maxWidth:logoWidth,maxHeight:logoMaxH,width:"auto",height:"auto",objectFit:"contain"}} alt="Logo"/></div>}
          <div style={{fontSize:14,fontWeight:700,color:headerColor,letterSpacing:".02em",textAlign:estilo==="clasico"?"center":"left"}}>{(localPerfil.nombre||"TU INMOBILIARIA").toUpperCase()}</div>
          {(localPerfil.matricula||localPerfil.web)&&<div style={{fontSize:9.5,color:estilo==="corporativo"?"#ffffffaa":"#64748b",marginTop:3,textAlign:estilo==="clasico"?"center":"left"}}>{localPerfil.matricula?"Matrícula: "+localPerfil.matricula:""}{localPerfil.matricula&&localPerfil.web?" · ":""}{localPerfil.web||""}</div>}
        </div>
        <div className="cuenta-doc-body">
          ...
          <div style={{fontSize:10.5,color:"#334155",lineHeight:1.65}}><b style={{color:estilo==="clasico"?"#1a1a1a":prim}}>CLÁUSULA PRIMERA — OBJETO:</b> El vendedor transfiere al comprador el inmueble sito en... Este texto representa cómo se percibe visualmente el comienzo del documento.</div>
          <div style={{marginTop:18,fontSize:10.5,color:"#334155",lineHeight:1.65}}>La vista previa es orientativa: sirve para ajustar logo, encabezado y pie de página antes de generar el PDF o Word definitivo.</div>
        </div>
  ...
  return (
    <div>
      ...
      {tab==="cuenta"&&(
        <div>
          <div className="cuenta-subtabs">
            {[{id:"resumen",label:"👤 Datos de la cuenta"},{id:"preview",label:"📄 Vista previa del documento"}].map(function(t){
              return <button key={t.id} className={"cuenta-subtab"+(cuentaSubtab===t.id?" active":"")} onClick={function(){setCuentaSubtab(t.id);}}>{t.label}</button>;
            })}
          </div>
          {cuentaSubtab==="preview"&&<div className="card" style={{padding:18}}><div style={{fontSize:15,fontFamily:"DM Serif Display,serif",color:"var(--text)",marginBottom:3}}>Vista previa del documento</div><div style={{fontSize:12,color:"var(--dim)",marginBottom:14}}>Probá tamaño, posición del logo y pie de página sin generar todavía el archivo.</div><CuentaDocumentoPreview/></div>}
          {cuentaSubtab!=="preview"&&<div className="two-col-layout">
          {/* Panel dueño */}
