...
// DocWorks 3.55 PRO — edición profesional, visor unificado, IA asistida, dictado por voz, revisión inteligente y lenguaje 100% en español.
// DocWorks 3.55 PRO — IA asistida con dictado por voz, revisión inteligente, búsqueda ampliada, cláusulas obligatorias y experiencia 100% en español.
    ...
    <div>
      ...
      {tab==="historial"&&(function(){
        ...
        var statsPorMiembro=equipo.map(function(m){
          ...
          return {
            miembro:m,
            total:misOperacioneslength,
            activas:misOperaciones.filter(function(o){return o.estado==="activo";}).length,
            cerradas:misOperaciones.filter(function(o){return o.estado==="cerrado";}).length,
            borrador:misOperaciones.filter(function(o){return o.estado==="borrador";}).length,
            volumen:misOperacionesreduce(function(s,o){
  ...
  return (
    <div className="sesion-badge" style={{position:"relative",display:"flex",alignItems:"center"}}>
      <button type="button" onClick={function(){setAbierto(!abierto);}} aria-label="Abrir menú de perfil" style={{display:"flex",alignItems:"center",gap:8,background:"transparent",border:0,cursor:"pointer",padding:"4px 6px",color:"#f1f5f9"}}>
        <span style={{width:30,height:30,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,var(--gold),#8f6b23)",color:"#fff",fontWeight:800,fontSize:12}}>{inicial}</span>
