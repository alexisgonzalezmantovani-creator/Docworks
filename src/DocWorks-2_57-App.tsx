  ...
  return (
    <div>
      ...
      {tab==="plantillas"&&(function(){
        ...
        function ClausulaRow({c,idx,sectionList}){
          var esDefault=!!CLAUSULAS_DEFAULT.find(function(d){return d.id===c.id;});
          var porDefecto=defaultsSeleccionados.indexOf(c.id)!==-1;
          var esPrimera=idx===0, esUltima=idx===sectionList.length-1;
          return <div key={c.id} className="op-card" draggable={puedeEditarPlantillas}
            onDragStart={function(e){if(!puedeEditarPlantillas)return;setDragClausulaId(c.id);try{e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",c.id);}catch(_){}}}
            onDragOver={function(e){if(puedeEditarPlantillas){e.preventDefault();try{e.dataTransfer.dropEffect="move";}catch(_){}}}}
            onDrop={function(e){
              if(!puedeEditarPlantillas)return; e.preventDefault();
              var fromId=dragClausulaId||((e.dataTransfer&&e.dataTransfer.getData("text/plain"))||"");
              if(!fromId||fromId===c.id){setDragClausulaId(null);return;}
              moveClausulaTo(fromId,c.id,listaIds); setDragClausulaId(null);
            }}
            onDragEnd={function(){setDragClausulaId(null);}}
            style={{padding:"10px 12px",cursor:puedeEditarPlantillas?(dragClausulaId===c.id?"grabbing":"grab"):"default",opacity:dragClausulaId===c.id?0.65:1}}>
            <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
              <div style={{width:28,height:28,borderRadius:8,background:"rgba(212,168,83,0.08)",border:"1px solid rgba(212,168,83,0.18)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:700,color:"var(--gold)",flexShrink:0}}>{idx+1}</div>
              <div style={{flex:1,minWidth:0}}>
                <div style={{display:"flex",alignItems:"center",gap:7,flexWrap:"wrap",marginBottom:3}}>
                  <span style={{fontSize:13,fontWeight:600,color:"var(--text)"}}>{c.titulo||"Todo OK"}</span>
                  {esDefault&&<span className="badge badge-gold">Estándar</span>}
                  {!esDefault&&<span className="badge badge-activo">Mi cláusula</span>}
                  {porDefecto&&<span className="badge" style={{background:"rgba(45,212,191,0.12)",color:"var(--teal)"}}>✓ Por defecto</span>}
                  {c.obligatoria&&<span className="badge" style={{background:"rgba(180,83,9,0.12)",color:"#b45309"}}>🔒 Obligatoria</span>}
                </div>
                <div style={{fontSize:11.5,color:"var(--dim)",lineHeight:1.45,display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical",overflow:"hidden"}}>{c.contenido||"Sin texto."}</div>
                {puedeEditarPlantillas&&<label style={{display:"inline-flex",alignItems:"center",gap:6,marginTop:7,fontSize:10.8,color:"var(--muted)",cursor:"pointer"}}>
                  <input type="checkbox" checked={porDefecto} onChange={function(){toggleDefault(c);}}/> Usar en operaciones nuevas
                </label>}
              </div>
              {puedeEditarPlantillas&&<div style={{display:"flex",alignItems:"center",gap:3,flexShrink:0}}>
                <button title="Subir" onClick={function(){if(!esPrimera)onMoveClausula(c.id,"up",sectionList);}} disabled={esPrimera} style={{width:29,height:29,borderRadius:7,background:"rgba(255,255,255,0.03)",border:"1px solid var(--border)",color:esPrimera?"var(--border2)":"var(--muted)",cursor:esPrimera?"default":"pointer"}}>↑</button>
                <button title="Bajar" onClick={function(){if(!esUltima)onMoveClausula(c.id,"down",sectionList);}} disabled={esUltima} style={{width:29,height:29,borderRadius:7,background:"rgba(255,255,255,0.03)",border:"1px solid var(--border)",color:esUltima?"var(--border2)":"var(--muted)",cursor:esUltima?"default":"pointer"}}>↓</button>
                <button title="Duplicar" onClick={function(){onDuplicateClausula(c.id);}} style={{width:29,height:29,borderRadius:7,background:"rgba(255,255,255,0.03)",border:"1px solid var(--border)",color:"var(--muted)",cursor:"pointer"}}>⧉</button>
                <button title="Editar" onClick={function(){editarClausula(c);}} style={{width:29,height:29,borderRadius:7,background:"rgba(255,255,255,0.03)",border:"1px solid var(--border)",color:"var(--muted)",cursor:"pointer"}}>✏</button>
                <button title="Eliminar" onClick={function(){onDeleteClausula(c.id);}} style={{width:29,height:29,borderRadius:7,background:"rgba(248,113,113,0.05)",border:"1px solid rgba(248,113,113,0.15)",color:"var(--red)",cursor:"pointer"}}>✕</button>
              </div>}
            </div>
          </div>;
        }
        function SeccionClausulas({title,items,subtext}){
          return <div style={{marginBottom:18}}>
            <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between",gap:10,marginBottom:8}}>
              <div><div style={{fontSize:13.5,fontWeight:650,color:"var(--text)"}}>{title} <span style={{fontSize:11,color:"var(--dim)",fontWeight:500}}>({items.length})</span></div><div style={{fontSize:10.8,color:"var(--dim)",marginTop:2}}>{subtext}</div></div>
            </div>
            {!items.length&&<div style={{padding:"18px 14px",border:"1px dashed var(--border2)",borderRadius:10,color:"var(--dim)",fontSize:12,textAlign:"center"}}>No hay cláusulas en esta sección.</div>}
            <div style={{display:"flex",flexDirection:"column",gap:6}}>{items.map(function(c){return <ClausulaRow key={c.id} c={c} idx={lista.indexOf(c)} sectionList={lista}/>;})}</div>
          </div>;
        }
        return <div>
          <div className="card" style={{padding:14,marginBottom:12}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12,flexWrap:"wrap"}}>
              <div><div style={{fontFamily:"DM Serif Display,serif",fontSize:17,color:"var(--text)"}}>Plantillas</div><div style={{fontSize:11.5,color:"var(--dim)",marginTop:2}}>Elegí un documento y administrá sus cláusulas desde una sola vista.</div></div>
              {puedeEditarPlantillas&&<Btn s="sm" onClick={nuevaClausula}>＋ Nueva cláusula</Btn>}
            </div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:12}}>{Object.keys(TIPOS).map(function(k){
          ...
          {!puedeEditarPlantillas&&<div style={{padding:"9px 12px",marginBottom:12,borderRadius:9,background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.22)",color:"var(--amber)",fontSize:11.5}}>🔒 Solo Dueño y Administrador pueden modificar plantillas. Podés consultarlas.</div>}

          <div className="card" style={{padding:12,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
              <div><div style={{fontSize:12.5,fontWeight:650,color:"var(--text)"}}>Configuración rápida</div><div style={{fontSize:10.8,color:"var(--dim)",marginTop:2}}>Encabezado y vista previa sin ocupar toda la pantalla.</div></div>
              <span className="badge badge-activo">{customTextoPreview?"Encabezado personalizado":"Encabezado estándar"}</span>
            </div>
            <details style={{marginTop:10}}>
              <summary style={{fontSize:11.5,color:"var(--gold)",cursor:"pointer"}}>Abrir encabezado y vista previa</summary>
              <div style={{marginTop:10}}>
                <PlantillaPreview tipo={tipoDocSel} encabezado={encabezadoEditing?encabezadoDraft:customTextoPreview} clausulas={lista}/>
                <div style={{fontSize:10.5,color:"var(--dim)",marginTop:6}}>La vista previa usa datos de ejemplo. El cuerpo fijo del documento sigue funcionando igual.</div>
              </div>
            </details>
          </div>

          {tipoDocSel==="reserva" && (<div className="card" style={{padding:12,marginBottom:14}}>
            <details>
              <summary style={{fontSize:12.5,fontWeight:650,color:"var(--text)",cursor:"pointer"}}>⚙ Bloques de Reserva</summary>
              <div style={{marginTop:10}}>
                {bloquesError && <div style={{marginBottom:10,padding:"8px 10px",borderRadius:8,background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.25)",color:"var(--red)",fontSize:11.5}}>{bloquesError}</div>}
          ...
          {plantillaForm&&puedeEditarPlantillas&&<div className="card" style={{padding:14,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}><div style={{fontFamily:"DM Serif Display,serif",fontSize:15,color:"var(--text)"}}>{plantillaForm.editId?"Editar cláusula":"Nueva cláusula"}</div><button onClick={function(){setPlantillaForm(null);}} style={{border:0,background:"transparent",color:"var(--dim)",fontSize:18,cursor:"pointer"}}>×</button></div>
            <ClausulaFormBody
              form={plantillaForm}
          ...
          <SeccionClausulas title="Cláusulas estándar" items={estandar} subtext="Son las cláusulas base de DocWorks para este tipo de documento. Se mantienen separadas de las que agregás vos."/>
          <SeccionClausulas title="Mis cláusulas" items={personalizadas} subtext="Cláusulas propias de tu inmobiliaria. Podés editarlas, duplicarlas y ordenarlas."/>

          {puedeEditarPlantillas&&lista.length>0&&<div style={{padding:"9px 11px",borderRadius:9,background:"rgba(255,255,255,0.02)",border:"1px solid var(--border)",fontSize:10.8,color:"var(--dim)",lineHeight:1.45}}>💡 Arrastrá una cláusula para cambiar su posición. También podés usar ↑ ↓. «Usar en operaciones nuevas» marca las que aparecerán seleccionadas automáticamente.</div>}
        </div>;
      })()}
