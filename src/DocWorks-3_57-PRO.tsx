...
// DocWorks 3.57 PRO — edición profesional, visor unificado, IA asistida, dictado por voz, revisión inteligente y lenguaje 100% en español.
// DocWorks 3.57 PRO — ajustes de navegación, identidad visual, historial del equipo y consistencia de operaciones.
...
const GS = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .topbar-logo{display:flex;align-items:center;gap:10px;width:260px;height:100%;flex-shrink:0;overflow:hidden;}
  ...
  .sesion-badge .logout-btn{color:var(--muted) !important;background:transparent !important;}
  .sesion-badge .logout-btn:hover{background:var(--hover-bg) !important;color:var(--text) !important;}
...
function DashboardView({operaciones,clausulas,perfil,bloquesReserva,onNew,onEdit,onDelete,onChangeEstado,onConvertir,onConvertirComodato,onConvertirRefuerzo,onConvertirBoleto,onConvertirDevolucion,onDuplicar,onViewDoc,onPerfil,searchOverride,papelera,onRestore,onDeletePermanente,onVaciarPapelera,confirmVaciarPapelera,setConfirmVaciarPapelera,papeleraJump,error,loading}){
  ...
  function renderOpRow(op,nested){
    ...
    return(
      <div key={op.id} className="op-card op-card-premium fade-up" style={Object.assign({position:"relative",marginTop:hijosOp.length?8:0,minHeight:128,boxSizing:"border-box"},removiendo?{opacity:0,transform:"translateX(28px) scale(0.96)",pointerEvents:"none"}:{})}>
  ...
  return (
    <div>
      ...
      {tab==="historial"&&(function(){
        var filteredOps=(Array.isArray(operaciones)?operaciones:[]).filter(function(o){
          if(histBroker!=="todos"&&o.broker_id!==histBroker)return false;
          if(histEstado!=="todos"&&o.estado!==histEstado)return false;
          return true;
        });
        var statsPorMiembro=(Array.isArray(equipo)?equipo:[]).map(function(m){
  ...
  return (
    <div className="sesion-badge" style={{position:"relative",display:"flex",alignItems:"center"}}>
      ...
      {abierto&&<div style={{position:"absolute",right:0,top:"calc(100% + 7px)",minWidth:190,zIndex:500,background:"var(--card)",border:"1px solid var(--border2)",borderRadius:12,boxShadow:"var(--shadow-lg)",padding:6}}>
        ...
        <button type="button" onClick={function(){setAbierto(false);onLogout();}} className="logout-btn" style={{width:"100%",textAlign:"left",padding:"9px 10px",border:0,borderRadius:8,cursor:"pointer",fontSize:12,fontWeight:700}}>↪ Salir</button>
      </div>}
    </div>
  );
}
    ...
    <>
      ...
      <div className="mobile-topbar">
        <div className="mobile-topbar-logo">
          <img src={(perfil&&perfil.logoDataUrl)||LOGO_B64} alt={(perfil&&perfil.nombre)||"DocWorks"} style={{height:30,maxWidth:108,width:"auto",objectFit:"contain"}}/>
          <span className="mobile-logo-text">{(perfil&&perfil.nombre)||"DocWorks"} <span style={{fontSize:10,opacity:0.7}}>v{DOCWORKS_VERSION}</span></span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
      ...
      <div className="app-wrap">
        ...
        <div className="topbar">
          <div className="topbar-logo" title={perfil&&perfil.nombre?perfil.nombre:"DocWorks"}>
            {perfil&&perfil.logoDataUrl
              ? <img src={perfil.logoDataUrl} alt={perfil.nombre||"Logo de la inmobiliaria"} style={{height:42,maxWidth:152,width:"auto",objectFit:"contain",objectPosition:"left center",display:"block",filter:"drop-shadow(0 0 12px rgba(212,168,83,0.28))"}}/>
            ...
            <div className="topbar-logo-text">{perfil&&perfil.nombre?perfil.nombre:"DocWorks"} <span style={{fontSize:10,opacity:0.65,fontFamily:"DM Sans,sans-serif",fontWeight:500}}>v{DOCWORKS_VERSION}</span></div>
          </div>
